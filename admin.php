...
if (isset($_SESSION['is_admin'])) {
    ...
    if (isset($_POST['save_telegram_bot']) || isset($_POST['register_telegram_webhook'])) {
        ...
        save_telegram_bot_config($botToken, $botUsername, $adminChatId, $channelIds, $premiumEmojiId, $extraEmojis);
    ...
    if (isset($_POST['create_reseller'])) {
        $rentUnit = $_POST['reseller_rent_unit'] ?? 'forever';
        $expiresAt = null;
        if ($rentUnit !== 'forever') {
            $rentValue = max(1, (int)($_POST['reseller_rent_value'] ?? 1));
            $expiresAt = time() + duration_to_seconds($rentValue, $rentUnit);
        }
        $resellerResult = create_reseller($_POST['reseller_username'] ?? '', $_POST['reseller_password'] ?? '', $expiresAt);
    }
    if (isset($_POST['toggle_reseller_id'])) {
        toggle_reseller((int)$_POST['toggle_reseller_id']);
    }
    if (isset($_POST['delete_reseller_id'])) {
        delete_reseller((int)$_POST['delete_reseller_id']);
    }
    if (isset($_POST['extend_reseller_id'])) {
        $extUnit = $_POST['extend_unit'] ?? 'month';
        $extSeconds = $extUnit === 'forever' ? null : duration_to_seconds(max(1, (int)($_POST['extend_value'] ?? 1)), $extUnit);
        extend_reseller((int)$_POST['extend_reseller_id'], $extSeconds);
    }
    ...
    // Lưu link liên hệ MẶC ĐỊNH toàn hệ thống (nút liên hệ nổi ở
    // index.php) - reseller nào chưa tự đặt riêng sẽ dùng link này.
...
$tab = $_GET['tab'] ?? 'stats';
if (!in_array($tab, ['keys', 'game', 'link', 'channel', 'notice', 'server', 'stats', 'reseller'], true)) $tab = 'stats';

// Icon nét mảnh (svg_icon()) giờ dùng bản DÙNG CHUNG trong config.php
// (gộp chung với reseller.php + thêm icon cho index.php) - gỡ bản trùng
...
<?php else: ?>
    ...
    <div class="topbar">
        ...
        <b style="font-size:15px"><?= ['keys'=>'Tạo Key','game'=>'Cấu hình Game','link'=>'Cấu hình Link','channel'=>'Cấu hình kênh','notice'=>'Thông báo người dùng','server'=>'Server App Android','stats'=>'Thống kê','reseller'=>'Quản lý Reseller'][$tab] ?? '' ?></b>
    ...
    <div class="sidebar" id="sidebar">
        ...
        <div class="sidebar-nav">
            ...
            <a href="admin.php?tab=reseller" class="sidebar-item <?= $tab==='reseller'?'active':'' ?>">
                <span class="ic"><?= svg_icon('users') ?></span><span class="label">Quản lý Reseller</span>
                <?php if ($tab==='reseller'): ?><span class="chev"><?= svg_icon('chevron', 15) ?></span><?php endif; ?>
            </a>
    ...
    <div class="box">
        ...
        <p style="font-size:12px;color:var(--text-dim);margin-top:-4px">
            Hiện ở nút liên hệ nổi trên trang chủ (index.php). Reseller nào chưa tự đặt link riêng
            (ở trang Reseller của họ) sẽ dùng link mặc định này. Để trống ô nào thì icon đó tự ẩn.
        </p>
    ...
    <?php elseif ($tab === 'reseller'):
        $resellerList = get_resellers();
        $baseUrlForReseller = rtrim(BASE_URL, '/');
    ?>

    <div class="box">
        <h3>Tạo tài khoản Reseller</h3>
        <p style="font-size:12.5px;color:var(--text-dim);margin-top:-5px;max-width:660px">Mỗi reseller có Cấu hình Link, Cấu hình kênh và Tạo Key <b>riêng biệt hoàn toàn</b> (không đụng tới cấu hình của admin hay reseller khác), nhưng vẫn dùng chung hệ thống key/vượt link hiện tại. Reseller đăng nhập tại <code><?= htmlspecialchars($baseUrlForReseller) ?>/reseller.php</code>.</p>
        <?php if (!empty($resellerResult)): ?><p class="<?= $resellerResult['ok'] ? 'ok' : 'err' ?>"><?= htmlspecialchars($resellerResult['message']) ?></p><?php endif; ?>
        <form method="post">
            <input type="hidden" name="csrf_token" value="<?= htmlspecialchars($_SESSION['csrf_token'] ?? '') ?>">
            <input type="text" name="reseller_username" required maxlength="32" placeholder="Username (chữ/số/gạch dưới)" style="width:200px">
            <input type="text" name="reseller_password" required maxlength="64" placeholder="Mật khẩu (tối thiểu 6 ký tự)" style="width:200px"><br>
            Thời hạn thuê:
            <input type="number" name="reseller_rent_value" value="1" min="1" style="width:80px">
            <select name="reseller_rent_unit">
                <option value="day">Ngày</option>
                <option value="week">Tuần</option>
                <option value="month" selected>Tháng</option>
                <option value="forever">Vĩnh viễn</option>
            </select>
            <button type="submit" name="create_reseller" value="1">+ Tạo tài khoản</button>
            <p style="font-size:11px;color:var(--text-dim);margin:6px 0 0">Hết thời hạn thuê, tài khoản (kèm game/kênh/cấu hình riêng của họ) sẽ <b>TỰ ĐỘNG BỊ XOÁ</b>. Key họ đã phát cho user vẫn giữ nguyên, chỉ gỡ liên kết reseller.</p>
        </form>
    </div>

    <div class="box">
        <h3>Danh sách Reseller (<?= count($resellerList) ?>)</h3>
        <div class="tablewrap">
            <table>
                <tr><th>Username</th><th>Ngày tạo</th><th>Hạn thuê</th><th>Trạng thái</th><th>Trang riêng</th><th></th></tr>
                <?php foreach ($resellerList as $r):
                    $expired = !empty($r['expires_at']) && (int)$r['expires_at'] < time();
                ?>
                <tr>
                    <td style="font-family:monospace"><?= htmlspecialchars($r['username']) ?><?php if (!empty($r['store_name'])): ?><br><span style="font-size:10.5px;color:var(--text-dim)">🏷 <?= htmlspecialchars($r['store_name']) ?></span><?php endif; ?></td>
                    <td><?= fmt_time($r['created_at']) ?></td>
                    <td>
                        <?php if (empty($r['expires_at'])): ?><span class="badge on">vĩnh viễn</span>
                        <?php else: ?><span class="badge <?= $expired ? 'off' : 'on' ?>"><?= $expired ? 'đã hết hạn' : fmt_time($r['expires_at']) ?></span><?php endif; ?>
                        <form method="post" style="display:flex;gap:3px;margin-top:5px;align-items:center">
                            <input type="hidden" name="csrf_token" value="<?= htmlspecialchars($_SESSION['csrf_token'] ?? '') ?>">
                            <input type="hidden" name="extend_reseller_id" value="<?= (int)$r['id'] ?>">
                            <input type="number" name="extend_value" value="1" min="1" style="width:52px;min-height:28px;padding:3px 5px">
                            <select name="extend_unit" style="min-height:28px;padding:3px 4px;font-size:11px">
                                <option value="day">Ngày</option>
                                <option value="week">Tuần</option>
                                <option value="month" selected>Tháng</option>
                                <option value="forever">Vĩnh viễn</option>
                            </select>
                            <button class="small" type="submit" style="padding:4px 8px">Gia hạn</button>
                        </form>
                    </td>
                    <td><span class="badge <?= $r['enabled'] ? 'on' : 'off' ?>"><?= $r['enabled'] ? 'đang bật' : 'đã khoá' ?></span></td>
                    <td style="font-size:11px"><code>index.php?r=<?= (int)$r['id'] ?></code></td>
                    <td>
                        <form method="post" style="display:inline"><input type="hidden" name="csrf_token" value="<?= htmlspecialchars($_SESSION['csrf_token'] ?? '') ?>"><input type="hidden" name="toggle_reseller_id" value="<?= (int)$r['id'] ?>"><button class="warn small" type="submit"><?= $r['enabled'] ? 'Khoá' : 'Mở khoá' ?></button></form>
                        <form method="post" style="display:inline"><input type="hidden" name="csrf_token" value="<?= htmlspecialchars($_SESSION['csrf_token'] ?? '') ?>"><input type="hidden" name="delete_reseller_id" value="<?= (int)$r['id'] ?>"><button class="danger small" type="submit" onclick="return confirm('Xoá tài khoản reseller này? Game/kênh/cấu hình riêng của họ sẽ bị xoá theo. Key họ đã phát vẫn còn nhưng gỡ liên kết reseller.')">Xoá</button></form>
                    </td>
                </tr>
                <?php endforeach; ?>
                <?php if (empty($resellerList)): ?><tr><td colspan="6" style="color:var(--text-dim);text-align:center;padding:24px">Chưa có reseller nào.</td></tr><?php endif; ?>
            </table>
        </div>
    </div>
