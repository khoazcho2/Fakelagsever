...
function render_transition_screen(int $current, int $total, string $nextUrl, bool $done, bool $isAdmin = false): void {
    ...
    <!DOCTYPE html><html lang="vi"><head><meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <title><?= $done ? 'Hoàn tất' : 'Đã vượt bước ' . $current . '/' . $total ?></title>
    <?= shared_notice_head() ?>
    <style>
    :root{--bg:#070A10;--surface:#101620;--surface2:#151D2A;--cyan:#59F5D5;--violet:#9C8CFF;--text:#F4F7FB;--text-dim:#93A1B5;--line:rgba(177,199,224,.14);--success:#61E6A4}
    *{box-sizing:border-box}body{font-family:"Inter",-apple-system,Arial,sans-serif;min-height:100vh;margin:0;padding:24px 16px;display:grid;place-items:center;color:var(--text);background:radial-gradient(circle at 12% 0%,rgba(89,245,213,.16),transparent 28rem),radial-gradient(circle at 90% 12%,rgba(156,140,255,.13),transparent 26rem),var(--bg)}
    ...
    .content{padding:31px 26px 29px}
    .badge{width:58px;height:58px;margin:0 auto 16px;display:grid;place-items:center;border-radius:18px;background:rgba(97,230,164,.1);border:1px solid rgba(97,230,164,.22);color:var(--success);font-size:26px;box-shadow:0 0 0 8px rgba(97,230,164,.045);animation:popIn .5s cubic-bezier(.34,1.56,.64,1)}
    .eyebrow{font:10px "JetBrains Mono",monospace;letter-spacing:.16em;color:var(--cyan)}
    h1{font:700 24px "Space Grotesk",sans-serif;letter-spacing:-.03em;margin:10px 0 9px}
    .dots{display:flex;justify-content:center;gap:7px;margin:18px 0 4px}
    .dots .dot{width:9px;height:9px;border-radius:50%;background:var(--surface2);border:1px solid var(--line);transition:all .3s ease}
    .dots .dot.done{background:var(--success);border-color:var(--success)}
    .dots .dot.now{background:var(--cyan);width:22px;border-radius:5px;box-shadow:0 0 10px rgba(89,245,213,.5);border-color:transparent}
    .bar{background:var(--surface2);border-radius:8px;height:6px;overflow:hidden;margin:18px 0 4px}
    .fill{background:linear-gradient(90deg,var(--cyan),var(--violet));height:100%;width:0%;transition:width 1s ease}
    .step{display:inline-flex;align-items:center;gap:7px;margin-top:14px;padding:7px 12px;border-radius:999px;background:rgba(156,140,255,.11);color:#C9C0FF;font:10px "JetBrains Mono",monospace;letter-spacing:.06em}
    .spinner{display:inline-block;width:12px;height:12px;border:2px solid rgba(201,192,255,.25);border-top-color:#C9C0FF;border-radius:50%;animation:spin .7s linear infinite;flex-shrink:0}
    .btn{display:flex;align-items:center;justify-content:center;gap:9px;width:100%;min-height:50px;margin-top:22px;padding:13px 16px;border:0;border-radius:13px;color:#071018;background:linear-gradient(110deg,var(--cyan),#B5FFF0 45%,var(--violet));box-shadow:0 12px 25px -14px rgba(89,245,213,.8);font:700 14px "Space Grotesk",sans-serif;text-decoration:none;cursor:pointer;transition:transform .15s,filter .15s}
    ...
    </head><body><main class="card"><div class="holo"></div><div class="content">
    <div class="badge"><?= $done ? '🎉' : '✓' ?></div>
    <div class="eyebrow">AUTH PROGRESS</div>
    <h1><?= $done ? 'Đã vượt đủ link!' : 'Đã vượt bước ' . $current . '/' . $total ?></h1>
    <?php if ($total > 1): ?>
    <div class="dots">
