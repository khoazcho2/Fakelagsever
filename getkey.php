...
// - Duy nhất Key 24 giờ, bắt buộc vượt 2 lần link cho mọi game.
// ============================================================
...
if (isset($_REQUEST['direct_tg_verify'])) {
    ...
    if (($_SERVER['REQUEST_METHOD'] ?? '') !== 'POST') {
        ...
        header('Allow: POST');
        echo json_encode(['success' => false, 'message' => 'Vui lòng tải lại trang để xác nhận.']);
        exit;
    }
    $reqToken = trim($_POST['token'] ?? '');
    $reqGate = trim($_POST['gate_token'] ?? '');
    ...
    if ($identifier === '') {
        echo json_encode(['success' => false, 'message' => 'Vui lòng nhập mã TG- từ bot, @username hoặc ID Telegram.']);
        exit;
    }
    ...
    if (str_starts_with(strtoupper($identifier), 'TG-')) {
        $tgUserId = redeem_telegram_link_code($identifier, $reqToken) ?? 0;
        if ($tgUserId <= 0) {
            echo json_encode(['success' => false, 'verified' => false, 'message' => 'Mã không hợp lệ, đã hết hạn hoặc đã dùng cho phiên khác. Gửi /start cho bot để lấy mã mới.']);
            exit;
        }
        $uInfo = get_telegram_user_info($tgUserId);
        $cleanId = (string)($uInfo['username'] ?? '');
    ...
    if ($tgUserId <= 0) {
        echo json_encode([
            'success' => false,
            'need_id' => true,
            'message' => 'Chưa tìm thấy tài khoản này. Gửi /start cho bot rồi dán mã TG- trong phản hồi của bot vào ô này.'
        ]);
        exit;
    }

    // A typed public username/ID is not proof that the visitor owns that account.
    $bound = $db->prepare("SELECT tg_user_id FROM keys WHERE token = ? AND status = 'pending' AND created_at >= ?");
    $bound->execute([$reqToken, time() - STALE_PENDING_SECONDS]);
    if ((int)$bound->fetchColumn() !== $tgUserId) {
        echo json_encode(['success' => false, 'verified' => false, 'message' => 'Tài khoản chưa liên kết với phiên web này. Nếu đã /start, hãy sao chép mã TG- trong tin nhắn mới nhất của bot và dán vào ô này để xác nhận.']);
        exit;
    }
    $membership = check_user_membership_all_required_groups($tgUserId);
    if (!empty($membership['errors'])) {
        echo json_encode(['success' => false, 'verified' => false, 'message' => $membership['errors'][0]]);
        exit;
    }
    $reqGroups = array_column($membership['groups'], 'chat_id');
    ...
    mark_channel_verified($reqToken, 0, $tgUserId);
    ...
    // Lấy thông tin tên user để mention
    $uInfoFull = get_telegram_user_info($tgUserId);
    $firstNameForNotify = trim((string)($uInfoFull['first_name'] ?? ''));
    $usernameForNotify  = trim((string)($uInfoFull['username'] ?? $cleanId));

    // Gửi thông báo vào tất cả các nhóm: @mention user và nhắc họ quay lại web nhập tên + vượt link (kèm Telegram Premium Icon)
    send_all_groups_joined_notification($tgUserId, $usernameForNotify, $firstNameForNotify, $reqGroups, (bool)$isUserPremium);
...
// Gói cố định phía server; tham số key_type cũ không thay đổi thời hạn/số bước.
...
// cố định (24h -> 2 bước), không phụ thuộc admin cấu hình
// bao nhiêu ô.
...
if (count($chain) !== PUBLIC_KEY_HOPS || in_array('', $chain, true)) {
...
// Tự động dọn dẹp các key pending cũ bị bỏ dở của cùng IP cho game này (> 5 phút trước)
try {
    $db->prepare("DELETE FROM keys WHERE status = 'pending' AND client_ip = ? AND game_id = ? AND created_at < ?")
       ->execute([$clientIp, (int)$game['id'], time() - 300]);
} catch (\Throwable $e) {}
...
function render_channel_gate(array $game, array $channels, string $token, string $gateToken, ?array $notice, string $error = '', array $openedChannels = [], bool $isAdmin = false): void {
            ...
            <form method="post" action="getkey.php?game=<?= urlencode($game['slug']) ?>" id="channelForm" style="margin-top:var(--sp-4)">
                ...
                <div class="bot-start-alert">
                    ...
                    <div class="bot-start-title">
                        <span>🤖</span> KEY 24H · VƯỢT 2 LẦN — MỞ BOT ĐỂ XÁC NHẬN
                    </div>
                ...
                <div class="direct-tg-box">
                    <div class="direct-tg-head">
                        <span class="direct-tg-title">✈ <b>Mã từ bot / Username / ID:</b></span>
                        <a href="<?= htmlspecialchars($botStartDirectUrl) ?>" target="_blank" rel="noopener" class="direct-tg-help">Lấy mã từ bot</a>
                    </div>
                    <div class="direct-tg-input-wrap">
                        <input type="text" id="tgIdentifierInput" placeholder="Dán mã TG- từ bot nếu đã /start..." autocomplete="off" spellcheck="false">
    ...
    function startVerifyPoll(link){
        if(link.dataset.polling==='1')return;
        link.dataset.polling='1';
        const box=link.parentElement.querySelector('.check');
        const cid=link.dataset.channelOpen;
        const deadline=Date.now()+10*60*1000;
        let inFlight=false;
        const poll=setInterval(()=>{
            if(link.classList.contains('done')||Date.now()>deadline){clearInterval(poll);link.dataset.polling='0';link.dataset.started='0';return;}
            if(inFlight)return;
            inFlight=true;
    ...
    function onBotDirectStartClick(elem) {
        if (!elem) return;
        if (elem.dataset.polling === '1') return;
        elem.dataset.polling = '1';
        ...
        const globalDeadline = Date.now() + 10 * 60 * 1000;
        let globalInFlight = false;
        const globalPoll = setInterval(() => {
            if (Date.now() > globalDeadline) {
                clearInterval(globalPoll);
                elem.dataset.polling = '0';
                elem.textContent = 'Chưa nhận được xác nhận. Bấm mở bot để thử lại.';
                return;
            }
            ...
            fetch('getkey.php?channel_verify_poll=1&game=' + encodeURIComponent(gameSlug) + '&token=' + encodeURIComponent(openToken) + '&gate_token=' + encodeURIComponent(gateToken) + '&channel=0', { credentials: 'same-origin', cache: 'no-store' })
                ...
                }).catch(() => {}).finally(() => { globalInFlight = false; });
        }, 2000);
    }
    ...
    function runDirectTgVerify() {
        ...
        if (!val) {
            ...
            msg.textContent = 'Vui lòng nhập mã TG- từ bot, @username hoặc ID Telegram.';
            input.focus();
            return;
        }
        btn.disabled = true;
        ...
        fetch('getkey.php', {
            ...
            body: new URLSearchParams({direct_tg_verify:'1', game:gameSlug, token:openToken, gate_token:gateToken, identifier:val})
        })
        .then(r => {
            if (!r.ok) {
