...
function get_dashboard_stats(): array {
    ...
    $devRows = $db->query("SELECT devices FROM keys WHERE devices != ''")->fetchAll(PDO::FETCH_COLUMN);
    $uniqueDevices = [];
    foreach ($devRows as $devStr) {
        foreach (explode(',', $devStr) as $d) {
            if ($d !== '') $uniqueDevices[$d] = true;
        }
    }
...
// Tìm lịch sử theo keycode hoặc device_id (dùng cho tra cứu trong admin)
function search_device_history(string $q, int $limit = 100): array {
    $db = get_db();
    if ($q === '') {
        return $db->query("SELECT * FROM device_history ORDER BY id DESC LIMIT $limit")->fetchAll(PDO::FETCH_ASSOC);
    }
    $stmt = $db->prepare("SELECT * FROM device_history WHERE keycode LIKE ? OR device_id LIKE ? OR ip LIKE ? ORDER BY id DESC LIMIT $limit");
    $like = '%' . $q . '%';
...
function shared_favicon(): string {
    $svg = '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><defs><linearGradient id="g" x1="0" y1="0" x2="1" y2="1"><stop offset="0" stop-color="%2300E5C7"/><stop offset="1" stop-color="%238B7CFF"/></linearGradient></defs><rect width="24" height="24" rx="6" fill="%230B0E14"/><path d="M8 13a4 4 0 1 1 3.46-2H15l2 2-1.5 1.5L14 13h-.5l-1 1-1-1H8z" fill="url(%23g)"/></svg>';
