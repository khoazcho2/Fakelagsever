...
if (isset($_SESSION['is_admin'])) {
    ...
    if (isset($_POST['save_telegram_bot']) || isset($_POST['register_telegram_webhook'])) {
        ...
        // Tự động tìm username từ Telegram API nếu chưa nhập
        ...
        if (isset($_POST['register_telegram_webhook'])) {
            if ($botToken === '') {
                ...
                if (!empty($res['ok'])) {
                    ...
                    $channelError = '❌ Lỗi kích hoạt Webhook: ' . ($res['description'] ?? 'Không kết nối được tới Telegram API');
                }
            }
        } else {
            $channelSaved = true;
            $tgBotFeedback = 'Đã lưu cấu hình Bot Telegram và bộ Icon Premium thành công.';
        }
    }
    if (isset($_POST['test_admin_telegram_msg'])) {
        ...
        if ($botToken === '' || $adminChatId === '') {
            ...
            $testMsg = "<blockquote>" .
                       ...
                       "{$icD} <b>Giao hàng:</b> Tự động 100% (20 giờ)\n" .
...
<?php else: ?>
    ...
    <div class="box">
        <h3>Cấu hình Game</h3>
        <p style="font-size:12px;color:#888;margin-top:-4px">Cấu hình thứ tự nhà cung cấp rút gọn link riêng cho khách VN và nước ngoài, từng game. Số lần vượt/hạn key giờ do người dùng chọn ở màn nhiệm vụ (Key 24h/36h).</p>
        ...
        <div class="game-card">
            ...
            <form method="post"><input type="hidden" name="csrf_token" value="<?= htmlspecialchars($_SESSION['csrf_token'] ?? '') ?>">
                ...
                <div class="region-form">
                    ...
                    <p style="font-size:11.5px;color:var(--text-dim);margin:2px 0 0">Số lần vượt &amp; hạn key giờ do người dùng tự chọn ở màn nhiệm vụ (Key 24h/36h). Mục này chỉ còn dùng để chọn thứ tự provider.</p>
        ...
        <div class="region-form" style="margin-top:17px;border-color:rgba(89,245,213,.22)">
            ...
            <form method="post" enctype="multipart/form-data" style="display:flex;flex-direction:column;gap:10px">
                ...
                <div style="display:flex;flex-wrap:wrap;gap:10px">
                    <div style="flex:1;min-width:260px">
                        ...
                        <input type="text" name="tg_bot_token" value="<?= htmlspecialchars($tgBotCfg['token']) ?>" placeholder="vd: 7123456789:AAFxYz..." style="width:100%">
                    </div>
                ...
                <div style="display:flex;flex-wrap:wrap;gap:10px;margin-top:2px">
                    <div style="flex:1;min-width:260px">
                        ...
                        <span style="font-size:10.5px;color:var(--text-dim)">Chỉ đăng tin phát Key 20H/40H (che mã key &amp; tag user). Không gửi tin tham gia / rời nhóm vào đây.</span>
                    </div>
                    <div style="flex:1;min-width:200px">
            ...
            <?php if ($tgBotCfg['token'] !== ''): ?>
                <div style="margin-top:10px;font-size:11.5px;color:var(--text-dim);background:rgba(255,255,255,.03);padding:10px 12px;border-radius:6px;border:1px solid var(--border);line-height:1.6">
                    ...
                    <div style="margin-top:3px">Link đăng ký thủ công (nếu muốn mở trực tiếp trên trình duyệt): <a href="https://api.telegram.org/bot<?= htmlspecialchars($tgBotCfg['token']) ?>/setWebhook?url=<?= urlencode($webhookTargetUrl) ?>&amp;allowed_updates=%5B%22message%22%2C%22edited_message%22%2C%22callback_query%22%2C%22chat_member%22%2C%22my_chat_member%22%5D" target="_blank" rel="noopener" style="color:var(--cyan)">Mở link setWebhook (kèm theo dõi chat_member)</a></div>
            ...
            <?php if (!empty($tgBotStatusInfo)): ?>
                <div style="margin-top:10px;padding:10px 14px;background:rgba(0,0,0,.3);border-radius:6px;border:1px solid rgba(89,245,213,.35);font-size:12px;line-height:1.6">
                    ...
                    <?php else: ?>
                        <div style="color:var(--red);margin-top:4px">❌ Token không hợp lệ hoặc không kết nối được Telegram: <?= htmlspecialchars($tgBotStatusInfo['me']['description'] ?? 'Lỗi không xác định') ?></div>
                    <?php endif; ?>
