...
/* Game explorer: tìm nhanh và lọc các game đang sẵn sàng nhận key. */
.game-toolbar{max-width:620px;margin:0 auto 12px;display:flex;align-items:center;gap:10px;flex-wrap:wrap}
.search-field{height:44px;display:flex;align-items:center;gap:9px;flex:1;min-width:220px;padding:0 11px 0 13px;border:1px solid var(--line);border-radius:12px;background:rgba(var(--surface-rgb),.72);color:var(--text-dim);transition:border-color .15s,box-shadow .15s}
.search-field:focus-within{border-color:rgba(89,245,213,.52);box-shadow:0 0 0 3px rgba(89,245,213,.08)}
.search-field input{width:100%;min-width:0;border:0;outline:0;background:transparent;color:var(--text);font:500 12.5px 'Be Vietnam Pro',sans-serif;appearance:none}
.search-field input::-webkit-search-cancel-button{display:none}
.search-field input::placeholder{color:#738096}
.search-clear{width:24px;height:24px;padding:0;border:0;border-radius:7px;background:transparent;color:var(--text-dim);font-size:18px;line-height:1;cursor:pointer;opacity:0;pointer-events:none;transition:opacity .15s,background .15s}
.search-clear.visible{opacity:1;pointer-events:auto}
.search-clear:hover{background:rgba(255,255,255,.08);color:var(--text)}
.filter-group{display:flex;gap:4px;padding:4px;border:1px solid var(--line);border-radius:12px;background:rgba(var(--surface-rgb),.58)}
.filter-btn{height:34px;padding:0 10px;border:0;border-radius:8px;background:transparent;color:var(--text-dim);font:600 11px 'Be Vietnam Pro',sans-serif;cursor:pointer;white-space:nowrap;transition:color .15s,background .15s}
.filter-btn:hover{color:var(--text)}
.filter-btn.active{background:rgba(89,245,213,.14);color:var(--cyan)}
.game-results{max-width:620px;min-height:17px;margin:-2px auto 10px;padding-left:2px;color:var(--text-dim);font:10.5px 'JetBrains Mono',monospace;letter-spacing:.02em}
.game-list{max-width:620px;margin:0 auto;display:grid;grid-template-columns:repeat(2,minmax(0,1fr));grid-auto-flow:dense;gap:14px}
.game-list .card{width:auto;max-width:none;margin:0;display:flex;flex-direction:column}
.game-list .card .card-body{flex:1}
.game-list .card.is-hidden{display:none}
.game-no-results{max-width:620px;margin:10px auto 0;padding:22px 18px;border:1px dashed var(--line);border-radius:16px;text-align:center;color:var(--text-dim);font-size:12px}
@media (max-width:700px){.game-list{grid-template-columns:1fr}}
@media (max-width:520px){.game-toolbar{align-items:stretch}.search-field{flex-basis:100%}.filter-group{flex:1}.filter-btn{flex:1}}

/* Motion layer: GSAP sẽ điều khiển opacity/transform khi có thể; các
   thuộc tính này chỉ tối ưu repaint và không làm nội dung biến mất nếu
   CDN không tải được. */
[data-motion]{will-change:transform,opacity}
.guide-layout{max-width:620px;margin:0 auto;display:grid;grid-template-columns:minmax(120px,.35fr) minmax(0,1fr);gap:22px;align-items:start}
.guide-heading{position:relative}
.guide-heading .section-label{display:block;margin:0;line-height:1.45}
.guide-heading .section-label::after{display:block;width:42px;height:2px;margin-top:12px;flex:none;background:var(--cyan)}
.guide-steps{min-width:0}
.guide-layout .step-card{max-width:none;margin:0 0 10px}
.motion-init [data-motion]{opacity:0;transform:translate3d(0,24px,0)}
.motion-init .guide-heading{transform:translate3d(0,0,0)}
.word-reveal{display:inline-block;opacity:.2;transform:translateY(4px)}
@media (max-width:700px){
    .guide-layout{display:block}
    .guide-heading{margin-bottom:15px}
    .guide-heading .section-label::after{margin-top:9px}
}
@media (prefers-reduced-motion:reduce){
    [data-motion]{will-change:auto}
    .motion-init [data-motion]{opacity:1;transform:none}
...
(function(){
    function initMotion(){
        ...
        document.querySelectorAll('.feature-body p').forEach(function(paragraph){
            ...
            gsap.to(paragraph.querySelectorAll('.word-reveal'), {
                opacity:1, y:0, ease:'none', stagger:.045,
                scrollTrigger:{trigger:paragraph, start:'top 86%', end:'bottom 42%', scrub:.7}
            });
        });
