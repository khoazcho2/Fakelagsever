...
.server3d-overlay {
    ...
    background: radial-gradient(circle at 50% 45%, #0A1226 0%, #060810 75%);
    overflow: hidden;
...
.server3d-title {
    font: 800 clamp(36px, 8vw, 68px) var(--font-body);
    letter-spacing: -0.04em; margin: 0; line-height: 1.05;
    background: linear-gradient(135deg, #FFFFFF 15%, #00F2FE 55%, #C084FC 100%);
    -webkit-background-clip: text; -webkit-text-fill-color: transparent;
    filter: drop-shadow(0 0 35px rgba(0, 242, 254, 0.5));
}
.server3d-sub {
    font: 600 clamp(12px, 2.5vw, 14.5px) var(--font-mono);
    letter-spacing: 0.12em; color: var(--text-dim); text-transform: uppercase;
    margin: 0;
}
.server3d-terminal {
    width: 100%; max-width: 480px; padding: 12px 16px; border-radius: var(--r-lg);
    background: rgba(6, 8, 16, 0.78); border: 1px solid rgba(148, 163, 184, 0.18);
    backdrop-filter: blur(14px); text-align: left; font: 500 12px var(--font-mono);
    color: var(--text-dim); box-shadow: 0 16px 36px rgba(0,0,0,0.6);
    line-height: 1.6;
}
...
.server3d-progress {
    width: 100%; max-width: 480px; height: 6px; border-radius: var(--r-full);
...
.server3d-enter-btn {
    ...
    height: 52px; padding: 0 32px; border: 0; border-radius: var(--r-lg);
    background: var(--grad-btn); color: #060810; font: 800 15px var(--font-body);
    display: inline-flex; align-items: center; gap: 10px; cursor: pointer;
    box-shadow: var(--shadow-btn), 0 0 30px rgba(0, 242, 254, 0.4);
    transition: all var(--dur-normal) var(--ease-out);
}
.server3d-enter-btn:hover {
    transform: translateY(-2px) scale(1.03); filter: brightness(1.1);
    box-shadow: var(--shadow-btn-hover), 0 0 45px rgba(0, 242, 254, 0.6);
}
.server3d-enter-btn:active { transform: scale(0.98); }

.server3d-skip-btn {
    position: absolute; top: 20px; right: 24px; z-index: 10; pointer-events: auto;
    padding: 8px 16px; border-radius: var(--r-full); background: rgba(13, 19, 34, 0.7);
    border: 1px solid var(--line); color: var(--text-dim); font: 600 12px var(--font-mono);
    cursor: pointer; transition: all var(--dur-fast); backdrop-filter: blur(8px);
}
.server3d-skip-btn:hover {
    color: var(--cyan); border-color: var(--cyan); background: rgba(0, 242, 254, 0.1);
}
...
<div class="server3d-overlay" id="server3dIntro" role="dialog" aria-modal="true" aria-label="HOQUOC KEY 3D Server">
    ...
    <div class="server3d-hud">
        <div class="server3d-badge">
            <span class="server3d-badge-dot"></span>
            <span>3D CLOUD SERVER // HOQUOC KEYAUTH</span>
        </div>
        ...
        <div class="server3d-terminal" id="server3dTerminal">
            <div class="line"><span class="cyan">❯</span> <span class="highlight">[INIT]</span> Khởi tạo HoQuoc 3D Server Node...</div>
            <div class="line" id="termLine2" style="display:none"><span class="ok">✔</span> <span class="highlight">[SECURE]</span> 256-Bit Hardware Encryption: ONLINE</div>
...
(function(){
    ...
    function init3DServerIntro() {
        ...
        camera.position.set(0, 0.6, defaultCamZ);
        ...
        var cyanKey = new THREE.DirectionalLight(0x00f2fe, 2.2);
        cyanKey.position.set(5, 6, 6);
        scene.add(cyanKey);

        var violetFill = new THREE.PointLight(0x8b5cf6, 3.5, 20);
        violetFill.position.set(-5, -3, 4);
        scene.add(violetFill);

        var coreGlowLight = new THREE.PointLight(0x00f2fe, 3.0, 8);
        coreGlowLight.position.set(0, 0.2, 0);
        scene.add(coreGlowLight);

        // 3D SERVER NODE GROUP
        var serverNode = new THREE.Group();
        scene.add(serverNode);

        // 1. Central Server Chassis (Tower)
        var chassisGeo = new THREE.BoxGeometry(1.6, 3.4, 1.6);
        ...
        serverNode.add(chassis);

        // 2. Corner Neon Energy Conduits (4 Pillars)
        var conduitGeo = new THREE.CylinderGeometry(0.04, 0.04, 3.42, 12);
        var conduitMat = new THREE.MeshBasicMaterial({ color: 0x00f2fe });
        var offsets = [
            [-0.78, -0.78], [0.78, -0.78], [-0.78, 0.78], [0.78, 0.78]
        ];
        offsets.forEach(function(pos){
            var c = new THREE.Mesh(conduitGeo, conduitMat);
            c.position.set(pos[0], 0, pos[1]);
            serverNode.add(c);
        });

        // 3. Server Bay Blade Drawers (6 horizontal racks)
        var bladeGeo = new THREE.BoxGeometry(1.48, 0.4, 1.58);
        var bladeMat = new THREE.MeshStandardMaterial({
            color: 0x11192e,
            metalness: 0.8,
            roughness: 0.3
        });
        var ledGeo = new THREE.BoxGeometry(0.035, 0.035, 0.02);
        var leds = [];
        var ledColors = [0x00f2fe, 0x10b981, 0xf59e0b, 0x8b5cf6];

        for (var b = 0; b < 6; b++) {
            var yPos = (b - 2.5) * 0.52;
            var blade = new THREE.Mesh(bladeGeo, bladeMat);
            blade.position.y = yPos;
            serverNode.add(blade);

            // Blinking LED activity lights on front face of blade
            for (var l = 0; l < 4; l++) {
                var lMat = new THREE.MeshBasicMaterial({ color: ledColors[l % ledColors.length] });
                var led = new THREE.Mesh(ledGeo, lMat);
                led.position.set(-0.55 + l * 0.36, yPos, 0.8);
                serverNode.add(led);
                leds.push({ mesh: led, mat: lMat, baseColor: ledColors[l % ledColors.length], speed: 1.5 + Math.random() * 3.5 });
            }
        }

        // 4. Quantum Core (Central Floating Octahedron Crystal)
        var coreGeo = new THREE.OctahedronGeometry(0.52, 0);
        ...
        var coreWireMat = new THREE.MeshBasicMaterial({ color: 0x8b5cf6, wireframe: true });
        var coreWire = new THREE.Mesh(coreGeo, coreWireMat);
        coreWire.scale.set(1.22, 1.22, 1.22);
        core.add(coreWire);

        // 5. Concentric Quantum Hologram Data Rings (3 Gimbal Rings)
        var ringMat1 = new THREE.MeshBasicMaterial({ color: 0x00f2fe });
        var ringMat2 = new THREE.MeshBasicMaterial({ color: 0x8b5cf6 });
        var ringMat3 = new THREE.MeshBasicMaterial({ color: 0xec4899 });

        var ring1 = new THREE.Mesh(new THREE.TorusGeometry(2.3, 0.016, 16, 120), ringMat1);
        var ring2 = new THREE.Mesh(new THREE.TorusGeometry(2.7, 0.016, 16, 120), ringMat2);
        var ring3 = new THREE.Mesh(new THREE.TorusGeometry(3.1, 0.014, 16, 120), ringMat3);
        ...
        // 6. Tron 3D Wireframe Grid Floor
        var grid = new THREE.GridHelper(32, 28, 0x00f2fe, 0x152240);
        grid.position.y = -2.4;
        scene.add(grid);

        // 7. Swirling Particle Starfield (Cyber Dust)
        ...
        function enterSystem() {
            ...
            // Camera warp flight into the glowing 3D core
        ...
        function animate() {
            ...
            if (!isDragging) {
                if (!prefersReducedMotion) {
                    serverNode.rotation.y += 0.008;
                    // Parallax tilt towards cursor
                    serverNode.rotation.x += ((mouse.y * 0.25 + 0.08) - serverNode.rotation.x) * 0.05;
                }
            }

            // Quantum Core & Wireframe counter-rotation
            core.rotation.y = time * 1.5;
            core.rotation.x = Math.sin(time) * 0.5;
            coreWire.rotation.y = -time * 2.2;
            coreMat.emissiveIntensity = 0.6 + Math.sin(time * 3) * 0.4;

            // Rotating Hologram Data Rings
            ring1.rotation.x += 0.018;
            ring1.rotation.y += 0.024;
            ring2.rotation.y += 0.014;
            ring2.rotation.z += 0.022;
            ...
            for (var i = 0; i < leds.length; i++) {
                var l = leds[i];
                var pulse = (Math.sin(time * l.speed + i) + 1) * 0.5;
                l.mat.color.setHex(pulse > 0.4 ? l.baseColor : 0x07111e);
            }
