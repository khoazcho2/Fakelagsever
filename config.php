...
// CSS animation dùng chung cho các trang thông báo (tránh lặp code)
function shared_notice_css(): string {
    return '*{box-sizing:border-box}
    body{font-family:-apple-system,Arial,sans-serif;max-width:400px;margin:80px auto;background:#0f1115;color:#eee;text-align:center;padding:0 16px;animation:fadeIn .4s ease}
    .box{background:#181b22;padding:24px;border-radius:12px;animation:popIn .4s cubic-bezier(.34,1.56,.64,1)}
    button,a.btn{display:inline-block;margin-top:14px;padding:10px 18px;background:#4f8cff;border-radius:8px;color:#fff;text-decoration:none;border:none;font-weight:bold;cursor:pointer;transition:transform .12s,filter .15s}
...
function render_shorten_error(): void {
    ...
    .box{max-width:460px}
    .debug{margin-top:18px;text-align:left;background:#0f1115;border-radius:8px;padding:12px;font-size:12px;color:#e0b23f;word-break:break-all;animation:fadeIn .3s ease .1s both}
    .debug b{color:#fff}
    ...
    <h3>⚠️ Không tạo được link rút gọn</h3>
...
function render_transition_screen(int $current, int $total, string $nextUrl, bool $done): void {
    ...
    *{box-sizing:border-box}
    body{font-family:-apple-system,Arial,sans-serif;max-width:360px;margin:80px auto;background:#0f1115;color:#eee;text-align:center;padding:0 16px;animation:fadeIn .4s ease}
    .box{background:#181b22;padding:26px 20px;border-radius:12px;animation:popIn .4s cubic-bezier(.34,1.56,.64,1)}
    .bar{background:#262a33;border-radius:8px;height:10px;overflow:hidden;margin:16px 0}
    .fill{background:linear-gradient(90deg,#4f8cff,#7db2ff);height:100%;width:0%;transition:width 1s ease}
    .step{font-size:14px;color:#aaa}
    .spinner{display:inline-block;width:16px;height:16px;border:2px solid #333;border-top-color:#4f8cff;border-radius:50%;animation:spin .7s linear infinite;vertical-align:middle;margin-right:6px}
    .checkmark{font-size:36px;animation:popIn .5s cubic-bezier(.34,1.56,.64,1)}
    a.btn{display:inline-block;margin-top:16px;padding:10px 20px;background:#4f8cff;border-radius:8px;color:#fff;text-decoration:none;font-weight:bold;transition:transform .12s}
    ...
    <h3 style="margin:8px 0 6px"><?= $done ? 'Đã vượt đủ link!' : 'Đã vượt bước ' . $current . '/' . $total ?></h3>
    ...
    <script>
    // Chạy progress bar mượt bằng JS thay vì set cứng % ngay lập tức
