...
function render_transition_screen(int $current, int $total, string $nextUrl, bool $done): void {
    ...
    *{box-sizing:border-box}
    ...
    <?php endif; ?>
    <p class="step"><span class="spinner" id="spin"></span><span id="stepText"><?= $done ? 'Đang chuyển tới trang nhận key...' : 'Vui lòng chờ để tiếp tục...' ?></span></p>
    <a class="btn" href="<?= htmlspecialchars($nextUrl) ?>" id="nextBtn"<?= $done ? '' : ' style="pointer-events:none;opacity:.5"' ?>><?= $done ? 'Nhận key ngay' : ('Tiếp tục (' . TRANSITION_WAIT_SECONDS . 's)') ?></a>
    </div>
    <script>
