...
function evaluate_groups_and_build_ui(int $tgUserId, string $token, int $channelId, string $firstName = 'bạn', bool $isPremium = false): array {
    ...
    if ($allJoined) {
        ...
        // Liệt kê các key user đang sở hữu nếu có
        $myKeysLines = [];
        try {
            $stmtMy = get_db()->prepare("SELECT keycode, duration_seconds, total_hops, status FROM keys WHERE tg_user_id = ? ORDER BY id DESC LIMIT 2");
            $stmtMy->execute([$tgUserId]);
            $myRows = $stmtMy->fetchAll(PDO::FETCH_ASSOC);
            foreach ($myRows as $mk) {
                $p = detect_key_package((int)($mk['duration_seconds'] ?? 72000), (int)($mk['total_hops'] ?? 1));
                $sttText = $mk['status'] === 'active' ? 'Đang dùng' : 'Pending';
                $myKeysLines[] = "• <code>{$mk['keycode']}</code> | <b>{$p['badge']}</b> ({$sttText})";
            }
        } catch (\Throwable $e) {}
        $myKeysSection = !empty($myKeysLines) ? ("\n🔑 <b>Key bạn đang có:</b>\n" . implode("\n", $myKeysLines) . "\n") : "";
        ...
        $text = "<blockquote>" .
                ...
                "</blockquote>\n" .
                "<i>👉 Bạn đã đủ điều kiện! Bấm nút bên dưới để mở Web nhận Key ngay:</i>";

        // CHỈ HIỂN THỊ NÚT MỞ WEB KHI ĐÃ ĐƯỢC BOT KIỂM TRA ĐỦ NHÓM!
        $keyboard = [
            'inline_keyboard' => [
                [
                    ['text' => '🌐 Mở trang Web lấy Key 🚀', 'url' => $webUrl]
                ]
            ]
        ];
