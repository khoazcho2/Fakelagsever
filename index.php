...
try {
    ...
    $counterDebugError = $e->getMessage();
    $totalActivated = 0;
}
// DEBUG TẠM THỜI - xem đúng file DB thật sự đang dùng + số liệu sống,
// để xác định index.php có đang đọc CÙNG 1 file keys.db với admin.php
// hay không. Xoá khối này sau khi debug xong.
try {
    $debugDb = get_db();
    $counterDebugInfo = [
        'CONFIG_BUILD_TAG' => defined('CONFIG_BUILD_TAG') ? CONFIG_BUILD_TAG : '(KHÔNG có - config.php CŨ)',
        'DB_PATH'        => defined('DB_PATH') ? DB_PATH : '(chưa định nghĩa)',
        'realpath'       => defined('DB_PATH') ? (realpath(DB_PATH) ?: '(không resolve được)') : '',
        'file_exists'    => defined('DB_PATH') ? (file_exists(DB_PATH) ? 'có' : 'KHÔNG') : '',
        'file_size'      => defined('DB_PATH') && file_exists(DB_PATH) ? filesize(DB_PATH) . ' bytes' : '',
        'file_mtime'     => defined('DB_PATH') && file_exists(DB_PATH) ? date('Y-m-d H:i:s', filemtime(DB_PATH)) : '',
        'total_keys'     => (int)$debugDb->query("SELECT COUNT(*) FROM keys")->fetchColumn(),
        'activated_keys' => (int)$debugDb->query("SELECT COUNT(*) FROM keys WHERE activated_at IS NOT NULL")->fetchColumn(),
        'counter_row'    => $debugDb->query("SELECT value FROM site_counters WHERE name='keys_issued'")->fetchColumn(),
    ];
} catch (Throwable $e) {
    $counterDebugInfo = ['error' => $e->getMessage()];
}
?>
<!DOCTYPE html>
<html lang="vi">
...
<div class="wrap">
    ...
    <?php // Luôn hiện (bỏ điều kiện >0 tạm thời) để debug: nếu bạn vẫn
    // không thấy dòng này trên trình duyệt sau khi deploy code mới,
    // nghĩa là server đang chạy CODE CŨ (cache/chưa restart), không phải
    // lỗi logic đếm - vì dòng này giờ luôn render bất kể giá trị. ?>
    <div class="proof"><span class="pulse-dot"></span> Đã phát <b><?= number_format($totalActivated) ?></b> key thành công <small style="opacity:.4">(v2)</small></div>
    <?php if (!empty($counterDebugError)): ?>
    <div style="max-width:520px;margin:8px auto 0;padding:8px 10px;border:1px solid rgba(255,120,133,.3);border-radius:8px;background:rgba(255,120,133,.08);color:#FF9CA6;font-size:11px;font-family:monospace;word-break:break-all">DEBUG lỗi counter: <?= htmlspecialchars($counterDebugError) ?></div>
    <?php endif; ?>
    <?php if (!empty($counterDebugInfo)): ?>
    <div style="max-width:520px;margin:8px auto 0;padding:10px 12px;border:1px solid rgba(89,245,213,.25);border-radius:10px;background:rgba(89,245,213,.06);color:#9FD;font-size:10.5px;font-family:monospace;text-align:left;word-break:break-all;line-height:1.7">
        <?php foreach ($counterDebugInfo as $k => $v): ?><?= htmlspecialchars($k) ?>: <b><?= htmlspecialchars((string)$v) ?></b><br><?php endforeach; ?>
    </div>
    <?php endif; ?>
