...
function get_resellers(): array {
    return get_db()->query("SELECT id, username, enabled, created_at FROM resellers ORDER BY id DESC")->fetchAll(PDO::FETCH_ASSOC);
}

function get_reseller_by_id(int $id): ?array {
    $stmt = get_db()->prepare("SELECT id, username, enabled, created_at FROM resellers WHERE id = ?");
    $stmt->execute([$id]);
    $row = $stmt->fetch(PDO::FETCH_ASSOC);
    return $row ?: null;
}
