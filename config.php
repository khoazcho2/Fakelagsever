...
// Gọi hàm này mỗi khi tạo xong 1 link/token Get Key mới cho 1 IP.
function record_link_generation_attempt(string $ip): void {
    if ($ip === '') return;
    $db = get_db();

    db_execute($db->prepare("INSERT INTO link_gen_attempts (ip, created_at) VALUES (?, ?)"), [$ip, time()]);
    ...
    if ($burstCount >= LINK_SPAM_BURST_MAX) {
        ...
        return; // đã band vĩnh viễn, khỏi cần check tiếp mốc 2
    }
