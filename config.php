...
function supabase_upsert_backup(array $payload): bool {
    ...
    if (!$ok) {
        error_log('[supabase backup] thất bại http=' . $httpCode . ' resp=' . substr((string)$resp, 0, 300));
    }
    return $ok;
}
...
function backup_to_supabase(PDO $pdo): void {
    ...
    try {
        ...
        @file_put_contents($statusFile, json_encode(['ok' => $ok, 'at' => time()]));

        if ($prevOk && !$ok) {
            telegram_notify("⚠️ <b>Sao lưu Supabase bắt đầu LỖI</b> - kiểm tra lại cấu hình/bảng backups.");
        } elseif (!$prevOk && $ok) {
