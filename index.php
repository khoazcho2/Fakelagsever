...
?>
<!DOCTYPE html>
<html lang="<?= $GLOBALS['LANG'] === 'en' ? 'en' : 'vi' ?>">
<head>
<meta charset="UTF-8">
...
/* IP của bạn */
.ip-box{max-width:620px;margin:20px auto 0;padding:14px 18px;border:1px solid var(--line);border-radius:16px;background:rgba(16,22,32,.7);display:flex;align-items:center;justify-content:space-between;gap:10px;flex-wrap:wrap}
...
.brand-name{font-family:'Space Grotesk',sans-serif;font-weight:700;font-size:15px;letter-spacing:-.01em}
...
.fab-item{width:46px;height:46px;border-radius:50%;display:flex;align-items:center;justify-content:center;font-size:20px;font-weight:700;text-decoration:none;color:#fff;box-shadow:0 10px 22px -8px rgba(0,0,0,.6);transition:transform .15s}
.fab-item:hover{transform:scale(1.08)}
.fab-zalo{background:#0068FF}
.fab-tele{background:#29A9EA}
.fab-fb{background:#1877F2}
.fab-toggle{width:56px;height:56px;border-radius:50%;border:0;cursor:pointer;font-size:24px;color:#071018;background:linear-gradient(135deg,var(--cyan),var(--violet));box-shadow:0 12px 28px -10px rgba(89,245,213,.7);transition:transform .2s ease}
...
<div class="wrap">
    ...
    <h1><?= t('Lấy Key Game', 'Get Game Keys') ?></h1>
    ...
    <div class="howto">
        <div class="howto-step"><div class="howto-num">1</div><div class="howto-text"><?= t('Chọn game bên dưới', 'Pick a game below') ?></div></div>
        <div class="howto-step"><div class="howto-num">2</div><div class="howto-text"><?= t('Vượt link rút gọn', 'Complete the shortlink steps') ?></div></div>
        <div class="howto-step"><div class="howto-num">3</div><div class="howto-text"><?= t('Nhận key ngay', 'Get your key instantly') ?></div></div>
    </div>

    <div class="reset-box" id="reset-section">
    ...
    <div class="card" style="animation-delay:<?= $i * 0.07 ?>s">
        ...
        <div class="card-cta">
            <?php if ($cooldown > 0): ?>
                <a class="btn btn-disabled" data-remaining="<?= $cooldown ?>" onclick="return false;">⏳ <?= t('Còn', 'Wait') ?> <span class="cd-text"><?= format_duration_label($cooldown) ?></span></a>
            <?php else: ?>
                <a class="btn" href="getkey.php?game=<?= urlencode($g['slug']) . $rParam ?>">⚡ <?= t('Lấy key miễn phí', 'Get free key') ?></a>
            <?php endif; ?>
        </div>
    </div>
...
<?php if (CONTACT_ZALO_URL !== '' || CONTACT_TELEGRAM_URL !== '' || CONTACT_FACEBOOK_URL !== ''): ?>
<div class="fab-contact">
    <div class="fab-menu" id="fabMenu">
        <?php if (CONTACT_ZALO_URL !== ''): ?><a class="fab-item fab-zalo" href="<?= htmlspecialchars(CONTACT_ZALO_URL) ?>" target="_blank" rel="noopener" title="Zalo">Z</a><?php endif; ?>
        <?php if (CONTACT_TELEGRAM_URL !== ''): ?><a class="fab-item fab-tele" href="<?= htmlspecialchars(CONTACT_TELEGRAM_URL) ?>" target="_blank" rel="noopener" title="Telegram">✈️</a><?php endif; ?>
        <?php if (CONTACT_FACEBOOK_URL !== ''): ?><a class="fab-item fab-fb" href="<?= htmlspecialchars(CONTACT_FACEBOOK_URL) ?>" target="_blank" rel="noopener" title="Facebook">f</a><?php endif; ?>
    </div>
    <button type="button" class="fab-toggle" id="fabToggle" aria-label="<?= t('Liên hệ', 'Contact') ?>" aria-expanded="false">💬</button>
</div>
<script>
(function(){
