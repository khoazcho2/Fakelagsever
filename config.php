...
function get_resellers(): array {
    return get_db()->query("SELECT id, username, store_name, enabled, created_at FROM resellers ORDER BY id DESC")->fetchAll(PDO::FETCH_ASSOC);
}

function get_reseller_by_id(int $id): ?array {
    $stmt = get_db()->prepare("SELECT id, username, store_name, enabled, created_at FROM resellers WHERE id = ?");
    $stmt->execute([$id]);
    $row = $stmt->fetch(PDO::FETCH_ASSOC);
    return $row ?: null;
}
...
// Trả về ['ok'=>bool, 'message'=>string]
function create_reseller(string $username, string $password): array {
    ...
    db_execute(
        get_db()->prepare("INSERT INTO resellers (username, password_hash, enabled, created_at) VALUES (?, ?, 1, ?)"),
        [$username, password_hash($password, PASSWORD_DEFAULT), time()]
    );
    return ['ok' => true, 'message' => "Đã tạo tài khoản reseller \"{$username}\"."];
}
