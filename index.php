...
body {
    ...
    overflow-x: hidden;
    background:
        radial-gradient(circle at 15% -8%, rgba(89, 245, 213, 0.14), transparent 28rem),
        radial-gradient(circle at 90% 16%, rgba(156, 140, 255, 0.12), transparent 25rem),
...
.wrap { position: relative; max-width: 760px; margin: 0 auto; padding: 36px 20px 68px; }

/* App-bar */
.app-bar {
    display: flex; align-items: center; gap: 12px; max-width: 760px; margin: 0 auto 16px;
    padding: 10px 16px; border: 1px solid var(--line); border-radius: var(--r-xl);
    background: rgba(var(--surface-rgb), 0.75); backdrop-filter: blur(12px); -webkit-backdrop-filter: blur(12px);
...
.app-bar-icon {
    ...
    box-shadow: 0 4px 12px rgba(89,245,213,.25);
}
.app-bar-name { display: flex; flex-direction: column; flex: 1; min-width: 0; }
.app-bar-name b { font-size: 14px; font-weight: 700; display: flex; align-items: center; gap: 5px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; color: var(--text); }
.app-bar-name b svg { color: var(--success); flex-shrink: 0; }
.app-bar-name span { font-size: 11px; color: var(--text-dim); }
.app-bar-dl {
    ...
    color: #071018; font-weight: 700; font-size: 12px; text-decoration: none; white-space: nowrap;
    transition: transform .15s, filter .15s; box-shadow: 0 4px 12px rgba(89,245,213,.25);
}
.app-bar-dl:hover { filter: brightness(1.08); transform: translateY(-1px); }

/* Header & Nav */
.site-header { display: flex; flex-wrap: wrap; align-items: center; justify-content: space-between; gap: 10px 16px; max-width: 760px; margin: 0 auto 6px; padding: 6px 0 16px; }
.brand { display: flex; align-items: center; gap: 8px; text-decoration: none; color: var(--text); font-weight: 700; }
.brand-mark { color: var(--cyan); display: flex; align-items: center; }
.brand-name { font-family: var(--font-body); font-weight: 700; font-size: 15px; letter-spacing: -.01em; }
.brand-name b { color: var(--cyan); }
.site-nav { display: flex; gap: 14px; font-size: 12.5px; }
.site-nav a { color: var(--text-dim); text-decoration: none; font-weight: 500; transition: color .15s; }
.site-nav a:hover { color: var(--cyan); }
.lang-switch { display: flex; align-items: center; gap: 3px; font-family: var(--font-mono); font-size: 11px; color: var(--text-dim); }
.lang-switch a { color: var(--text-dim); text-decoration: none; padding: 3px 6px; border-radius: var(--r-sm); transition: all .15s; }
.lang-switch a.active { color: var(--cyan); background: rgba(89,245,213,.12); font-weight: 700; }
.theme-switch { display: flex; align-items: center; gap: 2px; }
.theme-switch a { display: flex; align-items: center; justify-content: center; width: 28px; height: 28px; border-radius: var(--r-sm); color: var(--text-dim); text-decoration: none; transition: all .15s; }
.theme-switch a:hover { color: var(--text); }
.theme-switch a.active { color: var(--cyan); background: rgba(89,245,213,.12); }
.breadcrumb { max-width: 760px; margin: 0 auto 20px; font-size: 11px; color: var(--text-dim); font-family: var(--font-mono); letter-spacing: .04em; }

/* Hero */
.eyebrow {
    display: inline-flex; align-items: center; gap: 8px; margin: 0 auto 14px; padding: 6px 12px;
    border: 1px solid var(--line); border-radius: var(--r-full); background: rgba(var(--surface-rgb), 0.7);
    box-shadow: 0 4px 14px rgba(0,0,0,0.12); font-family: var(--font-mono); font-size: 10.5px; letter-spacing: .14em;
...
h1 {
    font-size: clamp(30px, 5.5vw, 50px); line-height: 1.1; letter-spacing: -.04em; margin: 0 auto 10px;
    max-width: 620px; font-weight: 800; text-align: center;
    background: linear-gradient(135deg, var(--text) 40%, var(--cyan) 120%);
    -webkit-background-clip: text; background-clip: text; -webkit-text-fill-color: transparent;
}
.sub { font-size: 14px; line-height: 1.6; max-width: 500px; margin: 0 auto 16px; text-align: center; color: var(--text-dim); }
.proof {
    display: inline-flex; align-items: center; justify-content: center; gap: 7px; margin: 0 auto 26px; padding: 7px 14px;
    border: 1px solid rgba(97,230,164,.2); border-radius: var(--r-full); background: rgba(97,230,164,.07);
    font-size: 12px; color: var(--text-dim);
}
.proof b { color: var(--success); font-family: var(--font-mono); font-weight: 700; }
.pulse-dot { width: 6px; height: 6px; border-radius: 50%; background: var(--success); box-shadow: 0 0 8px var(--success); animation: ds-blink 1.8s ease-in-out infinite; }

/* Status Card */
.status-card {
    max-width: 620px; margin: 0 auto 20px; padding: 16px 18px; border: 1px solid var(--line);
    border-radius: var(--r-xl); background: rgba(var(--surface-rgb), 0.72);
    backdrop-filter: blur(8px); -webkit-backdrop-filter: blur(8px); box-shadow: var(--shadow-card);
}
.status-card-head { display: flex; align-items: center; justify-content: space-between; gap: 8px; margin-bottom: 12px; }
.status-card-head .label { display: flex; align-items: center; gap: 6px; color: var(--text-dim); font-size: 11px; font-family: var(--font-mono); letter-spacing: .05em; font-weight: 600; }
.status-pill { display: inline-flex; align-items: center; gap: 5px; padding: 3px 10px; border-radius: var(--r-full); font-size: 10.5px; font-weight: 700; font-family: var(--font-mono); }
.status-pill.on { background: rgba(97,230,164,.12); color: var(--success); }
.status-pill.off { background: rgba(239,68,68,.12); color: var(--ds-danger); }
...
.status-box { padding: 10px 12px; border: 1px solid var(--line); border-radius: var(--r-md); text-align: center; background: rgba(var(--surface-rgb), 0.4); }
.status-box span { display: block; font-size: 9.5px; letter-spacing: .06em; color: var(--text-dim); text-transform: uppercase; margin-bottom: 4px; font-family: var(--font-mono); }
.status-box b { font-size: 13px; font-weight: 700; color: var(--text); }

/* Sample Card */
.sample-card {
    position: relative; max-width: 620px; margin: 0 auto 28px; border: 1px solid var(--line);
    border-radius: var(--r-2xl); background: var(--grad-card); overflow: hidden;
    box-shadow: var(--shadow-float);
}
.sample-holo { height: 3px; background: var(--grad-holo); background-size: 240% 100%; animation: ds-holo-shift 3.5s linear infinite; }
.sample-top { padding: 22px 24px 18px; text-align: center; }
.sample-tag { font-family: var(--font-mono); font-size: 10px; letter-spacing: .16em; color: var(--text-dim); text-transform: uppercase; }
.sample-key {
    font-family: var(--font-mono); font-weight: 700; font-size: clamp(20px, 4.8vw, 28px); letter-spacing: .1em;
    color: var(--cyan); text-shadow: 0 0 20px rgba(89,245,213,.35); margin-top: 8px; font-variant-numeric: tabular-nums;
}
.sample-perf { height: 0; border-top: 1.5px dashed var(--line); margin: 0 20px; position: relative; }
...
.sample-perf::before { left: -27px; }
.sample-perf::after { right: -27px; }
.sample-bottom { padding: 12px 24px 16px; display: flex; justify-content: space-between; align-items: center; font-size: 11px; color: var(--text-dim); }
.sample-bottom span { display: flex; flex-direction: column; gap: 2px; }
.sample-bottom b { color: var(--text); font-weight: 600; }

/* Guide */
.guide-layout { max-width: 620px; margin: 0 auto; display: grid; grid-template-columns: minmax(130px, .32fr) minmax(0, 1fr); gap: 20px; align-items: start; }
...
.step-card {
    max-width: none; margin: 0 0 10px; padding: 14px 16px; border: 1px solid var(--line);
    border-radius: var(--r-lg); background: rgba(var(--surface-rgb), 0.65);
...
.step-num { font-family: var(--font-mono); font-size: 22px; font-weight: 800; color: var(--cyan); line-height: 1; flex-shrink: 0; min-width: 28px; }
.step-body b { display: block; font-size: 13.5px; font-weight: 700; margin-bottom: 2px; color: var(--text); }
...
/* Sections & Toolbar */
.section-label {
    display: flex; align-items: center; gap: 10px; margin: 0 auto 12px; max-width: 620px;
    font-family: var(--font-mono); font-size: 10.5px; letter-spacing: .14em; color: var(--text-dim); text-transform: uppercase; font-weight: 600;
}
.section-label::after { content: ""; height: 1px; flex: 1; background: var(--line); }
.game-toolbar { max-width: 620px; margin: 0 auto 12px; display: flex; align-items: center; gap: 8px; flex-wrap: wrap; }
.search-field {
    height: 42px; display: flex; align-items: center; gap: 9px; flex: 1; min-width: 200px;
    padding: 0 12px; border: 1px solid var(--line); border-radius: var(--r-md);
    background: rgba(var(--surface-rgb), 0.7); color: var(--text-dim); transition: all .15s;
}
.search-field:focus-within { border-color: var(--cyan); box-shadow: 0 0 0 3px rgba(89,245,213,.12); }
.search-field input {
    width: 100%; min-width: 0; border: 0; outline: 0; background: transparent; color: var(--text);
    font: 500 13px var(--font-body); appearance: none;
}
.search-field input::-webkit-search-cancel-button { display: none; }
.search-field input::placeholder { color: var(--ds-text-muted); }
.search-clear {
    width: 22px; height: 22px; padding: 0; border: 0; border-radius: 6px; background: transparent;
    color: var(--text-dim); font-size: 16px; line-height: 1; cursor: pointer; opacity: 0; pointer-events: none; transition: all .15s;
}
.search-clear.visible { opacity: 1; pointer-events: auto; }
.search-clear:hover { background: rgba(255,255,255,.08); color: var(--text); }
.filter-group { display: flex; gap: 4px; padding: 3px; border: 1px solid var(--line); border-radius: var(--r-md); background: rgba(var(--surface-rgb), 0.6); }
.filter-btn {
    height: 34px; padding: 0 12px; border: 0; border-radius: var(--r-sm); background: transparent;
    color: var(--text-dim); font: 600 11.5px var(--font-body); cursor: pointer; white-space: nowrap; transition: all .15s;
}
.filter-btn:hover { color: var(--text); }
.filter-btn.active { background: rgba(89,245,213,.15); color: var(--cyan); font-weight: 700; }
.game-results { max-width: 620px; min-height: 16px; margin: -2px auto 10px; padding-left: 2px; color: var(--text-dim); font: 10.5px var(--font-mono); letter-spacing: .02em; }

/* Game Cards */
.game-list { max-width: 620px; margin: 0 auto; display: grid; grid-template-columns: repeat(2, minmax(0, 1fr)); gap: 12px; }
.card {
...
.card:hover { transform: translateY(-3px); border-color: rgba(89,245,213,.35); box-shadow: var(--shadow-card-hover); }
.holo { height: 3px; background: var(--grad-holo); background-size: 240% 100%; animation: ds-holo-shift 3.5s linear infinite; }
.card-body { padding: 18px; display: flex; align-items: center; gap: 14px; flex: 1; }
.icon-badge {
    width: 48px; height: 48px; border-radius: var(--r-lg); background: linear-gradient(145deg, var(--surface3), var(--surface2));
    display: flex; align-items: center; justify-content: center; font-size: 24px; flex-shrink: 0;
    border: 1px solid rgba(89,245,213,.2); box-shadow: inset 0 1px rgba(255,255,255,.08);
    transition: transform .25s var(--ease-spring);
}
.card:hover .icon-badge { transform: scale(1.08) rotate(-4deg); }
.card-info { flex: 1; min-width: 0; }
.gname { font-family: var(--font-body); font-weight: 700; font-size: 16px; letter-spacing: -.02em; color: var(--text); white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
.gmeta { display: flex; align-items: center; gap: 6px; margin-top: 4px; flex-wrap: wrap; }
.free-pill {
    font-size: 10px; font-weight: 700; color: var(--success); background: rgba(97,230,164,.12);
    padding: 3px 8px; border-radius: var(--r-full); display: inline-flex; align-items: center; gap: 4px; font-family: var(--font-mono);
}
.free-pill .dot { width: 5px; height: 5px; border-radius: 50%; background: var(--success); animation: ds-blink 1.6s ease-in-out infinite; }
.cooldown-pill {
    display: inline-flex; align-items: center; gap: 5px; padding: 3px 9px; border-radius: var(--r-full);
    background: rgba(245,201,105,.12); border: 1px solid rgba(245,201,105,.28); color: var(--ds-warn);
    font-size: 10.5px; font-family: var(--font-mono); font-variant-numeric: tabular-nums;
}
.cooldown-pill .dot { width: 5px; height: 5px; border-radius: 50%; background: currentColor; animation: ds-blink 1.4s infinite; }
.hops-note { font-size: 11px; color: var(--ds-text-muted); margin-top: 4px; font-family: var(--font-mono); }
.perf { height: 0; border-top: 1px dashed var(--line); margin: 0 18px; }
.card-cta { padding: 14px 18px 16px; }
a.btn {
    ...
    box-shadow: var(--shadow-btn); transition: transform .12s, filter .15s, box-shadow .15s;
}
a.btn:hover { filter: brightness(1.08); transform: translateY(-1px); }
a.btn:active { transform: scale(.98); }
.btn-disabled { opacity: .55; cursor: not-allowed; filter: grayscale(.3); pointer-events: none; }
.card.is-hidden { display: none; }
.game-no-results { max-width: 620px; margin: 10px auto 0; padding: 24px; border: 1px dashed var(--line); border-radius: var(--r-lg); text-align: center; color: var(--text-dim); font-size: 13px; }
.empty { text-align: center; color: var(--text-dim); font-size: 13.5px; margin: 20px auto; padding: 30px; border: 1px dashed var(--line); border-radius: var(--r-lg); }
...
/* Features */
.feature-card {
    max-width: 620px; margin: 0 auto 10px; padding: 14px 16px; border: 1px solid var(--line);
    border-radius: var(--r-lg); background: rgba(var(--surface-rgb), 0.68); display: flex; gap: 12px;
    align-items: flex-start; backdrop-filter: blur(8px);
}
.feature-icon {
    width: 36px; height: 36px; border-radius: var(--r-md); display: flex; align-items: center; justify-content: center;
    flex-shrink: 0; background: rgba(89,245,213,.12); color: var(--cyan);
}
...
.feature-body-head b { font-size: 13.5px; font-weight: 700; color: var(--text); }
.feature-tag { font-family: var(--font-mono); font-size: 9px; font-weight: 700; letter-spacing: .05em; padding: 2px 7px; border-radius: var(--r-full); background: rgba(89,245,213,.12); color: var(--cyan); white-space: nowrap; }
.feature-body p { font-size: 12px; color: var(--text-dim); margin: 3px 0 0; line-height: 1.5; }

/* Reset Key */
.reset-box {
    max-width: 620px; margin: 28px auto 0; padding: 20px 22px; border: 1px solid var(--line);
    border-radius: var(--r-xl); background: rgba(var(--surface-rgb), 0.8); box-shadow: var(--shadow-card);
    backdrop-filter: blur(8px);
}
.reset-title { font-family: var(--font-body); font-weight: 700; font-size: 15.5px; display: flex; align-items: center; gap: 8px; color: var(--text); }
.reset-sub { font-size: 12px; color: var(--text-dim); margin: 6px 0 14px; line-height: 1.55; }
.reset-form { display: flex; gap: 8px; }
.reset-form input {
    flex: 1; min-width: 0; height: 44px; padding: 0 14px; border-radius: var(--r-md);
    border: 1px solid var(--line); background: rgba(9,13,20,.6); color: var(--text);
    font-family: var(--font-mono); font-size: 12.5px; letter-spacing: .03em;
}
.reset-form input:focus { outline: none; border-color: var(--cyan); box-shadow: 0 0 0 3px rgba(89,245,213,.12); }
.reset-form button {
    flex-shrink: 0; height: 44px; padding: 0 18px; border: 0; border-radius: var(--r-md);
...
.reset-form button:hover { filter: brightness(1.08); }
.reset-form button:active { transform: scale(.97); }
.reset-msg { margin-top: 12px; padding: 10px 14px; border-radius: var(--r-md); font-size: 12px; line-height: 1.5; }
.reset-msg.ok { border: 1px solid rgba(97,230,164,.3); background: rgba(97,230,164,.1); color: var(--success); }
.reset-msg.err { border: 1px solid rgba(255,120,133,.3); background: rgba(255,120,133,.1); color: var(--ds-danger); }
@media (max-width: 540px) { .reset-form { flex-direction: column; } .reset-form button { width: 100%; } }

/* IP Box */
.ip-box {
    max-width: 620px; margin: 18px auto 0; padding: 12px 18px; border: 1px solid var(--line);
    border-radius: var(--r-lg); background: rgba(var(--surface-rgb), 0.65); display: flex;
    align-items: center; justify-content: space-between; gap: 10px; flex-wrap: wrap; backdrop-filter: blur(8px);
}
.ip-box-left { display: flex; align-items: center; gap: 10px; }
.ip-ic { color: var(--cyan); display: flex; align-items: center; }
.ip-label { font-family: var(--font-mono); font-size: 9.5px; letter-spacing: .1em; color: var(--text-dim); }
.ip-value { font-family: var(--font-mono); font-size: 13.5px; font-weight: 700; color: var(--cyan); letter-spacing: .02em; }
.ip-box-note { font-size: 11px; color: var(--ds-text-muted); }
@media (max-width: 480px) { .ip-box { flex-direction: column; align-items: flex-start; } }

.footer-note { text-align: center; font-size: 11px; color: var(--ds-text-muted); margin-top: 32px; font-family: var(--font-mono); letter-spacing: .05em; }
...
.fab-menu { display: flex; flex-direction: column; gap: 10px; margin-bottom: 2px; opacity: 0; transform: translateY(10px) scale(.9); pointer-events: none; transition: all .22s var(--ease-out); }
.fab-menu.open { opacity: 1; transform: none; pointer-events: auto; }
.fab-item { width: 44px; height: 44px; border-radius: 50%; display: flex; align-items: center; justify-content: center; text-decoration: none; color: #fff; box-shadow: 0 8px 20px -6px rgba(0,0,0,.6); transition: transform .15s; }
.fab-item:hover { transform: scale(1.08); }
.fab-zalo { background: #0068FF; }
.fab-tele { background: #29A9EA; }
.fab-fb { background: #1877F2; }
.fab-toggle { width: 52px; height: 52px; border-radius: 50%; border: 0; cursor: pointer; display: flex; align-items: center; justify-content: center; color: #071018; background: var(--grad-btn); box-shadow: 0 10px 24px -8px rgba(89,245,213,.7); transition: transform .2s ease; }
...
@media (max-width: 420px) {
    .site-nav { order: 3; width: 100%; justify-content: center; padding-top: 6px; border-top: 1px dashed var(--line); }
...
<div class="wrap">
    ...
    <?php else: ?>
    <div class="game-toolbar" role="search">
    ...
    <div class="game-results" id="gameResults" aria-live="polite"></div>
