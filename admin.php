...
if (isset($_SESSION['is_admin'])) {
    ...
    if (isset($_POST['test_admin_telegram_msg'])) {
        ...
        if ($botToken === '' || $adminChatId === '') {
            ...
            $testMsg = "<blockquote>" .
                       ...
                       "</blockquote>\n" .
                       "<i>Chat ID Admin: <code>" . htmlspecialchars($adminChatId) . "</code> &mdash; Đã áp dụng thành công bộ Icon theo nội dung!</i>";
    ...
    if (isset($_POST['restore_icons_file'])) {
        if (!empty($_FILES['icons_backup_file']['tmp_name'])) {
            ...
            if (is_array($data) && (isset($data['premium_emoji_id']) || isset($data['emoji_header']) || (isset($data['type']) && $data['type'] === 'telegram_icons_backup'))) {
                restore_telegram_icons_config($data);
                $channelSaved = true;
                $tgBotFeedback = '✅ Đã khôi phục thành công cấu hình Icon Premium từ file JSON sao lưu tải lên!';
            } else {
                $channelError = '❌ File tải lên không hợp lệ hoặc không đúng định dạng sao lưu Icon Telegram Premium.';
            }
        } else {
                ...
                <div style="margin-top:10px;padding:14px 16px;background:rgba(156,140,255,.06);border:1px solid rgba(156,140,255,.25);border-radius:8px">
                    ...
                    <div style="margin-top:14px;padding:13px 15px;background:rgba(0,242,254,.04);border:1px dashed rgba(0,242,254,.32);border-radius:8px">
                        ...
                        <p style="font-size:11px;color:var(--text-dim);margin:0 0 10px;line-height:1.5">
                            Mỗi khi bạn bấm <b>"Lưu Cấu Hình"</b> hoặc <b>"Sao Lưu Icon Hiện Tại"</b>, hệ thống tự động lưu bản snapshot Icon Premium, đồng thời đồng bộ vào Firebase/Supabase đám mây để không bị mất khi server Render restart.
                        </p>
