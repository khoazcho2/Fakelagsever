...
// ------------------------------------------------------------
// Link liên hệ hiển thị ở nút liên hệ nổi (index.php) - THAY bằng link
// thật của bạn. Để trống ('') thì nút đó tự ẩn, không hiện link chết.
// Có thể override qua biến môi trường (khỏi sửa code khi đổi số/kênh).
// ------------------------------------------------------------
define('CONTACT_ZALO_URL', getenv('CONTACT_ZALO_URL') ?: '');
define('CONTACT_TELEGRAM_URL', getenv('CONTACT_TELEGRAM_URL') ?: 'https://t.me/qiaxlamDev');
define('CONTACT_FACEBOOK_URL', getenv('CONTACT_FACEBOOK_URL') ?: '');
...
function get_resellers(): array {
    return get_db()->query("SELECT id, username, store_name, enabled, expires_at, created_at FROM resellers ORDER BY id DESC")->fetchAll(PDO::FETCH_ASSOC);
}

function get_reseller_by_id(int $id): ?array {
    $stmt = get_db()->prepare("SELECT id, username, store_name, enabled, expires_at, created_at FROM resellers WHERE id = ?");
    $stmt->execute([$id]);
    $row = $stmt->fetch(PDO::FETCH_ASSOC);
    return $row ?: null;
}
