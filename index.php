...
<div class="server3d-overlay" id="server3dIntro" role="dialog" aria-modal="true" aria-label="HOQUOC KEY 3D Server">
    ...
    <!-- Top Floating Header (Badge, Title, Subtitle) -->
    <div class="server3d-top">
        <div class="server3d-badge">
            <span class="server3d-badge-dot"></span>
            <span>3D QUANTUM SERVER // HOQUOC KEYAUTH</span>
        </div>
        <h1 class="server3d-title">HOQUOC KEY</h1>
        <p class="server3d-sub"><?= t('HỆ THỐNG CẤP KEY 3D SERVER CHÍNH THỨC', 'OFFICIAL 3D SERVER KEY SYSTEM') ?></p>
    </div>

    <!-- Bottom Floating Dock (Terminal, Progress, CTA) -->
    <div class="server3d-bottom">
        <div class="server3d-terminal" id="server3dTerminal">
            <div class="line"><span class="cyan">❯</span> <span class="highlight">[INIT]</span> Khởi tạo HoQuoc 3D Quantum Node...</div>
...
<script>
// ============================================================
// HOQUOC KEY 3D SERVER ENTRANCE ENGINE (Three.js WebGL)
// Interactive 3D Cyber Server Node with Holographic Data Rings,
// LED blade arrays, Quantum Core, Tron grid, and Warp Transition
// ============================================================
(function(){
    ...
    function init3DServerIntro() {
        ...
        var defaultCamZ = isMobile ? 8.2 : 6.8;
        var camera = new THREE.PerspectiveCamera(defaultFOV, width / height, 0.1, 100);
        camera.position.set(0, 0.75, defaultCamZ);
        camera.lookAt(0, 0, 0);
        ...
        var ambient = new THREE.AmbientLight(0x18243b, 1.4);
        scene.add(ambient);

        var cyanKey = new THREE.DirectionalLight(0x00f2fe, 3.2);
        cyanKey.position.set(6, 8, 7);
        scene.add(cyanKey);

        var violetFill = new THREE.PointLight(0xa855f7, 4.5, 25);
        violetFill.position.set(-6, -2, 5);
        scene.add(violetFill);

        var emeraldAccent = new THREE.PointLight(0x10b981, 2.8, 20);
        emeraldAccent.position.set(4, -5, -4);
        scene.add(emeraldAccent);

        var coreGlowLight = new THREE.PointLight(0x00f2fe, 4.2, 10);
        coreGlowLight.position.set(0, 0, 0);
        scene.add(coreGlowLight);

        // 3D SERVER RIG GROUP (Isometric 3D Tilt)
        var serverNode = new THREE.Group();
        serverNode.rotation.set(0.24, 0.35, 0);
        scene.add(serverNode);

        // 1. Central Holographic Quantum Core & Matrix
        var coreGroup = new THREE.Group();
        serverNode.add(coreGroup);

        // Inner glowing plasma heart
        var plasmaGeo = new THREE.SphereGeometry(0.52, 28, 28);
        var plasmaMat = new THREE.MeshBasicMaterial({ color: 0x00f2fe });
        var plasma = new THREE.Mesh(plasmaGeo, plasmaMat);
        coreGroup.add(plasma);

        // Chrome Cyber Diamond (Faceted 3D Crystal)
        var diamondGeo = new THREE.IcosahedronGeometry(0.95, 0);
        ...
        // Geometric wireframe cyber lattice cage
        var cageGeo = new THREE.IcosahedronGeometry(1.32, 1);
        ...
        // Outer dodecahedron gyro wire
        var outerGyroGeo = new THREE.DodecahedronGeometry(1.68, 0);
        var outerGyroMat = new THREE.MeshBasicMaterial({
            color: 0xa855f7,
            wireframe: true,
            transparent: true,
            opacity: 0.55
        });
        var outerGyro = new THREE.Mesh(outerGyroGeo, outerGyroMat);
        coreGroup.add(outerGyro);

        // 2. Six Floating Orbital Server Blade Pods (Hexagonal Array)
        var bladePods = [];
        var podGeo = new THREE.BoxGeometry(0.36, 1.85, 0.52);
        ...
        var finGeo = new THREE.BoxGeometry(0.32, 0.04, 0.46);
        ...
        var spineGeo = new THREE.BoxGeometry(0.04, 1.88, 0.04);
        var spineMatCyan = new THREE.MeshBasicMaterial({ color: 0x00f2fe });
        var spineMatViolet = new THREE.MeshBasicMaterial({ color: 0xa855f7 });

        var ledGeo = new THREE.BoxGeometry(0.04, 0.04, 0.02);
        var ledColors = [0x00f2fe, 0x10b981, 0xf59e0b, 0xec4899];
        var leds = [];

        var podCount = 6;
        var podRadius = isMobile ? 1.85 : 2.2;

        for (var i = 0; i < podCount; i++) {
            var angle = (i / podCount) * Math.PI * 2;
            var podGroup = new THREE.Group();

            // Main blade pod chassis
            var podChassis = new THREE.Mesh(podGeo, podMat);
            podGroup.add(podChassis);

            // Glowing vertical neon spine on outer face
            var spine = new THREE.Mesh(spineGeo, (i % 2 === 0) ? spineMatCyan : spineMatViolet);
            spine.position.set(0, 0, 0.27);
            podGroup.add(spine);

            // Heatsink fins
            for (var f = -2; f <= 2; f++) {
                var fin = new THREE.Mesh(finGeo, finMat);
                fin.position.set(0, f * 0.32, 0);
                podGroup.add(fin);
            }

            // Blinking status LED matrix on front face
            for (var l = 0; l < 4; l++) {
                var lColor = ledColors[(i + l) % ledColors.length];
                var lMat = new THREE.MeshBasicMaterial({ color: lColor });
                var led = new THREE.Mesh(ledGeo, lMat);
                led.position.set(0.12, -0.45 + l * 0.3, 0.27);
                podGroup.add(led);
                leds.push({
                    mesh: led,
                    mat: lMat,
                    baseColor: lColor,
                    speed: 2.0 + (i * 0.7 + l * 0.9) % 3.0
                });
            }

            // Position in circle and face inward to the core
            podGroup.position.set(Math.cos(angle) * podRadius, 0, Math.sin(angle) * podRadius);
            podGroup.rotation.y = -angle + Math.PI / 2;

            serverNode.add(podGroup);
            bladePods.push({
                group: podGroup,
                phase: i * 1.05
            });
        ...
        // 3. Multi-Axis Holographic Gyro Data Rings
        var ringMat1 = new THREE.MeshBasicMaterial({ color: 0x00f2fe, transparent: true, opacity: 0.85 });
        var ringMat2 = new THREE.MeshBasicMaterial({ color: 0xa855f7, transparent: true, opacity: 0.8 });
        var ringMat3 = new THREE.MeshBasicMaterial({ color: 0x38bdf8, transparent: true, opacity: 0.7 });
        var ringMat4 = new THREE.MeshBasicMaterial({ color: 0xec4899, transparent: true, opacity: 0.65 });

        var ring1 = new THREE.Mesh(new THREE.TorusGeometry(1.9, 0.018, 16, 100), ringMat1);
        var ring2 = new THREE.Mesh(new THREE.TorusGeometry(2.55, 0.02, 16, 100), ringMat2);
        var ring3 = new THREE.Mesh(new THREE.TorusGeometry(3.1, 0.022, 16, 120), ringMat3);
        var ring4 = new THREE.Mesh(new THREE.TorusGeometry(3.65, 0.016, 16, 120), ringMat4);
        ...
        serverNode.add(ring1);
        serverNode.add(ring2);
        serverNode.add(ring3);
        serverNode.add(ring4);

        // Orbiting satellite beacons on rings
        var beaconGeo = new THREE.OctahedronGeometry(0.09, 0);
        var beaconMat1 = new THREE.MeshBasicMaterial({ color: 0xffffff });
        var beaconMat2 = new THREE.MeshBasicMaterial({ color: 0x00f2fe });
        var beacon1 = new THREE.Mesh(beaconGeo, beaconMat1);
        var beacon2 = new THREE.Mesh(beaconGeo, beaconMat2);
        ring1.add(beacon1);
        ring2.add(beacon2);
        beacon1.position.x = 1.9;
        beacon2.position.y = 2.55;

        // 4. Dynamic Holographic Laser Scanner Plane
        var scannerGeo = new THREE.RingGeometry(0.35, 2.75, 48);
        var scannerMat = new THREE.MeshBasicMaterial({
            color: 0x00f2fe,
        ...
        var scanner = new THREE.Mesh(scannerGeo, scannerMat);
        scanner.rotation.x = Math.PI / 2;
        serverNode.add(scanner);

        // 5. Dual Perspective Cyber Grids (Floor & Ceiling)
        var floorGrid = new THREE.GridHelper(36, 28, 0x00f2fe, 0x0d1c36);
        floorGrid.position.y = -2.7;
        scene.add(floorGrid);

        var ceilGrid = new THREE.GridHelper(36, 28, 0x8b5cf6, 0x0a1428);
        ceilGrid.position.y = 3.2;
        scene.add(ceilGrid);

        // 6. Swirling Cyber Dust Particle Starfield
        var pCount = isMobile ? 180 : 380;
        var pGeo = new THREE.BufferGeometry();
        var pPos = new Float32Array(pCount * 3);
        var pSpeeds = [];

        for (var p = 0; p < pCount; p++) {
            var rad = 1.8 + Math.random() * 5.0;
            var ang = Math.random() * Math.PI * 2;
            pPos[p * 3] = Math.cos(ang) * rad;
            pPos[p * 3 + 1] = (Math.random() - 0.5) * 6;
            pPos[p * 3 + 2] = Math.sin(ang) * rad;
            pSpeeds.push({ rad: rad, ang: ang, vAng: 0.005 + Math.random() * 0.015, vY: 0.004 + Math.random() * 0.008 });
        }
        ...
        window.addEventListener('pointermove', function(e){
            ...
            if (isDragging) {
                ...
                prevPointer.x = e.clientX;
                prevPointer.y = e.clientY;
                serverNode.rotation.y += dx * 0.008;
                serverNode.rotation.x += dy * 0.008;
            }
        });
        ...
        function animate() {
            ...
            // Auto-rotation with cursor parallax
            if (!isDragging) {
                if (!prefersReducedMotion) {
                    serverNode.rotation.y += 0.007;
                    var targetX = mouse.y * 0.25 + 0.24;
                    serverNode.rotation.x += (targetX - serverNode.rotation.x) * 0.05;
                }
            }

            // Quantum Core & Wireframe counter-rotation
            diamond.rotation.y = time * 1.5;
            diamond.rotation.x = Math.sin(time * 0.8) * 0.4;
            cage.rotation.y = -time * 2.0;
            cage.rotation.z = Math.cos(time * 0.9) * 0.45;
            outerGyro.rotation.x = time * 0.85;
            outerGyro.rotation.y = -time * 1.1;

            var pulse = Math.sin(time * 4) * 0.12 + 0.95;
            plasma.scale.set(pulse, pulse, pulse);
            diamondMat.emissiveIntensity = 0.35 + Math.sin(time * 3) * 0.25;

            // Floating Server Blade Pods oscillation
            for (var b = 0; b < bladePods.length; b++) {
                var pod = bladePods[b];
                pod.group.position.y = Math.sin(time * 2.4 + pod.phase) * 0.16;
            }

            // Sweeping Holographic Laser Scanner
            scanner.position.y = Math.sin(time * 1.9) * 1.35;
            scanner.material.opacity = 0.22 + Math.cos(time * 3.8) * 0.09;

            // Rotating Gimbal Hologram Data Rings
            ring1.rotation.x += 0.015;
            ring1.rotation.y += 0.022;
            ring2.rotation.y += 0.012;
            ring2.rotation.z += 0.018;
            ring3.rotation.z += 0.009;
            ring3.rotation.x += 0.014;
            ring4.rotation.x += 0.007;
            ring4.rotation.y -= 0.011;

            // Blinking Blade LEDs
            for (var i = 0; i < leds.length; i++) {
                var l = leds[i];
                var lPulse = (Math.sin(time * l.speed + i) + 1) * 0.5;
                l.mat.color.setHex(lPulse > 0.45 ? l.baseColor : 0x050c18);
            }
            ...
            if (!prefersReducedMotion) {
                ...
                for (var j = 0; j < pCount; j++) {
                    var sp = pSpeeds[j];
                    sp.ang += sp.vAng;
                    posArr[j * 3] = Math.cos(sp.ang) * sp.rad;
                    posArr[j * 3 + 2] = Math.sin(sp.ang) * sp.rad;
                    posArr[j * 3 + 1] += sp.vY;
                    if (posArr[j * 3 + 1] > 3.2) posArr[j * 3 + 1] = -2.4;
                }
