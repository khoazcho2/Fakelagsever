...
if (isset($_SESSION['is_admin'])) {
    ...
    if (isset($_POST['save_telegram_bot']) || isset($_POST['register_telegram_webhook'])) {
        ...
        save_telegram_bot_config($botToken, $botUsername);
        ...
        if (isset($_POST['register_telegram_webhook'])) {
            if ($botToken === '') {
                ...
                $res = telegram_api_call('setWebhook', ['url' => $webhookUrl], $botToken);
                if (!empty($res['ok'])) {
                    $channelSaved = true;
                    $tgBotFeedback = '✅ Đã kích hoạt Webhook thành công tới: ' . $webhookUrl;
                } else {
            ...
            $channelSaved = true;
            $tgBotFeedback = 'Đã lưu cấu hình Bot Telegram.';
        }
    }
...
<?php else: ?>
    ...
    <div class="box">
        ...
        <div class="region-form" style="margin-top:17px;border-color:rgba(89,245,213,.22)">
            <h4 style="margin:0 0 5px;color:var(--text)">🤖 Bot Telegram (xác minh thật thành viên nhóm)</h4>
            <p style="font-size:11.5px;color:var(--text-dim);margin:0 0 10px;max-width:680px">
                Tạo bot qua <b>@BotFather</b>, thêm bot làm <b>quản trị viên (admin)</b> của nhóm cần xác minh, rồi nhập Token + Username vào bên dưới. Sau đó bấm <b>🔗 Kích hoạt Webhook</b> để kết nối tự động 1-click.
            </p>
            <form method="post" style="display:flex;flex-wrap:wrap;gap:8px;align-items:center">
                <input type="hidden" name="csrf_token" value="<?= htmlspecialchars($_SESSION['csrf_token'] ?? '') ?>">
                <input type="text" name="tg_bot_token" value="<?= htmlspecialchars($tgBotCfg['token']) ?>" placeholder="Bot Token (từ @BotFather)" style="width:min(100%,330px)">
                <input type="text" name="tg_bot_username" value="<?= htmlspecialchars($tgBotCfg['username']) ?>" placeholder="Bot Username (không cần @)" style="width:200px">
                <button type="submit" name="save_telegram_bot" value="1">Lưu Bot</button>
            ...
            <?php if ($tgBotCfg['token'] !== ''): ?>
                <div style="margin-top:10px;font-size:11.5px;color:var(--text-dim);background:rgba(255,255,255,.03);padding:8px 12px;border-radius:6px;border:1px solid var(--border);line-height:1.6">
                    <div>✓ Bot: <b>@<?= htmlspecialchars($tgBotCfg['username'] ?: 'Chưa xác định') ?></b> &nbsp;|&nbsp; Webhook URL: <code><?= htmlspecialchars($webhookTargetUrl) ?></code></div>
                    <div style="margin-top:3px">Link đăng ký thủ công (nếu muốn mở trực tiếp trên trình duyệt): <a href="https://api.telegram.org/bot<?= htmlspecialchars($tgBotCfg['token']) ?>/setWebhook?url=<?= urlencode($webhookTargetUrl) ?>" target="_blank" rel="noopener" style="color:var(--cyan)">Mở link setWebhook</a></div>
                </div>
            <?php endif; ?>
    ...
    <div class="box">
        ...
        <div class="tablewrap">
        <table>
            <tr><th>Key</th><th>Trạng thái</th><th>Thời hạn</th><th>Thiết bị tạo</th><th>Số thiết bị dùng</th><th>Bắt đầu dùng</th><th>Hết hạn</th><th></th></tr>
