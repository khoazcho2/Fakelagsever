...
<div class="wrap">
    ...
    <?php if ($resellerInfo): ?>
    <div class="eyebrow"><?= htmlspecialchars(mb_strtoupper($siteBrandName)) ?> <span>KEY VAULT</span></div>
    <h1><?= t('Lấy', 'Get') ?> <span style="color:var(--cyan)">Key</span> <?= t('Game', 'Keys') ?></h1>
    <p class="sub"><?= t('Vượt link, nhận key ngay — không cần đăng ký', 'Complete the link steps, get your key instantly — no sign-up needed') ?> · <?= t('Đại lý chính thức', 'Official reseller') ?>: <b><?= htmlspecialchars($siteBrandName) ?></b></p>
    <?php else: ?>
