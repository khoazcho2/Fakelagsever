...
$resellerId = null;
if (isset($_REQUEST['r']) && ctype_digit((string)$_REQUEST['r'])) {
    $rr = get_reseller_by_id((int)$_REQUEST['r']);
    if ($rr && $rr['enabled']) $resellerId = (int)$_REQUEST['r'];
}
...
function render_channel_gate(array $game, array $channels, string $token, string $gateToken, ?array $notice, string $error = '', array $openedChannels = [], bool $isAdmin = false, string $rParam = ''): void {
    ...
    function startVerifyPoll(link){
        ...
        const poll=setInterval(()=>{
            fetch('getkey.php?channel_verify_poll=1&game='+encodeURIComponent(gameSlug)+'&token='+encodeURIComponent(openToken)+'&gate_token='+encodeURIComponent(gateToken)+'&channel='+encodeURIComponent(cid)+rParam,{credentials:'same-origin',cache:'no-store'})
    ...
    function onBotDirectStartClick(elem) {
        ...
        const globalPoll = setInterval(() => {
            fetch('getkey.php?channel_verify_poll=1&game=' + encodeURIComponent(gameSlug) + '&token=' + encodeURIComponent(openToken) + '&gate_token=' + encodeURIComponent(gateToken) + '&channel=0' + rParam, { credentials: 'same-origin', cache: 'no-store' })
                .then(r => r.json()).then(data => {
                    if (data.verified) {
    ...
    function runDirectTgVerify() {
        ...
        fetch('getkey.php?direct_tg_verify=1&game=' + encodeURIComponent(gameSlug) + '&token=' + encodeURIComponent(openToken) + '&gate_token=' + encodeURIComponent(gateToken) + '&identifier=' + encodeURIComponent(val) + rParam, {
