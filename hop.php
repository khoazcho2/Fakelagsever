...
if (isset($_GET['advance'])) {
    if ((int)($_GET['h'] ?? 0) !== (int)$row['current_hop'] + 1) {
        http_response_code(409);
        render_notice_screen('Link của bước cũ', 'Vui lòng quay lại trang lấy key và tiếp tục bước hiện tại. Mỗi bước chỉ được tính một lần.');
        exit;
    }
    ...
    $advance = $db->prepare("UPDATE keys SET current_hop = ?, hop_started_at = NULL WHERE id = ? AND status = 'pending' AND current_hop = ? AND hop_started_at IS NOT NULL");
    $advance->execute([$newHop, $row['id'], $row['current_hop']]);
...
// 1 key. Nếu không, key 24h (2 bước) sẽ rút gọn 2 lần trên CÙNG 1 link
