...
function apply_ip_hard_block(string $ip, int $blockedUntil, string $reason): void {
    ...
    if ($current === false) {
        db_execute(
            $db->prepare("INSERT INTO ip_blocklist (ip, violation_count, last_reason, last_violation_at, blocked_until) VALUES (?, 1, ?, ?, ?)"),
            [$ip, $reason, time(), $blockedUntil]
        );
    } else {
        $finalUntil = max((int)$current, $blockedUntil);
        db_execute(
            $db->prepare("UPDATE ip_blocklist SET violation_count = violation_count + 1, last_reason = ?, last_violation_at = ?, blocked_until = ? WHERE ip = ?"),
            [$reason, time(), $finalUntil, $ip]
        );
    }
...
function apply_fp_hard_block(string $fp, int $blockedUntil, string $reason): void {
    ...
    if ($current === false) {
        db_execute(
            $db->prepare("INSERT INTO fp_blocklist (fp, violation_count, last_reason, last_violation_at, blocked_until) VALUES (?, 1, ?, ?, ?)"),
            [$fp, $reason, time(), $blockedUntil]
        );
    } else {
        $finalUntil = max((int)$current, $blockedUntil);
        db_execute(
            $db->prepare("UPDATE fp_blocklist SET violation_count = violation_count + 1, last_reason = ?, last_violation_at = ?, blocked_until = ? WHERE fp = ?"),
            [$reason, time(), $finalUntil, $fp]
        );
    }
