...
.navmenu{display:grid;grid-template-rows:0fr;opacity:0;background:var(--surface);border-radius:10px;margin-bottom:0;overflow:hidden;transition:grid-template-rows .25s ease,opacity .2s ease,margin-bottom .25s ease}
...
.navmenu a{color:var(--text);text-decoration:none;padding:10px 12px;border-radius:8px;font-size:14px;font-family:'Space Grotesk',sans-serif;font-weight:500;transition:background .15s,transform .1s}
.navmenu a:active{transform:scale(.98)}
.navmenu a.active{background:linear-gradient(135deg,var(--cyan),var(--violet));color:#0B0E14;font-weight:700}
.navmenu a:not(.active){background:var(--bg)}
.navmenu a:not(.active):hover{background:var(--surface2)}
...
<?php else: ?>
    ...
    <div class="box">
        <h3>🛡️ IP bị chặn (nghi ngờ bypass)</h3>
        ...
        <table>
            ...
            <tr>
                <td>
                    ...
                    <button class="small" style="margin-left:4px" onclick="copyKeycode('<?= $k['id'] ?>', this)" title="Sao chép key">📋</button>
                </td>
