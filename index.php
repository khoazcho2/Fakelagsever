...
// hàm attempt_reset_key() trong config.php. Keycode đã là duy nhất toàn hệ thống.
...
:root {
    ...
    /* Motion Design DNA Tokens */
    --ease-out-expo: cubic-bezier(0.16, 1, 0.3, 1);
    --ease-spring: cubic-bezier(0.34, 1.56, 0.64, 1);
    --ease-spring-soft: cubic-bezier(0.25, 1.3, 0.5, 1);
    --dur-micro: 0.12s;
    --dur-fast: 0.22s;
    --dur-normal: 0.36s;
    --dur-slow: 0.65s;
...
.guide-steps { min-width: 0; position: relative; }
.guide-steps::before {
    content: ""; position: absolute; left: 32px; top: 20px; bottom: 20px; width: 2px;
    background: linear-gradient(180deg, var(--cyan) 0%, var(--violet) 70%, transparent 100%);
...
.step-card {
    ...
    transition: transform var(--dur-normal) var(--ease-out-expo), border-color var(--dur-normal), box-shadow var(--dur-normal);
    position: relative; z-index: 1;
}
.step-card:hover { transform: translateY(-3px) translateX(4px); border-color: rgba(0, 242, 254, 0.4); box-shadow: 0 12px 28px -8px rgba(0,0,0,0.6), 0 0 20px rgba(0, 242, 254, 0.18); }
.step-card.active-step { border-color: var(--cyan); box-shadow: 0 0 20px rgba(0, 242, 254, 0.25); }
.step-num { font-family: var(--font-mono); font-size: 24px; font-weight: 800; color: var(--cyan); line-height: 1; flex-shrink: 0; min-width: 30px; text-shadow: 0 0 12px rgba(89,245,213,0.3); transition: transform var(--dur-fast) var(--ease-spring); }
...
/* Game Cards */
.game-list { max-width: 640px; margin: 0 auto; display: grid; grid-template-columns: repeat(2, minmax(0, 1fr)); gap: 14px; perspective: 1200px; }
.card {
    ...
    transition: transform var(--dur-normal) var(--ease-out-expo), border-color var(--dur-normal), box-shadow var(--dur-normal), opacity 0.3s var(--ease-out-expo), scale 0.3s var(--ease-out-expo);
    overflow: hidden; display: flex; flex-direction: column; position: relative;
    backdrop-filter: blur(14px); -webkit-backdrop-filter: blur(14px);
    transform-style: preserve-3d; will-change: transform, box-shadow;
}
/* Specular Spotlight pseudo-element tracking mouse coordinates */
.card::before {
    content: ""; position: absolute; inset: 0; border-radius: inherit; pointer-events: none;
    background: radial-gradient(360px circle at var(--mouse-x, 50%) var(--mouse-y, 50%), rgba(0, 242, 254, 0.16), transparent 75%);
...
.card:hover {
    ...
    box-shadow: 0 22px 50px -15px rgba(0,0,0,0.9), 0 0 32px rgba(0, 242, 254, 0.25), 0 0 0 1px rgba(0, 242, 254, 0.35);
}
.card.filtering-out {
    opacity: 0 !important;
...
.holo { height: 3px; background: var(--grad-holo); background-size: 240% 100%; animation: ds-holo-shift 3.5s linear infinite; position: relative; z-index: 4; }
.card-body { padding: 20px; display: flex; align-items: center; gap: 16px; flex: 1; position: relative; z-index: 3; transform: translateZ(10px); }
.icon-badge {
    ...
    transform: translateZ(24px); transition: transform 0.38s var(--ease-spring), box-shadow 0.38s var(--ease-spring);
}
.card:hover .icon-badge { transform: translateZ(36px) scale(1.12) rotate(-5deg); box-shadow: 0 8px 24px rgba(0, 242, 254, 0.4); }
.card-info { flex: 1; min-width: 0; transform: translateZ(14px); }
...
.free-pill {
    ...
    border: 1px solid rgba(16, 185, 129, 0.25); transition: transform var(--dur-fast) var(--ease-spring);
}
.free-pill:hover { transform: scale(1.05); }
...
.card-cta { padding: 15px 20px 18px; position: relative; z-index: 3; transform: translateZ(16px); }
a.btn {
    position: relative; overflow: hidden;
    ...
    box-shadow: var(--shadow-btn); transition: all var(--dur-normal) var(--ease-out-expo);
}
a.btn::after {
...
a.btn:hover { filter: brightness(1.08); transform: translateY(-2px); box-shadow: var(--shadow-btn-hover); }
a.btn:hover::after { opacity: 1; transform: rotate(25deg) translateX(450%); }
a.btn:active { transform: scale(0.97) translateY(0); }
.btn-disabled { opacity: 0.55; cursor: not-allowed; filter: grayscale(0.3); pointer-events: none; }

/* Cyber Ripple Wave Effect */
.ripple-wave {
    position: absolute; border-radius: 50%; pointer-events: none;
    background: radial-gradient(circle, rgba(255,255,255,0.85) 0%, rgba(0,242,254,0.45) 50%, transparent 75%);
    transform: scale(0); animation: ds-ripple 0.6s var(--ease-out-expo) forwards;
    z-index: 10;
}
@keyframes ds-ripple {
...
.reset-form input:focus { outline: none; border-color: var(--cyan); box-shadow: 0 0 0 3px rgba(0, 242, 254, 0.18), 0 0 18px rgba(0, 242, 254, 0.15); }
.reset-form button {
    ...
    cursor: pointer; transition: all var(--dur-fast); box-shadow: var(--shadow-btn); position: relative; overflow: hidden;
}
...
.reset-msg.ok { border: 1px solid rgba(16, 185, 129, 0.35); background: rgba(16, 185, 129, 0.12); color: var(--success); animation: ds-fade-up 0.4s var(--ease-out-expo) both; }
.reset-msg.err { border: 1px solid rgba(244, 63, 94, 0.35); background: rgba(244, 63, 94, 0.12); color: var(--danger); animation: ds-shake 0.45s cubic-bezier(0.36, 0.07, 0.19, 0.97) both; }
@media (max-width: 540px) { .reset-form { flex-direction: column; } .reset-form button { width: 100%; } }

/* Form Haptic & Pulse States */
.reset-shake { animation: ds-shake 0.45s cubic-bezier(0.36, 0.07, 0.19, 0.97) both; }
.reset-celebrate { animation: ds-pulse-glow 1.2s var(--ease-out-expo) both; }
...
.ip-ic { color: var(--cyan); display: flex; align-items: center; filter: drop-shadow(0 0 8px rgba(0, 242, 254, 0.5)); transition: transform var(--dur-normal) var(--ease-spring); }
.ip-box:hover .ip-ic { transform: scale(1.15) rotate(15deg); }
...
.fab-menu {
    display: flex; flex-direction: column; gap: 10px; margin-bottom: 2px;
    opacity: 0; transform: translateY(16px) scale(0.85); pointer-events: none;
    transition: opacity 0.28s var(--ease-out-expo), transform 0.35s var(--ease-spring);
}
.fab-menu.open { opacity: 1; transform: translateY(0) scale(1); pointer-events: auto; }
.fab-menu.open .fab-item { animation: ds-pop-in 0.4s var(--ease-spring) both; }
.fab-menu.open .fab-item:nth-child(1) { animation-delay: 0.04s; }
.fab-menu.open .fab-item:nth-child(2) { animation-delay: 0.08s; }
.fab-menu.open .fab-item:nth-child(3) { animation-delay: 0.12s; }
.fab-item { width: 46px; height: 46px; border-radius: 50%; display: flex; align-items: center; justify-content: center; text-decoration: none; color: #fff; box-shadow: 0 8px 24px -6px rgba(0,0,0,0.65); transition: transform 0.25s var(--ease-spring); }
.fab-item:hover { transform: scale(1.16) rotate(6deg); }
.fab-zalo { background: #0068FF; }
.fab-tele { background: #29A9EA; }
.fab-fb { background: #1877F2; }
.fab-toggle { width: 54px; height: 54px; border-radius: 50%; border: 0; cursor: pointer; display: flex; align-items: center; justify-content: center; color: #060810; background: var(--grad-btn); box-shadow: 0 10px 28px -6px rgba(0, 242, 254, 0.65), 0 0 20px rgba(139, 92, 246, 0.45); transition: transform 0.3s var(--ease-spring); }
.fab-toggle:hover { transform: scale(1.08); }
.fab-toggle.open { transform: rotate(45deg) scale(1.04); }
...
<div class="wrap">
    ...
    <header class="site-header" data-motion="hero">
        ...
        <div class="theme-switch">
            <a class="<?= $GLOBALS['THEME'] === 'dark' ? 'active' : '' ?>" href="?theme=dark" title="<?= t('Nền tối', 'Dark background') ?>"><?= svg_icon('moon', 16) ?></a>
            <a class="<?= $GLOBALS['THEME'] === 'light' ? 'active' : '' ?>" href="?theme=light" title="<?= t('Nền sáng', 'Light background') ?>"><?= svg_icon('sun', 16) ?></a>
        </div>
    </header>
    ...
    <div class="proof" data-motion="hero"><span class="pulse-dot"></span> <?= t('Đã phát', 'Delivered') ?> <b class="count-up" data-target="<?= (int)$totalActivated ?>"><?= number_format($totalActivated) ?></b> <?= t('key thành công', 'keys successfully') ?></div>
    <?php endif; ?>
    ...
    <div class="status-card" data-motion="reveal">
        ...
        <div class="status-grid">
            ...
            <div class="status-box"><span><?= t('Số Game', 'Games') ?></span><b class="count-up" data-target="<?= count($games) ?>"><?= count($games) ?></b></div>
            <div class="status-box"><span><?= t('Định dạng Key', 'Key format') ?></span><b>HQD-XXX</b></div>
            <div class="status-box"><span><?= t('Đã phát', 'Delivered') ?></span><b class="count-up" data-target="<?= (int)$totalActivated ?>"><?= number_format($totalActivated) ?></b></div>
        </div>
    </div>
    ...
    <div class="card" data-motion="reveal" data-game-card data-game-search="<?= htmlspecialchars($gameSearchText, ENT_QUOTES, 'UTF-8') ?>" data-ready="<?= $cooldown > 0 ? '0' : '1' ?>" style="animation-delay:<?= $i * 0.07 ?>s">
        ...
        <div class="card-cta">
            ...
            <?php else: ?>
                <a class="btn" href="getkey.php?game=<?= urlencode($g['slug']) ?>"><?= svg_icon('zap', 15) ?> <?= t('Lấy key miễn phí', 'Get free key') ?></a>
            <?php endif; ?>
        </div>
    </div>
    <?php endforeach; ?>
    ...
    <div class="reset-box <?= $resetResult ? ($resetResult['ok'] ? 'reset-celebrate' : 'reset-shake') : '' ?>" id="reset-section" data-motion="reveal" style="margin-top:26px">
...
<script>
// Game explorer: lọc mượt mà có animation (Motion Design Skill: state change & stagger)
(function(){
    ...
    function update(animated){
        ...
        if (!animated || window.matchMedia('(prefers-reduced-motion: reduce)').matches) {
            toHide.forEach(function(c){ c.classList.remove('filtering-out'); c.classList.add('is-hidden'); });
            toShow.forEach(function(c){ c.classList.remove('filtering-out', 'is-hidden'); });
            return;
        }

        // Animated FLIP-like transition
        ...
        filterTimeout = setTimeout(function(){
            toHide.forEach(function(c){
                ...
                c.classList.remove('filtering-out');
            });
    ...
    clear.addEventListener('click', function(){ input.value = ''; input.focus(); update(true); });
    ...
    function init3DServerIntro() {
        ...
        function enterSystem() {
            ...
            function warpStep(now) {
                ...
                // Warp acceleration of rig and particles
...
<script>
// ============================================================
// HOQUOC KEY MOTION DESIGN SYSTEM (Motion Design Skill & Design DNA)
// 3 Motion Layers: Primary (Narrative), Secondary (Micro-interactions),
// Ambient (Atmosphere). Respects prefers-reduced-motion.
// ============================================================
(function(){
    var isReducedMotion = window.matchMedia('(prefers-reduced-motion: reduce)').matches;
    var hasRunEntrance = false;

    // Triggered after 3D Server exit or on initial load if 3D overlay is hidden
    window.triggerPageEntrance = function(){
        if (hasRunEntrance) return;
        hasRunEntrance = true;

        if (isReducedMotion || !window.gsap) {
            document.documentElement.classList.remove('motion-init');
            initStaticCounters();
            return;
        }
        ...
        if (hero.length) {
            gsap.fromTo(hero, 
                ...
                { y: 0, opacity: 1, scale: 1, duration: 0.88, stagger: 0.08, ease: 'power3.out', clearProps: 'transform' }
            );
        }

        // Animate count-up for numbers inside hero immediately
        var heroCounters = document.querySelectorAll('.proof .count-up');
        heroCounters.forEach(function(el){ animateCounter(el); });
    };
    ...
    function animateCounter(el){
        ...
        if (isReducedMotion || !window.gsap) {
            el.textContent = Math.round(target).toLocaleString('vi-VN');
            return;
        }
        ...
        gsap.to(state, {
            ...
            ease: 'power2.out',
            onUpdate: function(){
                el.textContent = Math.round(state.val).toLocaleString('vi-VN');
            }
        });
    }
    ...
    function initMotion(){
        ...
        var overlay = document.getElementById('server3dIntro');
        // If 3D overlay is dismissed or absent on load, trigger hero entrance right away
        if (!overlay || overlay.classList.contains('dismissed') || getComputedStyle(overlay).display === 'none') {
            window.triggerPageEntrance();
        }

        // 1. Scroll-triggered section reveals with stagger
        gsap.utils.toArray('[data-motion="reveal"]').forEach(function(element, index){
            gsap.fromTo(element, 
                { y: 32, opacity: 0, scale: 0.985 }, 
                {
                    y: 0, opacity: 1, scale: 1, duration: 0.72, delay: (index % 3) * 0.04,
                    ...
                    scrollTrigger: {
                        ...
                        onEnter: function(){
                            var counters = element.querySelectorAll('.count-up');
        ...
        // 2. Guide Layout Pin & Step Active Progression
        ...
        document.querySelectorAll('.guide-steps .step-card').forEach(function(card){
            ScrollTrigger.create({
                trigger: card,
                start: 'top 75%',
                end: 'bottom 25%',
                onEnter: function(){ card.classList.add('active-step'); },
                onLeaveBack: function(){ card.classList.remove('active-step'); }
            });
        });

        // 3. Specular 3D Perspective Tilt on Game Cards
        document.querySelectorAll('.game-list .card').forEach(function(card){
            var target = card.querySelector('.icon-badge');
            var moveX = gsap.quickTo(target, 'x', {duration: 0.28, ease: 'power2.out'});
            var moveY = gsap.quickTo(target, 'y', {duration: 0.28, ease: 'power2.out'});
            var tiltX = gsap.quickTo(card, 'rotationX', {duration: 0.32, ease: 'power2.out'});
            var tiltY = gsap.quickTo(card, 'rotationY', {duration: 0.32, ease: 'power2.out'});
            var liftZ = gsap.quickTo(card, 'y', {duration: 0.32, ease: 'power2.out'});
            ...
            card.addEventListener('pointermove', function(event){
                ...
                // Update specular spotlight position in real-time
                card.style.setProperty('--mouse-x', mouseX + 'px');
                card.style.setProperty('--mouse-y', mouseY + 'px');
                card.style.setProperty('--spotlight-opacity', '1');

                var px = (mouseX - rect.width / 2) / (rect.width / 2);
                var py = (mouseY - rect.height / 2) / (rect.height / 2);
        ...
        // 4. Cyber Ripple Wave on CTA Buttons
        document.addEventListener('pointerdown', function(e){
            var btn = e.target.closest('a.btn, .app-bar-dl, .server3d-enter-btn, .reset-form button, .filter-btn');
            if (!btn) return;
        ...
        // 5. Magnetic Micro-pull on Header Buttons
        var magnetics = document.querySelectorAll('.app-bar-dl, .app-bar-icon');
        magnetics.forEach(function(el){
            var mX = gsap.quickTo(el, 'x', {duration: 0.25, ease: 'power2.out'});
            var mY = gsap.quickTo(el, 'y', {duration: 0.25, ease: 'power2.out'});
