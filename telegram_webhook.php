...
if (isset($update['callback_query'])) {
    ...
    if (str_starts_with($data, 'chk_')) {
        ...
        if ($chatId && $messageId) {
            ...
            if (is_array($editRes) && !empty($editRes['used_fallback'])) {
                notify_admin_icon_fallback_once((string)($editRes['original_error'] ?? ''));
            }
        }
    } else {
...
if (str_starts_with($text, '/start')) {
    ...
    if (is_array($sendRes) && !empty($sendRes['used_fallback'])) {
        notify_admin_icon_fallback_once((string)($sendRes['original_error'] ?? ''));
    }
    http_response_code(200);
    exit('OK');
}
