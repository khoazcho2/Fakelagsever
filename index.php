...
// Xử lý form "Reset Key" (chỉ Key 40h, mỗi key reset được 1 lần)
$resetResult = null;
...
/* -------------------------------------------------------------
   CELL 5: RESET KEY VAULT (Span 1x1)
   40h key reset form + rate limiting feedback
   ------------------------------------------------------------- */
.cell-reset {
    grid-column: span 1;
    grid-row: span 1;
...
/* -------------------------------------------------------------
   ...
   Telemetry (3) -> Reset Vault (4) -> Protocol (5) -> Telegram (6)
   ------------------------------------------------------------- */
@media (max-width: 960px) {
    ...
    .cell-reset { grid-column: span 2; grid-row: auto; }
...
@media (max-width: 640px) {
    ...
    .cell-reset      { order: 4; width: 100%; box-sizing: border-box; }
...
html[data-theme="light"] .reset-input-box { background: #FFFFFF; }
html[data-theme="light"] .btn-apk,
html[data-theme="light"] .game-btn,
html[data-theme="light"] .reset-submit-btn,
...
<div class="wrap">
    ...
    <div class="bento-grid">
                    ...
                    <div class="game-card-header">
                        ...
                        <div class="game-meta">
                            ...
                            <div class="game-badges">
                                ...
                                <?php endif; ?>
                                <span class="game-hops-text"><?= (int)$hops ?> <?= t('bước', 'step(s)') ?></span>
                            </div>
                        </div>
                    </div>
        ...
        <!-- CELL 5: RESET KEY VAULT (Span 1x1) -->
        <section class="bento-cell cell-reset" id="reset-section" aria-label="Reset Key Vault">
            <div class="cell-head">
                <div class="cell-title-wrap">
                    <span class="cell-badge">PORTAL 05 // VAULT</span>
                    <h2 class="cell-title"><?= svg_icon('reset', 16) ?> <?= t('Reset Key 40 Giờ', '40-Hour Key Reset') ?></h2>
                    <p class="cell-sub"><?= t('Làm mới hạn key & gỡ thiết bị cũ để đổi máy', 'Refresh expiration and clear old bound device') ?></p>
                </div>
            </div>

            <form class="reset-form-wrap" method="post" action="index.php#reset-section">
                <input class="reset-input-box" type="text" name="reset_keycode" placeholder="<?= t('Nhập mã key 40h (VD: HQD-...)', 'Enter 40h keycode (e.g. HQD-...)') ?>" autocomplete="off" required>
                <input type="hidden" name="reset_key_submit" value="1">
                <button class="reset-submit-btn" type="submit"><?= svg_icon('reset', 14) ?> <?= t('Xác Nhận Reset Key', 'Submit Reset') ?></button>
            </form>

            <?php if ($resetResult): ?>
            <div class="reset-msg-box <?= $resetResult['ok'] ? 'ok' : 'err' ?>">
                <?= htmlspecialchars($resetResult['message']) ?>
            </div>
            <?php else: ?>
            <p style="font-size:11px;color:var(--text-muted);margin:10px 0 0;line-height:1.45;">
                * <?= t('Chỉ áp dụng cho Key 40h. Mỗi key chỉ được reset tối đa 1 lần duy nhất.', 'Only applies to 40-hour keys. Strictly limited to 1 reset per key.') ?>
            </p>
            <?php endif; ?>
        </section>
