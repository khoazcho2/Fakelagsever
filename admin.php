...
// Khoá theo IP thật (bảng login_lockout), KHÔNG theo session - trước đây
// fail_count/lock_until nằm trong $_SESSION nên chỉ cần xoá cookie hoặc
// mở tab ẩn danh là có lại đủ 5 lượt thử, vô hiệu hoá cơ chế khoá trước
// brute-force thật. Xem record_login_failure()/get_login_lock_remaining()
// trong config.php.
$clientIp = get_client_ip();
$lockRemaining = get_login_lock_remaining('admin', $clientIp);
...
if (!$isLocked && isset($_POST['admin_username'], $_POST['admin_password'])) {
    ...
    if ($okUser && $okPass) {
        ...
        clear_login_failures('admin', $clientIp);
        // Sinh CSRF token mới cho phiên đăng nhập này
        $_SESSION['csrf_token'] = bin2hex(random_bytes(32));
    } else {
        $lockedSeconds = record_login_failure('admin', $clientIp);
        $error = $lockedSeconds > 0
            ? 'Sai quá nhiều lần, thử lại sau ' . ceil($lockedSeconds / 60) . ' phút'
...
if (isset($_SESSION['is_admin'])) {
    ...
    // Tải về file backup JSON cấu hình Icon Telegram Premium
    if (isset($_GET['download_telegram_icons_backup'])) {
        $backup = get_latest_telegram_icons_backup();
        if (!$backup) {
            $backup = backup_telegram_icons_config();
        }
        $filename = 'telegram_icons_backup_' . date('Ymd_His') . '.json';
    ...
    // Tải về file cấu hình Bot Telegram hoàn chỉnh (telegram_bot.json) để lưu vào Git repo máy tính
    if (isset($_GET['download_telegram_bot_config'])) {
        $cfg = get_telegram_bot_config();
        header('Content-Type: application/json; charset=utf-8');
        header('Content-Disposition: attachment; filename="telegram_bot.json"');
        header('Cache-Control: no-cache, must-revalidate');
        header('Pragma: no-cache');
    ...
    if (isset($_POST['save_telegram_bot']) || isset($_POST['register_telegram_webhook'])) {
        $botToken = trim($_POST['tg_bot_token'] ?? '');
        $botUsername = trim($_POST['tg_bot_username'] ?? '');
        $adminChatId = trim($_POST['tg_admin_chat_id'] ?? '');
        $channelIds = trim($_POST['tg_channel_ids'] ?? '');
        $broadcastChannel = trim($_POST['tg_broadcast_channel'] ?? '');
        $discussionGroup = trim($_POST['tg_discussion_group'] ?? '');
        $premiumEmojiId = trim($_POST['tg_premium_emoji_id'] ?? '');

        $extraEmojis = [
            'emoji_header'   => trim($_POST['tg_emoji_header'] ?? ''),
            'emoji_success'  => trim($_POST['tg_emoji_success'] ?? ''),
            'emoji_key'      => trim($_POST['tg_emoji_key'] ?? ''),
            'emoji_delivery' => trim($_POST['tg_emoji_delivery'] ?? ''),
            'emoji_warning'  => trim($_POST['tg_emoji_warning'] ?? ''),
            'emoji_bell'     => trim($_POST['tg_emoji_bell'] ?? ''),
            'custom_emojis'  => trim($_POST['tg_custom_emojis'] ?? ''),
        ];

        $envToken = trim((string)getenv('TELEGRAM_BOT_TOKEN'));
        $me = telegram_api_call('getMe', [], $botToken);
        if ($envToken !== '' && !hash_equals($envToken, $botToken)) {
            $channelError = 'Host đang dùng TELEGRAM_BOT_TOKEN khác. Hãy cập nhật biến môi trường này trước khi lưu token mới.';
        } elseif (empty($me['ok']) || empty($me['result']['username'])) {
            $channelError = telegram_error_message($me) . ' Cấu hình cũ chưa bị thay đổi.';
        } else {
            $botUsername = $me['result']['username'];
            try {
                save_telegram_bot_config($botToken, $botUsername, $adminChatId, $channelIds, $premiumEmojiId, $extraEmojis, $broadcastChannel, $discussionGroup, isset($_POST['tg_key_channel_notifications']));
                $channelSaved = true;
                $tgBotFeedback = 'Đã kiểm tra token và lưu cấu hình @' . $botUsername . '. Hãy kích hoạt Webhook để bot nhận tin nhắn.';
            } catch (\Throwable $e) {
                $channelError = 'Không lưu được cấu hình vào database. Kiểm tra quyền ghi thư mục data và thử lại.';
            }
        }

        if ($channelSaved && isset($_POST['register_telegram_webhook'])) {
            if ($botToken === '') {
                $channelError = 'Vui lòng nhập Bot Token trước khi kích hoạt Webhook.';
            } else {
                $webhookUrl = rtrim(BASE_URL, '/') . '/telegram_webhook.php';
                $res = telegram_api_call('setWebhook', [
                    'url'             => $webhookUrl,
                ...
                if (!empty($res['ok'])) {
                    $channelSaved = true;
                    $tgBotFeedback = '✅ Đã kích hoạt Webhook thành công tới: ' . $webhookUrl . ' (Đã đăng ký theo dõi sự kiện chat_member khi user thoát nhóm)';
                } else {
                    $channelError = telegram_error_message($res);
                }
            }
        }
    }
    if (isset($_POST['test_admin_telegram_msg'])) {
        $botToken = trim($_POST['tg_bot_token'] ?? '');
        $adminChatId = trim($_POST['tg_admin_chat_id'] ?? '');
        if ($botToken === '' || $adminChatId === '') {
            $channelError = 'Vui lòng nhập đầy đủ Bot Token và ID Chat Admin để gửi tin nhắn test.';
        } else {
            $icH = telegram_premium_icon('header', '👑');
            $icS = telegram_premium_icon('success', '✔');
            $icK = telegram_premium_icon('key', '🔑');
            $icD = telegram_premium_icon('delivery', '⚡');
            $icW = telegram_premium_icon('warning', '⚠️');
            $icB = telegram_premium_icon('bell', '📢');
            $testMsg = "<blockquote>" .
                       "{$icH} <b>BẢNG TEST ICON TELEGRAM PREMIUM ĐA DẠNG</b>\n\n" .
                       "{$icH} <b>Tiêu đề / VIP:</b> Đang hoạt động chuẩn\n" .
                       "{$icS} <b>Thành công / Duyệt:</b> Đã xác minh thành viên\n" .
                       "{$icK} <b>Mã Key kích hoạt:</b> <code>KEY-TEST-MULTI-ICONS-OK</code>\n" .
                       "{$icD} <b>Giao hàng:</b> Tự động 100% (24 giờ)\n" .
                       "{$icW} <b>Cảnh báo quy định:</b> Thoát nhóm xoá key\n" .
                       "{$icB} <b>Thông báo quản trị:</b> Hệ thống kết nối tốt\n" .
                       "</blockquote>\n" .
                       "<i>Chat ID Admin: <code>" . htmlspecialchars($adminChatId) . "</code> - Đã áp dụng thành công bộ Icon theo nội dung!</i>";
            ...
            if (!empty($res['ok'])) {
                $channelSaved = true;
                $fallbackNote = !empty($res['used_fallback'])
                    ? '<br><small style="color:#f59e0b">⚠️ <b>Ghi chú:</b> Bot của bạn là Bot thường (chưa có Telegram Premium), Telegram không cho phép bot thường gửi Custom Emoji động (lỗi <code>DOCUMENT_INVALID</code>). Hệ thống đã tự động chuyển sang Icon Unicode chuẩn đẹp mắt để đảm bảo gửi tin nhắn thành công 100%!</small>'
                    : '';
                $tgBotFeedback = '✅ Đã gửi tin nhắn test thành công tới ID Chat: ' . htmlspecialchars($adminChatId) . $fallbackNote;
            } else {
                $channelError = '❌ Gửi test thất bại: ' . ($res['description'] ?? 'Vui lòng kiểm tra lại ID Chat hoặc bấm /start với bot trước');
            }
        }
    }
    if (isset($_POST['test_start_msg_to_admin'])) {
        $cfg = get_telegram_bot_config();
        $adminChatId = trim((string)($cfg['admin_chat_id'] ?? ''));
        if ($adminChatId === '') {
            $channelError = 'Vui lòng điền Chat ID Admin ở phần cấu hình Bot để gửi thử nghiệm tin nhắn /start.';
        } else {
            $startData = render_tg_membership_content((int)$adminChatId, 'Admin Test', '');
            $testRes = telegram_api_call('sendMessage', [
                'chat_id'      => $adminChatId,
                'text'         => $startData['text'],
                'parse_mode'   => 'HTML',
                'reply_markup' => [
                    'inline_keyboard' => $startData['keyboard']
                ]
            ]);
            if (!empty($testRes['ok'])) {
                $channelSaved = true;
                if (!empty($testRes['used_fallback'])) {
                    $tgBotFeedback = '⚠️ <b>Đã gửi tin nhắn /start tới Telegram của bạn NHƯNG bị Telegram từ chối Icon Custom Emoji!</b><br>' .
                        'Lý do Telegram từ chối Custom Emoji: <code>' . htmlspecialchars($testRes['original_error'] ?? 'Không rõ') . '</code>';
                } else {
                    $tgBotFeedback = '🎉 <b>Thành công rực rỡ! Telegram đã chấp nhận Icon Premium động cho tin nhắn /start!</b> Hãy mở Telegram xem tin nhắn vừa gửi tới bạn!';
                }
            } else {
                $channelError = '❌ Gửi thử /start thất bại: ' . ($testRes['description'] ?? 'Lỗi không xác định');
            }
        }
    }
    if (isset($_POST['check_telegram_bot'])) {
        $botToken = trim($_POST['tg_bot_token'] ?? '');
        if ($botToken === '') {
            $channelError = 'Vui lòng nhập Bot Token để kiểm tra.';
        } else {
            $me = telegram_api_call('getMe', [], $botToken);
            $hook = telegram_api_call('getWebhookInfo', [], $botToken);
            $tgBotStatusInfo = ['me' => $me, 'hook' => $hook];
        }
    }
    if (isset($_POST['delete_telegram_webhook'])) {
        $res = telegram_api_call('deleteWebhook', []);
        if (!empty($res['ok'])) {
            $channelSaved = true;
            $tgBotFeedback = '✅ Đã huỷ liên kết Webhook.';
        } else {
            $channelError = '❌ Lỗi huỷ Webhook: ' . ($res['description'] ?? 'Không kết nối được tới Telegram API');
        }
    }

    // Sao lưu & Khôi phục Icon Telegram Premium
    if (isset($_POST['backup_icons_now'])) {
        $res = backup_telegram_icons_config();
        $channelSaved = true;
        $tgBotFeedback = '✅ Đã tạo bản sao lưu Icon Premium mới nhất thành công (Thời gian: ' . ($res['backed_up_date'] ?? date('H:i:s d/m/Y')) . ')!';
    }
    if (isset($_POST['restore_latest_icons_backup'])) {
        $ok = restore_telegram_icons_config();
        if ($ok) {
            $channelSaved = true;
            $tgBotFeedback = '✅ Đã khôi phục thành công cấu hình Icon Premium từ bản sao lưu gần nhất!';
        } else {
            $channelError = '❌ Không tìm thấy bản sao lưu Icon Premium nào để khôi phục.';
        }
    }
    if (isset($_POST['restore_icons_file'])) {
        if (!empty($_FILES['icons_backup_file']['tmp_name'])) {
            $content = @file_get_contents($_FILES['icons_backup_file']['tmp_name']);
            $data = json_decode($content, true);
            if (is_array($data)) {
                $cur = get_telegram_bot_config();
                $merged = array_merge($cur, $data);
                save_telegram_bot_config(
                    $merged['token'] ?? '',
                    $merged['username'] ?? '',
                    $merged['admin_chat_id'] ?? '',
                    $merged['channel_ids'] ?? '',
                    $merged['premium_emoji_id'] ?? '',
                    $merged
                );
                $channelSaved = true;
                $tgBotFeedback = '✅ Đã khôi phục thành công toàn bộ cấu hình Bot Telegram và Icon từ file JSON tải lên (Đã lưu vĩnh viễn vào Database)!';
            } else {
                $channelError = '❌ File tải lên không hợp lệ hoặc không đúng định dạng JSON.';
            }
        } else {
            $channelError = '❌ Vui lòng chọn file JSON sao lưu từ máy tính để khôi phục.';
        }
    }
    if (isset($_POST['apply_icons_preset'])) {
        restore_telegram_icons_config(get_default_telegram_icons_preset());
        $channelSaved = true;
        $tgBotFeedback = '✅ Đã nạp thành công bộ Icon Telegram Premium mẫu chuẩn đẹp mắt!';
    }

    // Gửi thông báo tới người dùng Bot Telegram (Broadcast All Users)
    if (isset($_POST['send_broadcast_telegram']) || isset($_POST['test_broadcast_telegram'])) {
        $bTitle = trim($_POST['broadcast_title'] ?? '');
        $bContent = trim($_POST['broadcast_content'] ?? '');
        $bIconId = trim($_POST['broadcast_icon_id'] ?? '');
        $bBtnText = trim($_POST['broadcast_btn_text'] ?? '');
        $bBtnUrl = trim($_POST['broadcast_btn_url'] ?? '');

        if ($bContent === '') {
            $channelError = 'Vui lòng nhập nội dung thông báo cần gửi.';
        } else {
            if (isset($_POST['test_broadcast_telegram'])) {
                $cfg = get_telegram_bot_config();
                $adminChatId = trim((string)($cfg['admin_chat_id'] ?? ''));
                if ($adminChatId === '') {
                    $channelError = 'Vui lòng điền Chat ID Admin ở phần cấu hình Bot để gửi thử nghiệm.';
                } else {
                    $testRes = send_telegram_broadcast_single($adminChatId, $bTitle, $bContent, $bIconId, $bBtnText, $bBtnUrl);
                    if (!empty($testRes['ok'])) {
                        $channelSaved = true;
                        $tgBotFeedback = "✅ Đã gửi thử thông báo tới Chat ID Admin ({$adminChatId}) thành công! Vui lòng kiểm tra Telegram để xem trước giao diện và icon.";
                    } else {
                        $channelError = '❌ Gửi thử thất bại: ' . ($testRes['description'] ?? 'Lỗi không xác định');
                    }
                }
            } else {
                $res = broadcast_telegram_message($bTitle, $bContent, $bIconId, $bBtnText, $bBtnUrl);
                if (!empty($res['success'])) {
                    $channelSaved = true;
                    $tgBotFeedback = "📢 Đã phát thông báo thành công tới {$res['sent']}/{$res['total']} người dùng! (Thất bại/chặn bot: {$res['failed']})";
                } else {
                    $channelError = '❌ ' . ($res['message'] ?? 'Không thể gửi thông báo.');
                }
            }
        }
    }
    ...
    if (isset($_POST['create_key'])) {
        ...
        $durationSeconds = PUBLIC_KEY_SECONDS;
    ...
    // Xoá hàng loạt key ĐANG ACTIVE nhưng CHƯA TỪNG DÙNG trong app -
    // $unusedMinHours để tránh xoá nhầm key vừa phát cho user vài phút
    // trước (họ có thể chưa kịp mở app). Mặc định 1 tiếng, admin có thể
    // đổi số giờ trong ô nhập trước khi bấm.
    if (isset($_POST['cleanup_unused_active'])) {
        $unusedMinHours = max(0, (float)($_POST['unused_min_hours'] ?? 1));
        $cleanupUnusedActiveCount = cleanup_unused_active_keys($db, (int)round($unusedMinHours * 3600));
    }
    ...
    // Lưu link liên hệ MẶC ĐỊNH toàn hệ thống (nút liên hệ nổi ở index.php).
    ...
    // Lưu link tải APK hiển thị ở nút "Tải APK" đầu trang index.php
    if (isset($_POST['save_apk_link'])) {
        save_apk_link(trim($_POST['apk_link'] ?? ''));
        $apkLinkSaved = true;
    }
    ...
    if (isset($_POST['unblock_fp'])) {
        unblock_fp(trim($_POST['unblock_fp']));
    }
...
if (isset($_SESSION['is_admin'])) {
    ...
    $params = [];
    if ($searchQ !== '') {
        $cleanQ = strtoupper(preg_replace('/[^A-Za-z0-9]/', '', $searchQ));
        $where .= " AND (REPLACE(UPPER(keycode), '-', '') LIKE ? OR tg_user_id LIKE ?)";
        $params[] = '%' . $cleanQ . '%';
        $params[] = '%' . $searchQ . '%';
    }
    if ($filterStatus === 'active_unused') {
        $where .= " AND status = 'active' AND first_used_at IS NULL";
    } elseif (in_array($filterStatus, ['active', 'pending', 'expired'], true)) {
        $where .= " AND status = ?";
        $params[] = $filterStatus;
    }
...
// Icon nét mảnh (svg_icon()) dùng chung trong config.php
...
:root {
    ...
    --line-strong: rgba(30, 41, 59, 0.22);
    --cyan: #0284C7;
    --violet: #6D28D9;
    --text: #0F172A;
    --text-dim: #475569;
...
body {
    ...
    color: var(--text);
    background:
        radial-gradient(circle at 10% -5%, rgba(2, 132, 199, 0.07), transparent 45rem),
        radial-gradient(circle at 90% 15%, rgba(109, 40, 217, 0.05), transparent 40rem),
        #F8FAFC;
}
body::before {
...
h4 { font-size: 12.5px; font-family: var(--font-mono); letter-spacing: .05em; color: var(--text-dim); text-transform: uppercase; margin: 0 0 8px; font-weight: 700; }
code { padding: 3px 6px; border-radius: var(--r-sm); background: #F1F5F9; color: var(--cyan); font-family: var(--font-mono); font-size: .88em; border: 1px solid var(--line); }

/* Form Controls */
input, select, textarea {
    min-height: 40px; padding: 8px 12px; margin: 4px 4px 4px 0;
    border: 1px solid var(--line-strong); border-radius: var(--r-md);
    background: #FFFFFF; color: var(--text); font-family: var(--font-body); font-size: 13px;
    box-shadow: 0 1px 2px rgba(15, 23, 42, 0.04); transition: border-color .15s, box-shadow .15s;
}
input::placeholder { color: var(--text-muted); }
input:focus, select:focus, textarea:focus {
    outline: none; border-color: var(--cyan); box-shadow: 0 0 0 3px rgba(2, 132, 199, 0.16);
}

button {
    min-height: 40px; padding: 8px 16px; border: none; border-radius: var(--r-md);
    background: linear-gradient(135deg, #0284C7 0%, #2563EB 60%, #4F46E5 100%); color: #FFFFFF; font-family: var(--font-body); font-weight: 700; font-size: 13px;
    cursor: pointer; transition: transform .12s, filter .15s, box-shadow .15s;
    box-shadow: 0 4px 14px -2px rgba(2, 132, 199, 0.35);
}
button:hover { filter: brightness(1.06); box-shadow: 0 6px 18px -2px rgba(2, 132, 199, 0.45); }
button:active { transform: scale(.97); }
button.danger {
    background: rgba(225, 29, 72, 0.08); border: 1px solid rgba(225, 29, 72, 0.25);
    color: var(--danger); box-shadow: none;
}
button.danger:hover { background: rgba(225, 29, 72, 0.15); filter: none; }
button.warn {
    background: rgba(217, 119, 6, 0.08); border: 1px solid rgba(217, 119, 6, 0.25);
    color: var(--warn); box-shadow: none;
}
button.warn:hover { background: rgba(217, 119, 6, 0.15); filter: none; }
...
.box {
    ...
    background: #FFFFFF; box-shadow: 0 4px 20px -2px rgba(15, 23, 42, 0.05), 0 0 0 1px rgba(15, 23, 42, 0.03);
    margin-bottom: 20px;
}
.box::before {
    content: ""; position: absolute; left: 22px; right: 22px; top: 0; height: 2px;
    background: linear-gradient(90deg, #0284C7, #38BDF8 35%, #818CF8 70%, transparent);
    border-radius: 2px;
}
...
.ok { background: rgba(5, 150, 105, 0.08); border: 1px solid rgba(5, 150, 105, 0.25); color: var(--success); }
.err { background: rgba(225, 29, 72, 0.08); border: 1px solid rgba(225, 29, 72, 0.25); color: var(--danger); }

/* Table */
.tablewrap {
    border: 1px solid var(--line); border-radius: var(--r-lg); overflow: auto;
    background: #FFFFFF; box-shadow: 0 1px 3px rgba(15, 23, 42, 0.05); -webkit-overflow-scrolling: touch;
}
...
th {
    height: 38px; background: #F1F5F9; font-family: var(--font-mono);
    font-size: 10.5px; letter-spacing: .08em; color: var(--text-dim); text-transform: uppercase; font-weight: 700;
}
...
.badge.active, .badge.on { background: rgba(5, 150, 105, 0.1); border-color: rgba(5, 150, 105, 0.25); color: var(--success); }
.badge.pending { background: rgba(217, 119, 6, 0.1); border-color: rgba(217, 119, 6, 0.25); color: var(--warn); }
.badge.expired { background: rgba(225, 29, 72, 0.1); border-color: rgba(225, 29, 72, 0.25); color: var(--danger); }
.badge.off { background: #F1F5F9; border-color: var(--line); color: var(--text-dim); }

/* Login Box */
.loginbox {
    max-width: 380px; margin: 12vh auto; padding: 36px 30px; border-radius: var(--r-2xl);
    border: 1px solid var(--line); background: #FFFFFF;
    box-shadow: 0 20px 45px -15px rgba(15, 23, 42, 0.12), 0 0 0 1px rgba(15, 23, 42, 0.04);
    animation: ds-rise .4s var(--ease-out);
}
.loginbox h2 {
    margin: 0 0 20px; font-size: 24px; text-align: center; font-weight: 800;
    color: #0F172A; letter-spacing: -0.02em;
}
.loginbox::after {
    content: "SECURE CONTROL PLANE"; display: block; margin-top: 22px;
...
.game-card {
    ...
    background: #FFFFFF; box-shadow: 0 2px 8px rgba(15, 23, 42, 0.04); margin-bottom: 12px;
    transition: border-color .15s, transform .15s, box-shadow .15s;
}
.game-card:hover { transform: translateY(-2px); border-color: rgba(2, 132, 199, 0.35); box-shadow: 0 6px 16px rgba(15, 23, 42, 0.08); }
...
.region-form { padding: 14px; border: 1px solid var(--line); border-radius: var(--r-md); background: #F8FAFC; }
...
/* Key Link Card */
.getkey-link-card {
    margin-top: 14px; padding: 16px; border: 1px solid rgba(2, 132, 199, 0.25); border-radius: var(--r-lg);
    background: linear-gradient(135deg, rgba(2, 132, 199, 0.06), rgba(99, 102, 241, 0.04));
}
.getkey-link-title { font: var(--fw-bold) 14px var(--font-body); color: var(--text); }
.getkey-link-sub { font-size: 12px; color: var(--text-dim); margin: 3px 0 10px; }
.getkey-link-row { display: flex; gap: 8px; align-items: stretch; }
.getkey-link-row input {
    flex: 1; min-width: 0; min-height: 42px; background: #FFFFFF; border: 1px solid var(--line-strong);
...
.getkey-link-row button {
    min-height: 42px; background: #0284C7; color: #FFFFFF; border-radius: var(--r-md); padding: 0 16px;
    display: flex; align-items: center; gap: 6px; font-family: var(--font-body); font-weight: 700; font-size: 12.5px;
    flex-shrink: 0; box-shadow: 0 2px 8px rgba(2, 132, 199, 0.3);
}
.getkey-link-row button:hover { filter: brightness(1.08); }
...
.topbar b { font-size: 15px; letter-spacing: -.01em; color: var(--text); }
.topbar b::before { content: "HQ / "; color: var(--cyan); font: 700 11px var(--font-mono); letter-spacing: .12em; }
.topbar > a {
    padding: 7px 12px; border: 1px solid var(--line); border-radius: var(--r-sm);
    background: #FFFFFF; color: var(--text-dim) !important; text-decoration: none !important; font-size: 12.5px; transition: all .15s;
}
.topbar > a:hover { border-color: var(--cyan); color: var(--cyan) !important; }
.hamburger {
    width: 38px; height: 38px; min-height: 38px; padding: 7px; border: 1px solid var(--line);
    border-radius: var(--r-md); background: #F8FAFC; color: var(--text); cursor: pointer; box-shadow: none;
}
.hamburger:hover { background: #F1F5F9; border-color: var(--line-strong); }

/* Sidebar */
.sidebar-overlay { position: fixed; inset: 0; background: rgba(15, 23, 42, 0.35); backdrop-filter: blur(4px); opacity: 0; pointer-events: none; transition: opacity .25s ease; z-index: 40; }
...
.sidebar {
    ...
    display: flex; flex-direction: column; box-shadow: 18px 0 50px rgba(15, 23, 42, 0.12);
}
.sidebar.open { transform: translateX(0); }
.sidebar-section-label { font-family: var(--font-mono); font-size: 10px; font-weight: 700; letter-spacing: .14em; color: var(--text-dim); text-transform: uppercase; padding: 24px 22px 14px; }
...
.sidebar-item:not(.active):hover { background: #F1F5F9; color: var(--text); }
.sidebar-item .ic { font-size: 17px; width: 20px; text-align: center; flex-shrink: 0; }
.sidebar-item.active { border-color: rgba(2, 132, 199, 0.25); background: #E0F2FE; color: #0284C7; font-weight: 700; }
.sidebar-item.active .ic { color: #0284C7; }
.sidebar-item .chev { margin-left: auto; color: var(--text-dim); font-size: 15px; }
.sidebar-item.active .chev { color: #0284C7; }
.sidebar-footer { padding: 16px; border-top: 1px solid var(--line); background: #F8FAFC; }
.sidebar-back {
    display: flex; align-items: center; justify-content: center; gap: 8px; width: 100%; min-height: 42px;
    background: #FFFFFF; border: 1px solid var(--line); color: var(--text); border-radius: var(--r-md);
    text-decoration: none; font-weight: 700; font-size: 13px; margin-bottom: 10px; cursor: pointer; transition: all .15s;
    box-shadow: 0 1px 2px rgba(15, 23, 42, 0.04);
}
.sidebar-back:hover { border-color: var(--cyan); color: var(--cyan); background: #F0F9FF; }
.sidebar-account { display: flex; align-items: center; gap: 10px; padding: 6px 4px; }
.sidebar-avatar {
    width: 36px; height: 36px; border-radius: 50%; background: linear-gradient(135deg, #0284C7, #7C3AED);
    display: flex; align-items: center; justify-content: center; font-weight: 700; color: #FFFFFF; font-size: 14px; flex-shrink: 0;
...
.provider-row { display: flex; align-items: center; justify-content: space-between; padding: 10px 14px; border: 1px solid var(--line); border-radius: var(--r-md); background: #FFFFFF; margin-bottom: 8px; transition: transform .15s, border-color .15s; box-shadow: 0 1px 3px rgba(15, 23, 42, 0.04); }
.provider-row:hover { transform: translateX(2px); border-color: rgba(2, 132, 199, 0.3); }

/* Stats */
.stats-grid { display: grid; grid-template-columns: repeat(3, 1fr); gap: 12px; }
.stat-card {
    padding: 18px 14px; border: 1px solid var(--line); border-radius: var(--r-lg);
    background: #FFFFFF; text-align: center; box-shadow: 0 2px 8px rgba(15, 23, 42, 0.04);
    transition: transform .15s, border-color .15s, box-shadow .15s;
}
.stat-card:hover { transform: translateY(-2px); border-color: rgba(2, 132, 199, 0.35); box-shadow: 0 6px 16px rgba(15, 23, 42, 0.08); }
.stat-ic { display: block; margin: 0 auto 6px; color: var(--cyan); opacity: .9; }
.stat-num { font-family: var(--font-mono); font-size: 26px; font-weight: 700; color: #0284C7; letter-spacing: -.03em; font-variant-numeric: tabular-nums; }
.stat-label { font-size: 11.5px; color: var(--text-dim); margin-top: 3px; font-weight: 500; }

/* Power Toggle */
.power-toggle {
    width: 72px; height: 40px; border-radius: var(--r-full); border: none; cursor: pointer; position: relative;
    transition: background .25s ease; padding: 0; box-shadow: 0 0 0 5px rgba(5, 150, 105, 0.1);
}
.power-toggle.on { background: linear-gradient(135deg, var(--success), #059669); }
.power-toggle.off { background: #CBD5E1; box-shadow: 0 0 0 5px rgba(148, 163, 184, 0.15); }
.power-toggle-knob {
    position: absolute; top: 4px; width: 32px; height: 32px; border-radius: 50%; background: #fff;
    box-shadow: 0 2px 6px rgba(0,0,0,0.18); transition: left .25s var(--ease-spring);
}
.power-toggle.on .power-toggle-knob { left: 36px; }
...
<?php if (!isset($_SESSION['is_admin'])): ?>
    <div class="box loginbox">
        ...
        <?php if ($isLocked): ?>
            <p class="err">Tài khoản tạm khóa do sai nhiều lần, thử lại sau <?= ceil($lockRemaining / 60) ?> phút.</p>
        <?php else: ?>
...
<?php else: ?>
    ...
    <div class="box" style="border:1px solid rgba(255,107,107,.4);background:rgba(255,107,107,.08)">
        <h3 style="color:var(--danger, #FF6B6B)">⚠️ Đang dùng mật khẩu mặc định</h3>
        <p style="font-size:13px;color:var(--text-dim);margin:0">
            Tài khoản admin chưa từng được đổi mật khẩu (đang chạy bằng giá trị mặc định trong code).
            Nếu mã nguồn từng bị lộ (ví dụ commit nhầm lên GitHub công khai), bất kỳ ai đọc được cũng đăng nhập được ngay.
            Hãy đổi mật khẩu ở mục <b>"Đổi mật khẩu Admin"</b> bên dưới hoặc set biến môi trường <code>ADMIN_PASSWORD</code>.
        </p>
    </div>
    <?php endif; ?>
        ...
        <div style="display:flex;align-items:center;gap:22px;flex-wrap:wrap;justify-content:center">
            <div style="position:relative;width:140px;height:140px;flex-shrink:0">
                <svg viewBox="0 0 36 36" style="width:100%;height:100%;transform:rotate(0deg)">
                    <circle cx="18" cy="18" r="15.9155" fill="none" stroke="var(--surface3, #E2E8F0)" stroke-width="3.6"/>
    ...
    <div class="box">
        ...
        <div class="tablewrap">
        <table>
            <tr><th>IP</th><th>Thiết bị</th><th>Số lần vi phạm</th><th>Lý do gần nhất</th><th>Chặn tới</th><th></th></tr>
            <?php foreach ($blocked as $b): ?>
            <tr>
                <td><?= htmlspecialchars($b['ip']) ?></td>
                <td><?= htmlspecialchars($b['last_platform'] ?: '?') ?></td>
            ...
            <?php endforeach; ?>
            <?php if (empty($blocked)): ?><tr><td colspan="6" style="color:#888">Không có IP nào bị chặn</td></tr><?php endif; ?>
        </table>
        </div>
    </div>

    <div class="box">
        <h3><?= svg_icon('shield', 17) ?> Trình duyệt bị chặn (đổi IP né chặn)</h3>
        <p style="font-size:12px;color:#888;margin-top:-4px">Nhận diện qua cookie trình duyệt (gk_fp) - bắt được trường hợp đổi mạng/IP (4G, wifi, VPN...) nhưng chưa xoá cookie. Không thay thế bảng IP ở trên, chạy song song.</p>
        <?php $blockedFps = get_blocked_fps(); ?>
        <div class="tablewrap">
        <table>
            <tr><th>FP (cookie)</th><th>Thiết bị</th><th>Số lần vi phạm</th><th>Lý do gần nhất</th><th>Chặn tới</th><th></th></tr>
            <?php foreach ($blockedFps as $b): ?>
            <tr>
                <td style="font-family:monospace" title="<?= htmlspecialchars($b['fp']) ?>"><?= htmlspecialchars(substr($b['fp'], 0, 12)) ?>…</td>
                <td><?= htmlspecialchars($b['last_platform'] ?: '?') ?></td>
                <td><?= (int)$b['violation_count'] ?></td>
                <td style="max-width:200px;white-space:normal"><?= htmlspecialchars($b['last_reason']) ?></td>
                <td><?= fmt_time((int)$b['blocked_until']) ?></td>
                <td>
                    <form method="post" style="display:inline"><input type="hidden" name="csrf_token" value="<?= htmlspecialchars($_SESSION['csrf_token'] ?? '') ?>">
                        <input type="hidden" name="unblock_fp" value="<?= htmlspecialchars($b['fp']) ?>">
                        <button class="warn small" type="submit">Gỡ chặn</button>
                    </form>
                </td>
            </tr>
            <?php endforeach; ?>
            <?php if (empty($blockedFps)): ?><tr><td colspan="6" style="color:#888">Không có trình duyệt nào bị chặn</td></tr><?php endif; ?>
        </table>
        </div>
    </div>
    ...
    <div class="box">
        ...
        <?php if (!empty($_SESSION['last_shorten_debug'])): $dbg = $_SESSION['last_shorten_debug']; ?>
        <div style="margin:14px 0;padding:13px 14px;border:1px solid rgba(255,156,166,.25);border-radius:12px;background:rgba(255,120,133,.06)">
            <b style="font-size:12.5px;color:#FF9CA6">🔍 Debug: lần gọi API rút gọn gần nhất</b>
            <p style="margin:8px 0 3px;font-size:11.5px;color:#aaa">Provider: <b><?= htmlspecialchars($dbg['provider']) ?></b> · HTTP <?= htmlspecialchars((string)$dbg['http_code']) ?> · <?= htmlspecialchars($dbg['time']) ?></p>
            <?php if ($dbg['curl_err']): ?><p style="margin:3px 0;font-size:11.5px;color:#FF9CA6">Lỗi kết nối: <?= htmlspecialchars($dbg['curl_err']) ?></p><?php endif; ?>
            <p style="margin:3px 0;font-size:11px;color:#888">URL gọi (đã che API key): <?= htmlspecialchars($dbg['url']) ?></p>
            <p style="margin:6px 0 2px;font-size:11.5px;color:#aaa">Response thô từ provider:</p>
            <pre style="margin:0;padding:9px 10px;border-radius:8px;background:#0A0E16;color:#8FE3C5;font-size:11px;white-space:pre-wrap;word-break:break-all"><?= htmlspecialchars($dbg['raw']) ?></pre>
        </div>
        <?php endif; ?>
    ...
    <div class="box">
        <h3>Link liên hệ mặc định</h3>
        <p style="font-size:12px;color:var(--text-dim);margin-top:-4px">
            Hiện ở nút liên hệ nổi trên trang chủ (index.php). Để trống ô nào thì icon đó tự ẩn.
        </p>
        <?php if (!empty($contactSaved)): ?><p class="ok">Đã lưu link liên hệ mặc định.</p><?php endif; ?>
        <?php $globalContact = get_global_contact(); ?>
        <form method="post">
            <input type="hidden" name="csrf_token" value="<?= htmlspecialchars($_SESSION['csrf_token'] ?? '') ?>">
            <input type="text" name="contact_zalo" value="<?= htmlspecialchars($globalContact['zalo']) ?>" placeholder="Link Zalo (vd: https://zalo.me/09xxxxxxxx)" style="width:100%;margin-bottom:6px"><br>
            <input type="text" name="contact_telegram" value="<?= htmlspecialchars($globalContact['telegram']) ?>" placeholder="Link Telegram (vd: https://t.me/username)" style="width:100%;margin-bottom:6px"><br>
            <input type="text" name="contact_facebook" value="<?= htmlspecialchars($globalContact['facebook']) ?>" placeholder="Link Facebook (vd: https://fb.com/fanpage)" style="width:100%;margin-bottom:10px"><br>
            <button type="submit" name="save_contact" value="1">Lưu link liên hệ</button>
        </form>
    </div>
    ...
    <div class="box">
        <h3>Link tải APK</h3>
        <p style="font-size:12px;color:var(--text-dim);margin-top:-4px">
            Hiện ở nút "Tải APK" đầu trang chủ (index.php). Để trống thì nút tự ẩn.
        </p>
        <?php if (!empty($apkLinkSaved)): ?><p class="ok">Đã lưu link tải APK.</p><?php endif; ?>
        <form method="post">
            <input type="hidden" name="csrf_token" value="<?= htmlspecialchars($_SESSION['csrf_token'] ?? '') ?>">
            <input type="text" name="apk_link" value="<?= htmlspecialchars(get_apk_link()) ?>" placeholder="Link file APK (vd: https://.../app-release.apk)" style="width:100%;margin-bottom:10px"><br>
            <button type="submit" name="save_apk_link" value="1">Lưu link APK</button>
        </form>
    </div>
    ...
    <div class="box">
        <h3>Cấu hình Game</h3>
        <p style="font-size:12px;color:#888;margin-top:-4px">Cấu hình thứ tự nhà cung cấp rút gọn link riêng cho khách VN và nước ngoài, từng game. Duy nhất Key 24 giờ, bắt buộc vượt 2 lần link.</p>
        ...
        <div class="game-card">
            ...
            <form method="post"><input type="hidden" name="csrf_token" value="<?= htmlspecialchars($_SESSION['csrf_token'] ?? '') ?>">
                ...
                <div class="region-form">
                    ...
                    <p style="font-size:11.5px;color:var(--text-dim);margin:2px 0 0">Duy nhất Key 24 giờ, bắt buộc vượt 2 lần link. Mục này chỉ còn dùng để chọn thứ tự provider.</p>
                    <input type="hidden" name="<?= $region ?>_hops" value="<?= $hops ?>">
                    <input type="hidden" name="<?= $region ?>_hours" value="<?= $hours ?>">
                    <label style="font-size:12px;color:#888">Thứ tự link vượt (tuỳ chọn, để trống = dùng provider active)</label>
                    <div class="step-row">
                        <?php for ($i = 1; $i <= PUBLIC_KEY_HOPS; $i++): $cur = $chain[$i - 1] ?? ''; ?>
    ...
    <div class="box">
        ...
        <?php if (!empty($tgBotFeedback)): ?><p class="ok"><?= htmlspecialchars($tgBotFeedback) ?></p><?php endif; ?>
        <?php if (!empty($channelSaved) && empty($tgBotFeedback)): ?><p class="ok">Đã cập nhật cấu hình kênh.</p><?php endif; ?>
        <?php if (!empty($channelError)): ?><p class="err"><?= htmlspecialchars($channelError) ?></p><?php endif; ?>

        <?php
            $tgBotCfg = get_telegram_bot_config();
            $webhookTargetUrl = rtrim(BASE_URL, '/') . '/telegram_webhook.php';
        ?>
        <div class="region-form" style="margin-top:17px;border-color:rgba(89,245,213,.22)">
            <h4 style="margin:0 0 5px;color:var(--text)">🤖 Cấu hình Bot Telegram (Xác minh nhiệm vụ &amp; Thông báo Admin)</h4>
            <p style="font-size:11.5px;color:var(--text-dim);margin:0 0 12px;max-width:720px">
                Nhập <b>Token Bot</b> từ @BotFather, <b>ID Channel</b> (các nhóm bắt buộc user phải vào để lấy key), và <b>ID Chat Admin</b> (nhận tin nhắn báo động khi IP bị khoá, backup lỗi...). Sau đó bấm <b>🔗 Kích hoạt Webhook</b> để kết nối tự động 1-click.
            </p>
            <form method="post" enctype="multipart/form-data" style="display:flex;flex-direction:column;gap:10px">
                ...
                <div style="display:flex;flex-wrap:wrap;gap:10px">
                    <div style="flex:1;min-width:260px">
                        <label style="display:block;font-size:11.5px;color:var(--text-dim);margin-bottom:4px">🔑 <b>Bot Token</b> (từ @BotFather):</label>
                        <input type="password" autocomplete="new-password" name="tg_bot_token" value="<?= htmlspecialchars($tgBotCfg['token']) ?>" placeholder="vd: 7123456789:AAFxYz..." style="width:100%">
                    </div>
                    <div style="flex:1;min-width:200px">
                        <label style="display:block;font-size:11.5px;color:var(--text-dim);margin-bottom:4px">👤 <b>Bot Username</b> (không cần @):</label>
                        <input type="text" name="tg_bot_username" value="<?= htmlspecialchars($tgBotCfg['username']) ?>" placeholder="vd: MyVerifyBot" style="width:100%">
                    </div>
                </div>

                <div style="display:flex;flex-wrap:wrap;gap:10px">
                    <div style="flex:1;min-width:260px">
                        <label style="display:block;font-size:11.5px;color:var(--text-dim);margin-bottom:4px">📢 <b>ID Channel / Nhóm nhiệm vụ</b> (ngăn cách bằng dấu phẩy):</label>
                        <input type="text" name="tg_channel_ids" value="<?= htmlspecialchars($tgBotCfg['channel_ids'] ?? '') ?>" placeholder="vd: @hoquoc30, @AuraPingVN hoặc ID số -100..." style="width:100%">
                        <span style="font-size:10.5px;color:var(--text-dim)">User phải tham gia đủ các nhóm này thì bot mới duyệt mở khoá trên web.</span>
                    </div>
                    <div style="flex:1;min-width:200px">
                        <label style="display:block;font-size:11.5px;color:var(--text-dim);margin-bottom:4px">💬 <b>ID Chat Admin</b> (nhận thông báo bot):</label>
                        <input type="text" name="tg_admin_chat_id" value="<?= htmlspecialchars($tgBotCfg['admin_chat_id'] ?? '') ?>" placeholder="vd: 123456789 (lấy từ @userinfobot)" style="width:100%">
                        <span style="font-size:10.5px;color:var(--text-dim)">Chat với @userinfobot trên Telegram để xem ID chat của bạn.</span>
                    </div>
                </div>

                <div style="display:flex;flex-wrap:wrap;gap:10px;margin-top:2px">
                    <div style="flex:1;min-width:260px">
                        <label style="display:block;font-size:11.5px;color:var(--text-dim);margin-bottom:4px">📡 <b>Kênh Thông Báo Phát Key (Channel)</b>:</label>
                        <input type="text" name="tg_broadcast_channel" value="<?= htmlspecialchars($tgBotCfg['broadcast_channel'] ?? '') ?>" placeholder="vd: @AuraPingVN (kênh channel phát tin nhận key)" style="width:100%">
                        <label style="display:flex;align-items:center;gap:8px;margin:10px 0 5px;cursor:pointer">
                            <input type="checkbox" name="tg_key_channel_notifications" value="1" <?= ($tgBotCfg['key_channel_notifications'] ?? '1') === '1' ? 'checked' : '' ?> style="width:auto;margin:0">
                            <b>Gửi thông báo nhận key thành công lên channel</b>
                        </label>
                        <p style="font-size:11.5px;color:var(--text-dim);margin:0 0 8px">Bật: gửi lên channel và gửi riêng cho người nhận. Tắt: chỉ gửi riêng cho người nhận key. Bấm Lưu Cấu Hình để áp dụng.</p>
                        <span style="font-size:10.5px;color:var(--text-dim)">Chỉ đăng tin phát Key 24H (vượt 2 lần) (che mã key &amp; tag user). Không gửi tin tham gia / rời nhóm vào đây.</span>
                    </div>
                    <div style="flex:1;min-width:200px">
                        <label style="display:block;font-size:11.5px;color:var(--text-dim);margin-bottom:4px">👥 <b>Nhóm Chat Thảo Luận (Group)</b>:</label>
                        <input type="text" name="tg_discussion_group" value="<?= htmlspecialchars($tgBotCfg['discussion_group'] ?? '') ?>" placeholder="vd: @hoquoc30 (nhóm chat thảo luận)" style="width:100%">
                        <span style="font-size:10.5px;color:var(--text-dim)">Nhận tin chào mừng user tham gia đủ nhóm và tin xoá key khi thoát nhóm.</span>
                    </div>
                </div>

                <div style="margin-top:10px;padding:14px 16px;background:rgba(156,140,255,.06);border:1px solid rgba(156,140,255,.25);border-radius:8px">
                    <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:8px">
                        <label style="font-size:12.5px;font-weight:700;color:var(--text);display:flex;align-items:center;gap:6px">
                            <span>⭐</span> <b>CẤU HÌNH NHIỀU ICON TELEGRAM PREMIUM (THEO NỘI DUNG BOT GỬI)</b>
                        </label>
                        <span style="font-size:10.5px;color:var(--ds-violet);background:rgba(156,140,255,.15);padding:2px 8px;border-radius:10px">Custom Emoji Multi-Icons</span>
                    </div>
                    <p style="font-size:11.5px;color:var(--text-dim);margin:0 0 12px;line-height:1.5">
                        Bạn có thể tùy thích gán các ID Icon Telegram Premium khác nhau cho từng loại nội dung bot gửi (Tiêu đề, Tích xanh duyệt, Mã Key, Giao hàng tự động, Cảnh báo huỷ key...). Nếu ô nào để trống, bot sẽ tự động lấy <b>ID Icon Mặc Định</b>.<br>
                        <span style="color:#f59e0b">💡 <b>Lưu ý kỹ thuật:</b> Telegram quy định chỉ những Bot có Telegram Premium hoặc bot sở hữu bộ sticker đó mới gửi được Custom Emoji động (<code>&lt;tg-emoji&gt;</code>). Nếu bot của bạn là Bot thường, hệ thống đã tích hợp bộ chuyển đổi tự động sang Icon Unicode chuẩn đẹp mắt để đảm bảo tin nhắn không bao giờ bị lỗi <code>DOCUMENT_INVALID</code>!</span>
                    </p>

                    <div style="margin-bottom:12px">
                        <label style="display:block;font-size:11.5px;color:var(--text);margin-bottom:4px">⭐ <b>ID Icon Mặc Định (Để trống nếu chỉ dùng Icon Unicode thường):</b></label>
                        <input type="text" name="tg_premium_emoji_id" value="<?= htmlspecialchars($tgBotCfg['premium_emoji_id'] ?? '') ?>" placeholder="vd: 5467659213675176899 (hoặc để trống nếu dùng bot thường)" style="width:100%;border-color:rgba(156,140,255,.4)">
                    </div>

                    <div style="display:grid;grid-template-columns:repeat(auto-fit, minmax(230px, 1fr));gap:10px;margin-bottom:12px">
                        <div style="background:rgba(0,0,0,.2);padding:8px 10px;border-radius:6px;border:1px solid var(--border)">
                            <label style="display:block;font-size:11px;font-weight:600;color:var(--text);margin-bottom:3px">👑 <b>Icon Tiêu Đề / VIP (Header)</b>:</label>
                            <input type="text" name="tg_emoji_header" value="<?= htmlspecialchars($tgBotCfg['emoji_header'] ?? '') ?>" placeholder="vd: 5467659213675176899" style="width:100%;font-size:12px">
                            <span style="font-size:10px;color:var(--text-dim)">Dùng ở tên Game, Header, Chào mừng</span>
                        </div>

                        <div style="background:rgba(0,0,0,.2);padding:8px 10px;border-radius:6px;border:1px solid var(--border)">
                            <label style="display:block;font-size:11px;font-weight:600;color:var(--text);margin-bottom:3px">✅ <b>Icon Xác Minh Xong (Success)</b>:</label>
                            <input type="text" name="tg_emoji_success" value="<?= htmlspecialchars($tgBotCfg['emoji_success'] ?? '') ?>" placeholder="ID Emoji tích xanh thành công" style="width:100%;font-size:12px">
                            <span style="font-size:10px;color:var(--text-dim)">Dùng khi user đã tham gia đủ nhóm</span>
                        </div>

                        <div style="background:rgba(0,0,0,.2);padding:8px 10px;border-radius:6px;border:1px solid var(--border)">
                            <label style="display:block;font-size:11px;font-weight:600;color:var(--text);margin-bottom:3px">🔑 <b>Icon Mã Key / Game (Key)</b>:</label>
                            <input type="text" name="tg_emoji_key" value="<?= htmlspecialchars($tgBotCfg['emoji_key'] ?? '') ?>" placeholder="ID Emoji chìa khoá / key" style="width:100%;font-size:12px">
                            <span style="font-size:10px;color:var(--text-dim)">Dùng ở dòng hiển thị mã Key kích hoạt</span>
                        </div>

                        <div style="background:rgba(0,0,0,.2);padding:8px 10px;border-radius:6px;border:1px solid var(--border)">
                            <label style="display:block;font-size:11px;font-weight:600;color:var(--text);margin-bottom:3px">⚡ <b>Icon Giao Hàng / Tự Động (Delivery)</b>:</label>
                            <input type="text" name="tg_emoji_delivery" value="<?= htmlspecialchars($tgBotCfg['emoji_delivery'] ?? '') ?>" placeholder="ID Emoji sấm sét / tự động" style="width:100%;font-size:12px">
                            <span style="font-size:10px;color:var(--text-dim)">Dùng ở dòng Giao hàng: Tự động, Thời hạn</span>
                        </div>

                        <div style="background:rgba(0,0,0,.2);padding:8px 10px;border-radius:6px;border:1px solid var(--border)">
                            <label style="display:block;font-size:11px;font-weight:600;color:var(--text);margin-bottom:3px">⚠️ <b>Icon Cảnh Báo / Huỷ Key (Warning)</b>:</label>
                            <input type="text" name="tg_emoji_warning" value="<?= htmlspecialchars($tgBotCfg['emoji_warning'] ?? '') ?>" placeholder="ID Emoji tam giác cảnh báo / cấm" style="width:100%;font-size:12px">
                            <span style="font-size:10px;color:var(--text-dim)">Dùng ở cảnh báo thoát nhóm sẽ xoá key</span>
                        </div>

                        <div style="background:rgba(0,0,0,.2);padding:8px 10px;border-radius:6px;border:1px solid var(--border)">
                            <label style="display:block;font-size:11px;font-weight:600;color:var(--text);margin-bottom:3px">📢 <b>Icon Thông Báo Quản Trị (Bell)</b>:</label>
                            <input type="text" name="tg_emoji_bell" value="<?= htmlspecialchars($tgBotCfg['emoji_bell'] ?? '') ?>" placeholder="ID Emoji loa thông báo / ghim" style="width:100%;font-size:12px">
                            <span style="font-size:10px;color:var(--text-dim)">Dùng ở dòng Thông báo từ quản trị, tin tức</span>
                        </div>
                    </div>

                    <div>
                        <label style="display:block;font-size:11.5px;color:var(--text);margin-bottom:4px">🏷️ <b>Thêm nhiều Icon Custom Emojis khác tùy ý (Mỗi dòng 1 icon: <code>tag: emoji_id</code>):</b></label>
                        <textarea name="tg_custom_emojis" rows="3" placeholder="vd:&#10;fire: 5467659213675176899&#10;gift: 5467659213675176899&#10;crown: 5467659213675176899" style="width:100%;font-family:var(--font-mono);font-size:12px"><?= htmlspecialchars($tgBotCfg['custom_emojis'] ?? '') ?></textarea>
                        <span style="font-size:10.5px;color:var(--text-dim)">Bạn có thể định nghĩa thêm không giới hạn các icon tùy thích. Trong tin nhắn có thể gọi <code>{icon:tag}</code> hoặc hàm <code>telegram_premium_icon('tag')</code>.</span>
                    </div>

                    <!-- KHỐI SAO LƯU & KHÔI PHỤC BỘ ICON TELEGRAM PREMIUM -->
                    <div style="margin-top:14px;padding:13px 15px;background:rgba(0,242,254,.04);border:1px dashed rgba(0,242,254,.32);border-radius:8px">
                        <div style="display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:8px;margin-bottom:8px">
                            <label style="font-size:12px;font-weight:700;color:var(--text);display:flex;align-items:center;gap:6px">
                                <span>📦</span> <b>QUẢN LÝ SAO LƯU &amp; KHÔI PHỤC ICON PREMIUM (BACKUP &amp; RESTORE)</b>
                            </label>
                            <?php $latestIconBk = get_latest_telegram_icons_backup(); ?>
                            <span style="font-size:11px;color:var(--cyan);background:rgba(0,242,254,.1);padding:3px 10px;border-radius:12px;font-family:var(--font-mono)">
                                <?= !empty($latestIconBk['backed_up_date']) ? ('Đã sao lưu: ' . htmlspecialchars($latestIconBk['backed_up_date'])) : 'Chưa có bản sao lưu' ?>
                            </span>
                        </div>
                        <p style="font-size:11px;color:var(--text-dim);margin:0 0 10px;line-height:1.5">
                            Cấu hình Bot &amp; Icon hiện đã được <b>lưu vĩnh viễn trực tiếp vào Database SQLite (bảng bot_settings)</b> và đồng bộ sao lưu Cloud. Khi bạn deploy lại web trên Render, hệ thống sẽ tự động giữ nguyên các cài đặt của bạn mà không bị mất. Bạn cũng có thể bấm <b>"Tải telegram_bot.json về máy"</b> để lưu vào Git repo local!
                        </p>

                        <div style="display:flex;flex-wrap:wrap;gap:8px;align-items:center;margin-bottom:10px">
                            <button type="submit" name="backup_icons_now" value="1" style="background:rgba(0,242,254,.16);color:var(--cyan);border:1px solid rgba(0,242,254,.45);font-size:12px;padding:6px 12px;cursor:pointer;border-radius:6px;font-weight:600">
                                💾 Sao Lưu Icon Hiện Tại
                            </button>
                            <a href="admin.php?download_telegram_bot_config=1" style="background:rgba(59,130,246,.16);color:#60a5fa;border:1px solid rgba(59,130,246,.45);font-size:12px;padding:6px 12px;text-decoration:none;display:inline-flex;align-items:center;gap:4px;border-radius:6px;font-weight:600" title="Tải file cấu hình Bot Telegram để lưu vào thư mục data/ ở máy tính">
                                📥 Tải telegram_bot.json về máy
                            </a>
                            <a href="admin.php?download_telegram_icons_backup=1" style="background:rgba(156,140,255,.16);color:var(--ds-violet);border:1px solid rgba(156,140,255,.45);font-size:12px;padding:6px 12px;text-decoration:none;display:inline-flex;align-items:center;gap:4px;border-radius:6px;font-weight:600">
                                📥 Tải File Backup (.json)
                            </a>
                            <button type="submit" name="restore_latest_icons_backup" value="1" class="warn" style="font-size:12px;padding:6px 12px;cursor:pointer;border-radius:6px" onclick="return confirm('Khôi phục cấu hình icon từ bản sao lưu gần nhất?')">
                                ↺ Khôi Phục Bản Gần Nhất
                            </button>
                            <button type="submit" name="apply_icons_preset" value="1" style="background:rgba(16,185,129,.16);color:var(--green);border:1px solid rgba(16,185,129,.45);font-size:12px;padding:6px 12px;cursor:pointer;border-radius:6px;font-weight:600" onclick="return confirm('Nạp bộ icon mẫu chuẩn Telegram Premium?')">
                                ⭐ Nạp Bộ Icon Mẫu Chuẩn
                            </button>
                        </div>

                        <div style="display:flex;align-items:center;gap:8px;flex-wrap:wrap;background:rgba(0,0,0,.25);padding:8px 10px;border-radius:6px;border:1px solid var(--border)">
                            <label style="font-size:11px;color:var(--text);font-weight:600;white-space:nowrap">📤 Nhập file JSON sao lưu:</label>
                            <input type="file" name="icons_backup_file" accept=".json,application/json" style="font-size:11px;padding:3px;flex:1;min-width:180px">
                            <button type="submit" name="restore_icons_file" value="1" style="font-size:11.5px;padding:5px 12px;background:var(--ds-surface2);border:1px solid var(--border);border-radius:5px;cursor:pointer">
                                Khôi Phục Từ File
                            </button>
                        </div>
                    </div>
                ...
                <div style="display:flex;flex-wrap:wrap;gap:8px;align-items:center;margin-top:6px">
                    <button type="submit" name="save_telegram_bot" value="1">Lưu Cấu Hình</button>
                    <button type="submit" name="register_telegram_webhook" value="1" style="background:#0088cc;color:#fff;border-color:#0088cc">🔗 Kích hoạt Webhook (1-click)</button>
                    <?php if ($tgBotCfg['token'] !== ''): ?>
                        <button type="submit" name="check_telegram_bot" value="1" class="warn">🔍 Kiểm tra kết nối</button>
                        <?php if (!empty($tgBotCfg['admin_chat_id'])): ?>
                            <button type="submit" name="test_admin_telegram_msg" value="1" style="background:rgba(156,140,255,.2);color:var(--ds-violet);border-color:rgba(156,140,255,.4)">📨 Test gửi tin Admin</button>
                            <button type="submit" name="test_start_msg_to_admin" value="1" style="background:rgba(245,158,11,.18);color:#f59e0b;border:1px solid rgba(245,158,11,.45);font-size:12px;padding:6px 12px;border-radius:6px;font-weight:600" title="Gửi thử đúng tin nhắn /start tới Telegram của bạn để kiểm tra Custom Emoji và xem phản hồi">🧪 Test gửi đúng /start đến Admin</button>
                        <?php endif; ?>
                        <button type="submit" name="delete_telegram_webhook" value="1" class="danger small" onclick="return confirm('Huỷ webhook hiện tại của bot?')">Huỷ Webhook</button>
                    <?php endif; ?>
                </div>
            </form>

            <?php
            $recentTgLogs = get_telegram_recent_logs();
            ?>
            <div style="margin-top:14px;padding:12px 14px;background:rgba(0,0,0,.35);border:1px solid var(--border);border-radius:8px">
                <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:8px">
                    <span style="font-size:12px;font-weight:700;color:var(--text);display:flex;align-items:center;gap:6px">
                        📋 <b>NHẬT KÝ TELEGRAM BOT API (XEM LÝ DO LỖI /start HOẶC ICON THƯỜNG)</b>
                    </span>
                    <span style="font-size:10.5px;color:var(--text-dim)"><?= count($recentTgLogs) ?> lượt gần nhất</span>
                </div>
                <div style="max-height:240px;overflow-y:auto;font-family:var(--font-mono);font-size:11px;background:rgba(0,0,0,.45);padding:8px 10px;border-radius:6px;border:1px solid rgba(255,255,255,.07);line-height:1.6">
                    <?php if (empty($recentTgLogs)): ?>
                        <span style="color:var(--text-dim)">Chưa có lượt gọi Telegram API nào được ghi nhận. Hãy gửi /start tới bot hoặc bấm nút "Test gửi đúng /start đến Admin" ở trên để xem log chi tiết!</span>
                    <?php else: ?>
                        <?php foreach ($recentTgLogs as $l): ?>
                            <div style="margin-bottom:8px;padding-bottom:6px;border-bottom:1px solid rgba(255,255,255,.07)">
                                <span style="color:var(--text-dim)">[<?= htmlspecialchars($l['time'] ?? '') ?>]</span>
                                <b style="color:var(--cyan)"><?= htmlspecialchars($l['method'] ?? '') ?></b>
                                &nbsp;chat_id: <code><?= htmlspecialchars($l['chat_id'] ?? '') ?></code> &mdash;
                                <?php if (!empty($l['is_ok'])): ?>
                                    <span style="color:#10b981;font-weight:700">✓ Thành công 200 OK</span>
                                <?php else: ?>
                                    <span style="color:#ef4444;font-weight:700">✗ Thất bại (Mã lỗi: <?= htmlspecialchars((string)($l['error_code'] ?? '')) ?>)</span>
                                <?php endif; ?>
                                <?php if (!empty($l['fallback_error'])): ?>
                                    <div style="color:#f59e0b;margin-top:2px;background:rgba(245,158,11,.1);padding:4px 8px;border-radius:4px;border:1px solid rgba(245,158,11,.25)">
                                        ⚠️ <b>TẠI SAO BỊ RỚT VỀ ICON THƯỜNG:</b> Telegram từ chối Custom Emoji: <code style="color:#fcd34d"><?= htmlspecialchars($l['fallback_error']) ?></code>
                                    </div>
                                <?php endif; ?>
                                <?php if (!empty($l['description']) && empty($l['is_ok'])): ?>
                                    <div style="color:#ef4444;margin-top:2px">
                                        ❌ Phản hồi từ Telegram: <code><?= htmlspecialchars($l['description']) ?></code>
                                    </div>
                                <?php endif; ?>
                                <div style="color:var(--text-dim);font-size:10px;margin-top:2px;word-break:break-all">
                                    Nội dung: <?= htmlspecialchars($l['text_preview'] ?? '') ?>
                                </div>
                            </div>
                        <?php endforeach; ?>
                    <?php endif; ?>
            ...
            <?php if ($tgBotCfg['token'] !== ''): ?>
                <div style="margin-top:10px;font-size:11.5px;color:var(--text-dim);background:rgba(255,255,255,.03);padding:10px 12px;border-radius:6px;border:1px solid var(--border);line-height:1.6">
                    <div>✓ Bot: <b>@<?= htmlspecialchars($tgBotCfg['username'] ?: 'Chưa xác định') ?></b> &nbsp;|&nbsp; Webhook URL: <code><?= htmlspecialchars($webhookTargetUrl) ?></code></div>
                    <div style="margin-top:6px;color:var(--cyan);font-size:11px">
                        💡 <b>Tính năng tự động:</b> Khi user vượt link nhận key, Bot sẽ tự động gửi mã Key về tài khoản Telegram của user. Nếu user thoát khỏi nhóm Telegram yêu cầu, hệ thống sẽ tự động <b>XOÁ KEY</b> ngay lập tức!
                    </div>
                </div>
            <?php endif; ?>

            <?php if (!empty($tgBotStatusInfo)): ?>
                <div style="margin-top:10px;padding:10px 14px;background:rgba(0,0,0,.3);border-radius:6px;border:1px solid rgba(89,245,213,.35);font-size:12px;line-height:1.6">
                    <b style="color:var(--text)">Kết quả kiểm tra Bot &amp; Webhook:</b>
                    <?php if (!empty($tgBotStatusInfo['me']['ok'])): ?>
                        <div style="color:var(--green);margin-top:4px">✓ Kết nối Bot thành công: <b><?= htmlspecialchars($tgBotStatusInfo['me']['result']['first_name'] ?? '') ?></b> (@<?= htmlspecialchars($tgBotStatusInfo['me']['result']['username'] ?? '') ?>) &mdash; Bot ID: <code><?= htmlspecialchars((string)($tgBotStatusInfo['me']['result']['id'] ?? '')) ?></code></div>
                    <?php else: ?>
                        <div style="color:var(--red);margin-top:4px">❌ <?= htmlspecialchars(telegram_error_message($tgBotStatusInfo['me'])) ?></div>
                    <?php endif; ?>

                    <?php if (!empty($tgBotStatusInfo['hook']['ok'])): ?>
                        <?php $wh = $tgBotStatusInfo['hook']['result']; ?>
                        <div style="margin-top:4px">
                            Trạng thái Webhook: <?= !empty($wh['url']) ? '<span style="color:var(--green)">✓ Đang trỏ tới: <code>' . htmlspecialchars($wh['url']) . '</code></span>' : '<span style="color:var(--red)">❌ Chưa đăng ký webhook trên Telegram</span>' ?>
                            <?php if (!empty($wh['last_error_message'])): ?>
                                <div style="color:var(--red);margin-top:2px">⚠️ Lỗi gửi webhook gần nhất (<?= !empty($wh['last_error_date']) ? date('H:i:s d/m/Y', $wh['last_error_date']) : '' ?>): <?= htmlspecialchars($wh['last_error_message']) ?></div>
                            <?php endif; ?>
                            <?php if (isset($wh['pending_update_count'])): ?>
                                <div style="color:var(--text-dim);font-size:11px;margin-top:2px">Tin nhắn chờ Telegram chuyển phát: <?= (int)$wh['pending_update_count'] ?></div>
                            <?php endif; ?>
                        </div>
                    <?php endif; ?>
                </div>
        ...
        <?php
            $allUserIds = get_all_telegram_user_ids();
            $totalUsersCount = count($allUserIds);
        ?>
        <div class="region-form" style="margin-top:17px;border-color:rgba(156,140,255,.35);background:rgba(156,140,255,.03)">
            <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:8px;flex-wrap:wrap;gap:8px">
                <h4 style="margin:0;color:var(--text);display:flex;align-items:center;gap:8px">
                    <span style="font-size:16px">📢</span> <b>Gửi Thông Báo Đến Toàn Bộ Người Dùng (Broadcast All Users)</b>
                </h4>
                <span style="font-size:11.5px;color:var(--cyan);background:rgba(89,245,213,.1);padding:3px 10px;border-radius:12px;border:1px solid rgba(89,245,213,.25)">
                    👥 Tổng: <b><?= $totalUsersCount ?></b> người dùng đã dùng bot
                </span>
            </div>
            <p style="font-size:11.5px;color:var(--text-dim);margin:0 0 14px;line-height:1.5">
                Bot sẽ tự động gửi thông báo trực tiếp vào tin nhắn Telegram riêng của tất cả người dùng trong hệ thống (đã từng mở bot hoặc lấy key). Bạn có thể nhập <b>ID Icon Telegram Premium</b> để làm nổi bật thông báo bằng Custom Emoji động.
            </p>
            ...
            <form method="post" style="display:flex;flex-direction:column;gap:11px">
                ...
                <div style="display:flex;flex-wrap:wrap;gap:10px">
                    <div style="flex:2;min-width:260px">
                        <label style="display:block;font-size:11.5px;font-weight:600;color:var(--text);margin-bottom:4px">📌 <b>Tiêu đề thông báo:</b></label>
                        <input type="text" name="broadcast_title" placeholder="vd: THÔNG BÁO BẢO TRÌ &amp; CẬP NHẬT MỚI" style="width:100%" value="THÔNG BÁO TỪ BAN QUẢN TRỊ">
                    </div>
                    <div style="flex:1;min-width:200px">
                        <label style="display:block;font-size:11.5px;font-weight:600;color:var(--text);margin-bottom:4px">⭐ <b>ID Icon Premium (Tùy chọn):</b></label>
                        <input type="text" name="broadcast_icon_id" placeholder="vd: 5467659213675176899" style="width:100%" value="<?= htmlspecialchars($tgBotCfg['premium_emoji_id'] ?? '') ?>">
                        <span style="font-size:10px;color:var(--text-dim)">Để trống sẽ dùng icon mặc định của bot</span>
                    </div>
                </div>

                <div>
                    <label style="display:block;font-size:11.5px;font-weight:600;color:var(--text);margin-bottom:4px">📝 <b>Nội dung thông báo gửi cho người dùng:</b></label>
                    <textarea name="broadcast_content" required rows="5" placeholder="Nhập nội dung thông báo...&#10;Hỗ trợ chữ đậm &lt;b&gt;, nghiêng &lt;i&gt;, code &lt;code&gt; hoặc các icon Telegram." style="width:100%;padding:10px 12px;border-radius:8px;border:1px solid var(--border);background:rgba(0,0,0,.25);color:var(--text);font:13px 'Inter',sans-serif;line-height:1.5;resize:vertical"></textarea>
                    <span style="font-size:10.5px;color:var(--text-dim)">💡 Bot sẽ tự động đóng khung quote đẹp mắt và gắn kèm thời gian phát thông báo.</span>
                </div>

                <div style="display:flex;flex-wrap:wrap;gap:10px;background:rgba(0,0,0,.15);padding:10px 12px;border-radius:6px;border:1px solid var(--border)">
                    <div style="flex:1;min-width:220px">
                        <label style="display:block;font-size:11px;font-weight:600;color:var(--text-dim);margin-bottom:3px">🔘 Chữ trên nút bấm (Tùy chọn):</label>
                        <input type="text" name="broadcast_btn_text" placeholder="vd: 🌐 Vào Trang Web Lấy Key 🚀" style="width:100%;font-size:12px" value="🌐 Mở Trang Web HoQuocKey 🚀">
                    </div>
                    <div style="flex:1;min-width:260px">
                        <label style="display:block;font-size:11px;font-weight:600;color:var(--text-dim);margin-bottom:3px">🔗 Đường link nút bấm (URL):</label>
                        <input type="url" name="broadcast_btn_url" placeholder="https://..." style="width:100%;font-size:12px" value="<?= htmlspecialchars(rtrim(BASE_URL, '/') . '/') ?>">
                    </div>
                </div>

                <div style="display:flex;flex-wrap:wrap;gap:8px;align-items:center;margin-top:4px">
                    <?php if (!empty($tgBotCfg['admin_chat_id'])): ?>
                        <button type="submit" name="test_broadcast_telegram" value="1" class="warn" style="background:rgba(245,158,11,.12);color:#f59e0b;border-color:rgba(245,158,11,.35)">🧪 Gửi Thử Cho Admin Trước</button>
                    <?php endif; ?>
                    <button type="submit" name="send_broadcast_telegram" value="1" style="background:#7c3aed;color:#fff;border-color:#7c3aed" onclick="return confirm('Bạn có chắc chắn muốn phát tin nhắn thông báo này tới TẤT CẢ <?= $totalUsersCount ?> người dùng bot không?')">📢 Phát Thông Báo Đến Tất Cả User (Broadcast)</button>
                </div>
            </form>
        </div>
        ...
        <div class="region-form" style="margin-top:17px">
            ...
            <form method="post">
                ...
                <input type="url" name="channel_url" required maxlength="500" placeholder="https://t.me/yourgroup (link hiển thị, không phải deep link)" style="width:min(100%,360px)"><br>
                <input type="text" name="channel_requirement" maxlength="150" placeholder="Yêu cầu (vd: Đăng ký kênh để ủng hộ)" style="width:min(100%,420px)"><br>
                <input type="text" name="channel_tg_chat_id" maxlength="40" placeholder="Telegram Chat ID (để trống = không xác minh thật, vd: -1001234567890)" style="width:min(100%,420px)">
            ...
            <p style="font-size:11px;color:var(--text-dim);margin:8px 2px 0">Điền <b>Chat ID</b> chỉ áp dụng cho kênh loại Telegram cần xác minh THẬT qua Bot (user phải thực sự ở trong nhóm mới được tính, không chỉ bấm link rồi chờ). Bỏ trống nếu chỉ cần bấm+chờ như các kênh khác.</p>
        </div>

        <div style="margin-top:21px" class="tablewrap">
            <table>
                <tr><th>Thứ tự</th><th>Kênh</th><th>Yêu cầu</th><th>Xác minh</th><th>Trạng thái</th><th></th></tr>
                ...
                <tr>
                    ...
                    <td><?php if (!empty($channel['tg_chat_id'])): ?><span class="badge on">Bot: <?= htmlspecialchars($channel['tg_chat_id']) ?></span><?php else: ?><span style="font-size:11px;color:var(--text-dim)">Bấm + chờ</span><?php endif; ?></td>
                ...
                <?php endforeach; ?>
                <?php if (empty($channelRows)): ?><tr><td colspan="6" style="color:var(--text-dim);text-align:center;padding:24px">Chưa có kênh nào. Nếu chưa thêm kênh, user sẽ đi thẳng vào bước tạo link.</td></tr><?php endif; ?>
            </table>
        </div>
    </div>
    ...
    <div class="box">
        ...
        <form method="post"><input type="hidden" name="csrf_token" value="<?= htmlspecialchars($_SESSION['csrf_token'] ?? '') ?>">
            ...
            <div style="font-size:12px;color:#888;margin-top:6px">Thời hạn chỉ bắt đầu tính từ lúc key được dùng lần đầu trong app. Tối đa 200 key/lần.</div>
        </form>
    </div>
    ...
    <div class="box">
        ...
        <form method="get" style="margin-bottom:10px">
            ...
            <select name="status">
                ...
                <option value="active_unused" <?= $filterStatus==='active_unused'?'selected':'' ?>>Active — chưa dùng trong app</option>
        ...
        <form method="post" style="margin-bottom:10px;display:flex;align-items:center;gap:6px;flex-wrap:wrap">
            <input type="hidden" name="csrf_token" value="<?= htmlspecialchars($_SESSION['csrf_token'] ?? '') ?>">
            <button type="submit" name="cleanup_unused_active" value="1" class="danger small" onclick="return confirm('Xoá TOÀN BỘ key đang active nhưng CHƯA TỪNG dùng trong app (quá số giờ chọn bên dưới)? Không thể hoàn tác.')">Xoá key Active chưa dùng</button>
            <span style="font-size:12px;color:var(--text-dim)">quá</span>
            <input type="number" name="unused_min_hours" value="1" min="0" step="0.5" style="width:64px;margin:0">
            <span style="font-size:12px;color:var(--text-dim)">giờ chưa mở app</span>
            <?php if (isset($cleanupUnusedActiveCount)): ?><span style="font-size:12px;color:var(--text-dim);margin-left:4px">— Đã xoá <?= $cleanupUnusedActiveCount ?> key</span><?php endif; ?>
        </form>
        <div class="tablewrap">
        <table>
            <tr><th>Key</th><th>Trạng thái</th><th>Thời hạn</th><th>Telegram User</th><th>Thiết bị tạo</th><th>Số thiết bị dùng</th><th>Bắt đầu dùng</th><th>Hết hạn</th><th></th></tr>
            ...
            <tr>
                ...
                <td>
                    ...
                    <?php if ($status === 'active' && !empty($k['activated_at'])): ?>
                        <div style="font-size:10.5px;color:var(--text-dim);margin-top:3px" title="Thời điểm kích hoạt"><?= fmt_time($k['activated_at']) ?></div>
                    <?php elseif ($status === 'pending'): ?>
                        <div style="font-size:10.5px;color:var(--text-dim);margin-top:3px" title="Tiến độ vượt link">Bước <?= (int)($k['current_hop'] ?? 0) ?>/<?= (int)($k['total_hops'] ?? 1) ?></div>
                    <?php endif; ?>
                </td>
                <td><?= fmt_duration((int)$k['duration_seconds']) ?></td>
                <td>
                    <?php if (!empty($k['tg_user_id'])):
                        $uInfo = get_telegram_user_info((int)$k['tg_user_id']);
                        $isPrem = !empty($k['is_premium']) || !empty($uInfo['is_premium']);
                        $uNameStr = !empty($uInfo['username']) ? ('@' . $uInfo['username']) : (!empty($k['tg_username']) ? ('@' . $k['tg_username']) : '');
                    ?>
                        <div style="display:inline-flex;align-items:center;gap:5px;flex-wrap:wrap">
                            <span style="display:inline-flex;align-items:center;gap:4px;font-size:11px;background:rgba(0,136,204,.15);color:#0088cc;padding:2px 7px;border-radius:4px;border:1px solid rgba(0,136,204,.3)" title="Telegram ID: <?= htmlspecialchars((string)$k['tg_user_id']) ?><?= $uNameStr ? (' (' . htmlspecialchars($uNameStr) . ')') : '' ?>">
                                ✈️ <?= htmlspecialchars((string)$k['tg_user_id']) ?><?= $uNameStr ? (' <span style="font-size:10px;opacity:.85">' . htmlspecialchars($uNameStr) . '</span>') : '' ?>
                            </span>
                            <?php if ($isPrem): ?>
                                <?= telegram_premium_badge_html('Premium') ?>
                            <?php endif; ?>
                        </div>
                    <?php else: ?>
                ...
                <td><?= htmlspecialchars($k['client_platform'] ?: '?') ?></td>
                <td>
