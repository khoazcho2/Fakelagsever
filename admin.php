...
<?php else: ?>
    ...
    <div class="box" style="<?= $serverStatus['closed'] ? 'border:1px solid var(--danger);background:rgba(255,107,107,.06)' : 'border:1px solid var(--success);background:rgba(52,211,153,.06)' ?>">
        <h3><?= svg_icon('shield', 17) ?> Trạng thái Server (API app)</h3>
        <?php if ($serverStatus['closed']): ?>
            <div style="display:inline-flex;align-items:center;gap:6px;background:rgba(255,107,107,.15);color:var(--danger);font-weight:700;font-size:13px;padding:6px 14px;border-radius:20px;margin-bottom:6px">
                <span style="width:8px;height:8px;border-radius:50%;background:var(--danger)"></span> SERVER ĐANG ĐÓNG
            </div>
            <p style="font-size:12.5px;color:#888">App Android nhập key sẽ không nhận được phản hồi gì (giống server treo).</p>
            <?php if (!empty($serverStatus['reason'])): ?><p style="font-size:12px;color:#888">Lý do: <?= htmlspecialchars($serverStatus['reason']) ?></p><?php endif; ?>
            <?php if (!empty($serverStatus['changed_at'])): ?><p style="font-size:12px;color:#888">Đóng lúc: <?= fmt_time((int)$serverStatus['changed_at']) ?></p><?php endif; ?>
            <form method="post" style="margin-top:8px">
                <button type="submit" name="open_server" value="1" style="background:var(--success);color:#06170D">Mở lại Server</button>
            </form>
        <?php else: ?>
            <div style="display:inline-flex;align-items:center;gap:6px;background:rgba(52,211,153,.15);color:var(--success);font-weight:700;font-size:13px;padding:6px 14px;border-radius:20px;margin-bottom:6px">
                <span style="width:8px;height:8px;border-radius:50%;background:var(--success);box-shadow:0 0 6px var(--success)"></span> SERVER ĐANG ACTIVE
            </div>
            <form method="post" style="margin-top:10px">
                <input type="text" name="close_reason" placeholder="Lý do đóng (tuỳ chọn, chỉ admin thấy)" style="width:260px">
                <button type="submit" name="close_server" value="1" class="danger" onclick="return confirm('Đóng server ngay? App Android nhập key sẽ không nhận được phản hồi gì cho tới khi bạn mở lại.')">🔴 Đóng Server ngay</button>
            </form>
        <?php endif; ?>
    </div>
