...
function render_tg_membership_content(int $userId, string $firstName, string $payload = ''): array {
    ...
    // FIX: lấy icon qua telegram_premium_icon() thay vì hardcode emoji Unicode thẳng
    // trong string — đây là lý do trước đó icon Premium không áp dụng cho tin nhắn
    // xác thực (chỉ tin Test trong admin.php gọi đúng hàm này).
    $icHeader = telegram_premium_icon('header', '⚡');
    $icBell   = telegram_premium_icon('bell', '👋');
    $icSucc   = telegram_premium_icon('success', '✅');
    $icDeliv  = telegram_premium_icon('delivery', '📦');
    $icWarn   = telegram_premium_icon('warning', '⚠️');
    $icPoint  = telegram_premium_icon('bell', '👉');
    ...
    foreach ($groups as $idx => $g) {
        ...
        if (!$isJoined) {
            $missingCount++;
            $statusLines[] = "🔺 <b>" . htmlspecialchars($g['name']) . "</b>: CHƯA THAM GIA";
        } else {
            $statusLines[] = $icSucc . " <b>" . htmlspecialchars($g['name']) . "</b>: ĐÃ THAM GIA";
        }
        $groupButtons[] = [
    ...
    if ($allJoined) {
        ...
        $text = "{$icHeader} <b>XÁC THỰC THÀNH VIÊN THÀNH CÔNG!</b>\n\n" .
                "{$icBell} Chào <b>{$escapedName}</b>,\n" .
                "{$icSucc} Bạn đã tham gia đầy đủ tất cả các nhóm bắt buộc:\n\n" .
                implode("\n", $statusLines) . "\n\n" .
                "{$icDeliv} <b>Giao hàng:</b> Tự động sau khi bot kiểm tra đủ\n" .
                "{$icWarn} <b>Quy định:</b> Thoát nhóm sau khi nhận key sẽ bị huỷ key tự động!\n\n" .
        ...
        $text = "{$icHeader} <b>XÁC THỰC THÀNH VIÊN BOT TELEGRAM</b>\n\n" .
                "{$icBell} Chào <b>{$escapedName}</b>,\n" .
                "{$icWarn} Bạn chưa tham gia đủ {$totalGroups} nhóm bắt buộc:\n\n" .
                implode("\n", $statusLines) . "\n\n" .
                "{$icDeliv} <b>Giao hàng:</b> Tự động sau khi bot kiểm tra đủ\n" .
                "{$icWarn} <b>Quy định:</b> Thoát nhóm sau khi nhận key sẽ bị huỷ key tự động!\n\n" .
