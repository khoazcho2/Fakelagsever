...
// Endpoint kiểm tra trực tiếp theo @username hoặc ID Telegram từ form trên web
if (isset($_REQUEST['direct_tg_verify'])) {
    ...
    if (ctype_digit($cleanId)) {
        ...
        // Tìm ID số từ username trong bảng tg_users
        $stmtU = $db->prepare("SELECT tg_user_id, is_premium FROM tg_users WHERE username = ? LIMIT 1");
        $stmtU->execute([strtolower($cleanId)]);
    ...
    if ($tgUserId <= 0) {
        echo json_encode([
            'success' => false,
            'need_id' => true,
            'message' => "Chưa tìm thấy @{$cleanId} trong hệ thống. Bạn vui lòng nhập ID số (chat @userinfobot trên Telegram để lấy) hoặc bấm nút 'Vào Bot Xác Minh' bên trên để bot tự ghi nhận nhé."
        ]);
        exit;
    }
    ...
    foreach ($channels as $ch) {
        if ($ch['type'] === 'telegram' && !empty($ch['tg_chat_id'])) {
            ...
            if (!in_array($cId, $reqGroups, true)) {
                $reqGroups[] = $cId;
            }
        }
    }
    ...
    try {
        $db->prepare("UPDATE keys SET tg_user_id = ?, is_premium = ? WHERE token = ?")->execute([$tgUserId, $isUserPremium ? 1 : 0, $reqToken]);
    } catch (\Throwable $e) {}
...
function render_channel_gate(array $game, array $channels, string $token, string $gateToken, ?array $notice, string $error = '', array $openedChannels = [], bool $isAdmin = false, string $rParam = ''): void {
    ...
    function onBotDirectStartClick(elem) {
        ...
        const globalPoll = setInterval(() => {
            fetch('getkey.php?channel_verify_poll=1&token=' + encodeURIComponent(openToken) + '&gate_token=' + encodeURIComponent(gateToken) + '&channel=0', { credentials: 'same-origin', cache: 'no-store' })
                .then(r => r.json()).then(data => {
                    if (data.verified) {
    ...
    function runDirectTgVerify() {
        ...
        fetch('getkey.php?direct_tg_verify=1&token=' + encodeURIComponent(openToken) + '&gate_token=' + encodeURIComponent(gateToken) + '&identifier=' + encodeURIComponent(val), {
