...
if (isset($_SESSION['is_admin'])) {
    ...
    // Xoá hàng loạt tất cả key đã hết hạn (dọn dẹp)
    if (isset($_POST['cleanup_expired'])) {
        $now = time();
        db_execute($db->prepare("DELETE FROM keys WHERE status = 'expired' OR (status = 'active' AND expires_at IS NOT NULL AND expires_at < ?)"), [$now]);
    }
...
if (isset($_SESSION['is_admin'])) {
    ...
    $params = [];
    if ($searchQ !== '') {
        $sql .= " AND REPLACE(UPPER(keycode), '-', '') LIKE ?";
...
<?php else: ?>
    ...
    <div class="box">
        ...
        <div class="stats-grid">
            <div class="stat-card"><span class="stat-ic"><?= svg_icon('key', 20) ?></span><div class="stat-num"><?= $stats['total_keys'] ?></div><div class="stat-label">Tổng số key</div></div>
            <div class="stat-card"><span class="stat-ic" style="color:var(--success)"><?= svg_icon('check-circle', 20) ?></span><div class="stat-num"><?= $stats['active_keys'] ?></div><div class="stat-label">Đang hoạt động</div></div>
            <div class="stat-card"><span class="stat-ic" style="color:var(--warn)"><?= svg_icon('warning', 20) ?></span><div class="stat-num"><?= $stats['pending_keys'] ?></div><div class="stat-label">Chưa kích hoạt</div></div>
            <div class="stat-card"><span class="stat-ic" style="color:var(--danger)"><?= svg_icon('x-circle', 20) ?></span><div class="stat-num"><?= $stats['expired_keys'] ?></div><div class="stat-label">Đã hết hạn</div></div>
    ...
    <div class="box" style="text-align:center;padding:32px 20px">
        ...
        <form method="post" onsubmit="return confirm(<?= $closed ? "'Mở lại server?'" : "'Đóng server ngay? App Android nhập key sẽ không nhận được phản hồi gì cho tới khi bạn mở lại.'" ?><input type="hidden" name="csrf_token" value="<?= htmlspecialchars($_SESSION['csrf_token'] ?? '') ?>">)">
            <?php if ($closed): ?>
    ...
    <div class="box">
        ...
        <form method="post" style="margin-bottom:10px"><input type="hidden" name="csrf_token" value="<?= htmlspecialchars($_SESSION['csrf_token'] ?? '') ?>">
            <button type="submit" name="cleanup_expired" value="1" class="warn small" onclick="return confirm('Xoá toàn bộ key đã hết hạn? Không thể hoàn tác.')">Dọn dẹp key hết hạn</button>
        </form>
        <div class="tablewrap">
        <table>
