...
if (isset($_SESSION['is_admin'])) {
    ...
    if (isset($_POST['test_admin_telegram_msg'])) {
        ...
        if ($botToken === '' || $adminChatId === '') {
            ...
            if (!empty($res['ok'])) {
                $channelSaved = true;
                $tgBotFeedback = '✅ Đã gửi tin nhắn test thành công tới ID Chat: ' . htmlspecialchars($adminChatId);
            } else {
            ...
            <form method="post" enctype="multipart/form-data" style="display:flex;flex-direction:column;gap:10px">
                ...
                <div style="margin-top:10px;padding:14px 16px;background:rgba(156,140,255,.06);border:1px solid rgba(156,140,255,.25);border-radius:8px">
                    ...
                    <p style="font-size:11.5px;color:var(--text-dim);margin:0 0 12px;line-height:1.5">
                        Bạn có thể tùy thích gán các ID Icon Telegram Premium khác nhau cho từng loại nội dung bot gửi (Tiêu đề, Tích xanh duyệt, Mã Key, Giao hàng tự động, Cảnh báo huỷ key...). Nếu ô nào để trống, bot sẽ tự động lấy <b>ID Icon Mặc Định</b>.
                    </p>

                    <div style="margin-bottom:12px">
                        <label style="display:block;font-size:11.5px;color:var(--text);margin-bottom:4px">⭐ <b>ID Icon Mặc Định (Chung cho toàn bộ bot nếu chưa đặt riêng):</b></label>
                        <input type="text" name="tg_premium_emoji_id" value="<?= htmlspecialchars($tgBotCfg['premium_emoji_id'] ?? '5467659213675176899') ?>" placeholder="vd: 5467659213675176899" style="width:100%;border-color:rgba(156,140,255,.4)">
                    </div>
