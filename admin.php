...
if (isset($_SESSION['is_admin'])) {
    ...
    if (isset($_POST['save_telegram_bot'])) {
        save_telegram_bot_config(trim($_POST['tg_bot_token'] ?? ''), trim($_POST['tg_bot_username'] ?? ''));
        $channelSaved = true;
    }
...
<?php else: ?>
    ...
    <div class="box">
        ...
        <?php $tgBotCfg = get_telegram_bot_config(); ?>
        <div class="region-form" style="margin-top:17px;border-color:rgba(89,245,213,.22)">
            ...
            <p style="font-size:11.5px;color:var(--text-dim);margin:0 0 10px;max-width:600px">Tạo bot qua @BotFather, thêm bot làm <b>admin</b> của nhóm cần xác minh, rồi nhập Token + Username tại đây. Sau đó đăng ký webhook 1 lần bằng cách mở: <code style="font-size:10.5px">https://api.telegram.org/bot&lt;TOKEN&gt;/setWebhook?url=https://&lt;domain-cua-ban&gt;/telegram_webhook.php</code></p>
            <form method="post">
                <input type="hidden" name="csrf_token" value="<?= htmlspecialchars($_SESSION['csrf_token'] ?? '') ?>">
                <input type="text" name="tg_bot_token" value="<?= htmlspecialchars($tgBotCfg['token']) ?>" placeholder="Bot Token (từ @BotFather)" style="width:min(100%,340px)">
                <input type="text" name="tg_bot_username" value="<?= htmlspecialchars($tgBotCfg['username']) ?>" placeholder="Bot Username (không cần @)" style="width:220px">
                <button type="submit" name="save_telegram_bot" value="1">Lưu Bot</button>
            </form>
            <?php if ($tgBotCfg['token'] !== ''): ?><p style="font-size:11px;color:var(--text-dim);margin:8px 0 0">✓ Đã cấu hình bot: @<?= htmlspecialchars($tgBotCfg['username'] ?: '?') ?></p><?php endif; ?>
        </div>
