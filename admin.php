...
if (isset($_SESSION['is_admin'])) {
    ...
    if (isset($_POST['restore_backup']) && isset($_FILES['backup_file']) && $_FILES['backup_file']['error'] === UPLOAD_ERR_OK) {
        ...
        if (!$backup) {
            ...
            $n = count($backup['games'] ?? []) + count($backup['keys'] ?? []) + count($backup['ip_blocklist'] ?? []) + count($backup['device_history'] ?? []);
...
body{
    position:relative;max-width:1180px;padding:0 30px 60px;
    background:radial-gradient(circle at 8% -10%,rgba(89,245,213,.12),transparent 32rem),radial-gradient(circle at 94% 10%,rgba(156,140,255,.1),transparent 30rem),var(--bg);
}
body::before{
    ...
    <div class="box">
        ...
        <table>
            ...
            <?php
                ...
                $historyRows = search_device_history($historyQ, 100);
            ?>
        ...
        <p style="font-size:11px;color:#888;margin-top:6px">Hiện tối đa 100 dòng gần nhất.</p>
    </div>
