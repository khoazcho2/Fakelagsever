...
// - User được chọn loại key ngay ở màn "Yêu cầu nhiệm vụ": Key 24h
//   (vượt 1 lần) hoặc Key 36h (vượt 2 lần). Lựa chọn này chỉ áp dụng
//   khi có cấu hình bắt buộc kênh (channel gate); key vẫn được tạo
//   với số bước mặc định trước, và được CẬP NHẬT lại total_hops /
//   duration_seconds / chain ngay khi user xác nhận xong nhiệm vụ.
// ============================================================
...
// Định nghĩa các loại key user có thể chọn ở màn nhiệm vụ: value => [hops, hours, label]
const KEY_TYPE_OPTIONS = [
    '24h' => ['hops' => 1, 'hours' => 24, 'label' => 'Key 24 giờ', 'sub' => 'Vượt 1 lần link'],
    '36h' => ['hops' => 2, 'hours' => 36, 'label' => 'Key 36 giờ', 'sub' => 'Vượt 2 lần link'],
];

function normalize_key_type(?string $value): string {
    return isset($value, KEY_TYPE_OPTIONS[$value]) ? $value : '24h';
}
...
if (isset($_POST['channel_gate'])) {
    ...
    sort($doneIds);
    sort($requiredIds);
    $selectedKeyType = normalize_key_type($_POST['key_type'] ?? null);
    ...
    if ($doneIds !== $requiredIds) {
        render_channel_gate($game, $channels, $pendingToken, $expectedGate, $publicNotice, 'Hãy mở và xác nhận đầy đủ tất cả kênh trước khi tiếp tục.', $_SESSION['channel_opened'][$pendingToken] ?? [], $selectedKeyType);
        exit;
    }
    ...
    if (!empty($notReady)) {
        render_channel_gate($game, $channels, $pendingToken, $expectedGate, $publicNotice, 'Vui lòng bấm Tham Gia và đợi đủ ' . CHANNEL_WAIT_SECONDS . ' giây cho từng kênh.', $openedChannels, $selectedKeyType);
        exit;
    }

    // Áp dụng loại key user vừa chọn (24h/1 bước hoặc 36h/2 bước) ngay
    // trước khi chuyển qua hop.php - ghi đè lại total_hops/duration/chain
    // của key pending đã tạo sẵn trước đó.
    $typeInfo = KEY_TYPE_OPTIONS[$selectedKeyType];
    $cfgForType = get_shortener_config();
    $newChain = implode(',', array_fill(0, $typeInfo['hops'], $cfgForType['active'] ?? ''));
    $db->prepare("UPDATE keys SET total_hops = ?, duration_seconds = ?, chain = ? WHERE token = ? AND status = 'pending'")
        ->execute([$typeInfo['hops'], $typeInfo['hours'] * 3600, $newChain, $pendingToken]);
...
if (!empty($_COOKIE[$pendingCookie])) {
    ...
    if ($existing) {
        ...
        if (!empty($channels) && $gateStale) {
            ...
            render_channel_gate($game, $channels, $existing['token'], $gateToken, $publicNotice, '', $_SESSION['channel_opened'][$existing['token']] ?? []);
            exit;
        }
...
// Tạo key với số bước/thời hạn MẶC ĐỊNH trước (theo cấu hình game). Nếu
// có cổng nhiệm vụ kênh, giá trị này sẽ được GHI ĐÈ lại theo lựa chọn
// 24h/36h của user ngay khi họ xác nhận xong nhiệm vụ (xem khối xử lý
// $_POST['channel_gate'] phía trên).
...
function render_channel_gate(array $game, array $channels, string $token, string $gateToken, ?array $notice, string $error = '', array $openedChannels = [], string $selectedKeyType = '24h'): void {
    $typeIcons = ['youtube' => '▶', 'tiktok' => '♪', 'telegram' => '✈', 'facebook' => 'f', 'discord' => '◉', 'instagram' => '◎', 'other' => '↗'];
    $wait = CHANNEL_WAIT_SECONDS;
    $selectedKeyType = normalize_key_type($selectedKeyType);
    ?>
    <!DOCTYPE html><html lang="vi"><head><meta charset="UTF-8">
    ...
    *{box-sizing:border-box}body{min-height:100vh;margin:0;padding:24px 16px;font-family:"Inter",-apple-system,Arial,sans-serif;color:var(--text);background:radial-gradient(circle at 12% 0%,rgba(89,245,213,.16),transparent 28rem),radial-gradient(circle at 90% 12%,rgba(156,140,255,.13),transparent 26rem),var(--bg)}.wrap{width:min(100%,520px);margin:0 auto}.brand{text-align:center;margin:7px 0 17px;color:#A9B6C8;font:10px "JetBrains Mono",monospace;letter-spacing:.16em}.brand b{color:var(--cyan)}.card{overflow:hidden;border:1px solid var(--line);border-radius:24px;background:linear-gradient(145deg,rgba(22,31,44,.97),rgba(12,17,26,.99));box-shadow:0 30px 80px -30px #000,inset 0 1px rgba(255,255,255,.07)}.holo{height:4px;background:linear-gradient(90deg,var(--cyan),var(--violet),#F0A6FF,var(--cyan));background-size:240% 100%;animation:shift 3s linear infinite}.content{padding:26px}.eyebrow{font:10px "JetBrains Mono",monospace;letter-spacing:.16em;color:var(--cyan)}h1{font:700 24px "Space Grotesk",sans-serif;letter-spacing:-.03em;margin:8px 0 7px}p{margin:0;color:var(--dim);font-size:13px;line-height:1.6}.game{display:inline-flex;align-items:center;gap:7px;margin-top:15px;padding:7px 10px;border-radius:999px;background:rgba(156,140,255,.1);color:#CAC3FF;font-size:11px}
    ...
    <?php foreach (KEY_TYPE_OPTIONS as $value => $opt): ?>
        <label><input type="radio" name="key_type" value="<?= htmlspecialchars($value) ?>" <?= $value === $selectedKeyType ? 'checked' : '' ?>><span class="kt-title"><?= htmlspecialchars($opt['label']) ?></span><span class="kt-sub"><?= htmlspecialchars($opt['sub']) ?></span></label>
    <?php endforeach; ?>
    </div>
    <?php foreach ($channels as $channel):
        ...
        $isDone = $elapsed !== null && $elapsed >= $wait;
        $remaining = $elapsed !== null ? max(0, $wait - $elapsed) : null;
