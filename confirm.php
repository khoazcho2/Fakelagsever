...
// Lấy hoặc đồng bộ tg_user_id cho key này (nếu trước đó lưu trong channel_verifications)
...
if ($tgUserId <= 0) {
    try {
        ...
        db_execute($stmtTg, [$token]);
        $foundTg = (int)$stmtTg->fetchColumn();
        if ($foundTg > 0) {
            $tgUserId = $foundTg;
            db_execute($db->prepare("UPDATE keys SET tg_user_id = ? WHERE id = ?"), [$tgUserId, $row['id']]);
            $row['tg_user_id'] = $tgUserId;
        }
    } catch (\Throwable $e) {}
}

// KIỂM TRA CHỦ ĐỘNG: Người dùng còn ở trong toàn bộ các nhóm Telegram bắt buộc không?
// Nếu đã thoát nhóm: XOÁ KEY NGAY LẬP TỨC và chặn xem key!
if ($tgUserId > 0) {
    $membershipCheck = check_user_membership_all_required_groups($tgUserId);
    if (!empty($membershipCheck['errors'])) {
        http_response_code(503);
        render_notice_screen('Tạm thời chưa thể xác minh Telegram', $membershipCheck['errors'][0] . ' Key của bạn được giữ nguyên.');
        exit;
    }
    if (!$membershipCheck['all_joined']) {
        revoke_user_keys_on_telegram_leave($tgUserId, "Rời khỏi nhóm: " . implode(', ', $membershipCheck['missing']));
        http_response_code(403);
        render_notice_screen(
            'Key đã bị xoá!',
            'Hệ thống phát hiện tài khoản Telegram của bạn đã rời khỏi nhóm: ' . implode(', ', $membershipCheck['missing']) . '. Theo quy định, Key của bạn đã bị xoá vĩnh viễn và vô hiệu hoá ngay lập tức!'
        );
        exit;
    }
}
...
if ($row['status'] === 'pending') {
    ...
    $activateOk = db_execute($upd, [time(), $row['id']]);

    if (!$activateOk) {
        // QUAN TRỌNG: nếu ghi DB thất bại thật sự (hết số lần retry của
        // db_execute do server đang quá tải), TUYỆT ĐỐI không được hiển
        // thị key như đã kích hoạt - trước đây code cứ chạy tiếp bất kể
        // kết quả, khiến user thấy "ĐÃ KÍCH HOẠT" + xem được key trong
        // khi DB thật vẫn còn 'pending' (admin panel thấy khác hẳn user).
        // Báo lỗi và để user tải lại trang thử lại, key vẫn nguyên pending
        // nên vượt/quay lại KHÔNG bị mất tiến độ.
        http_response_code(503);
        render_notice_screen(
            'Hệ thống đang bận',
            'Server đang xử lý nhiều yêu cầu cùng lúc nên chưa kích hoạt được key ngay. Vui lòng tải lại trang này sau vài giây - tiến độ của bạn vẫn được giữ nguyên, không cần vượt link lại từ đầu.'
        );
        exit;
    }
    ...
    // 1. Ép SQLite xả bộ nhớ đệm WAL xuống file database chính ngay lập tức
    db_wal_checkpoint($db);

    // 2. Dọn dẹp các key pending bỏ dở trước đó của cùng user / IP cho game này
    // để tránh rác và không gây nhầm lẫn trong Admin Panel
    try {
        if ($tgUserId > 0) {
            $db->prepare("DELETE FROM keys WHERE status = 'pending' AND (tg_user_id = ? OR (client_ip = ? AND client_ip != '')) AND id != ?")
               ->execute([$tgUserId, $row['client_ip'] ?? '', $row['id']]);
        } elseif (!empty($row['client_ip'])) {
            $db->prepare("DELETE FROM keys WHERE status = 'pending' AND client_ip = ? AND game_id = ? AND id != ?")
               ->execute([$row['client_ip'], (int)$row['game_id'], $row['id']]);
        }
    } catch (\Throwable $e) {}

    // 3. ÉP BUỘC SAO LƯU CLOUD NGAY LẬP TỨC (Bỏ qua throttle 60s)
    // Đảm bảo trạng thái 'active' được đẩy lên Firebase & Supabase ngay tại thời điểm này,
    // dù server Render có restart hay sleep ngay sau đó thì dữ liệu trên Cloud vẫn luôn là 'active' 100%!
    trigger_cloud_backup($db, true);

    // BOT TỰ ĐỘNG GỬI MÃ KEY VỀ TÀI KHOẢN TELEGRAM CỦA USER (CHAT RIÊNG 1-1)
    if ($tgUserId > 0) {
        send_key_issued_telegram_notification($tgUserId, $row, $game);
    }

    // GỬI THÔNG BÁO PHÁT KEY LÊN KÊNH (CHANNEL) - CHE MÃ KEY & TAG TÊN USER, KHÔNG GỬI VÀO NHÓM CHAT
    send_key_issued_channel_broadcast($tgUserId, $row, $game);

    if ($game) {
        // Thời gian chặn lấy key lần kế = đúng thời hạn của key vừa lấy
        // Key mới chờ 24 giờ; key lịch sử theo thời hạn đã cấp.
...
$tgUserInfo = $tgUserId > 0 ? get_telegram_user_info($tgUserId) : null;
$isTelegramPremium = !empty($row['is_premium']) || !empty($tgUserInfo['is_premium']);
?>
<!DOCTYPE html>
...
body {
    background:
        radial-gradient(circle at 12% 0%, rgba(0, 242, 254, .18), transparent 30rem),
        radial-gradient(circle at 90% 16%, rgba(139, 92, 246, .16), transparent 28rem),
        radial-gradient(circle at 50% 90%, rgba(236, 72, 153, .07), transparent 26rem),
...
.perm-badge { display: inline-flex; align-items: center; gap: 4px; font: var(--fw-bold) 10px var(--font-mono); color: var(--violet); background: rgba(139, 92, 246, .12); padding: 4px 10px; border-radius: var(--r-full); margin-top: 8px; border: 1px solid rgba(139, 92, 246, .2); }
...
.keybox {
    ...
    border: 1px solid rgba(0, 242, 254, .35); overflow: hidden;
    box-shadow: inset 0 1px rgba(255,255,255,.06), 0 8px 24px -16px rgba(0, 242, 254, .5);
}
.keybox::after {
    content: ''; position: absolute; inset: 0; pointer-events: none;
    background: linear-gradient(115deg, transparent 38%, rgba(0, 242, 254, .18) 50%, transparent 62%);
...
.keycode {
    ...
    letter-spacing: .08em; color: var(--cyan); text-shadow: 0 0 24px rgba(0, 242, 254, .5);
...
.copybtn {
    ...
    cursor: pointer; transition: all var(--dur-normal) var(--ease-out); box-shadow: var(--shadow-btn);
}
.copybtn:hover { filter: brightness(1.08); transform: translateY(-2px); box-shadow: var(--shadow-btn-hover); }
.copybtn:active { transform: scale(.98); }
.copybtn.copied { background: linear-gradient(135deg, #10B981, #059669); color: #FFFFFF; box-shadow: 0 8px 24px -6px rgba(16, 185, 129, 0.6); }
...
<div class="card">
    ...
    <div class="card-top">
        ...
        <div style="display:flex;align-items:center;justify-content:center;gap:6px;flex-wrap:wrap;margin-top:8px">
            <?php
                $keyPkg = detect_key_package((int)$row['duration_seconds'], (int)($row['total_hops'] ?? 1));
            ?>
            <div class="perm-badge" style="background:rgba(0,242,254,.12);color:var(--cyan);border-color:rgba(0,242,254,.25)"><?= htmlspecialchars($keyPkg['badge']) ?></div>
            <?php if ($isPermanent): ?><div class="perm-badge">♾️ KEY VĨNH VIỄN</div><?php endif; ?>
            <?php if ($isTelegramPremium): ?><div><?= telegram_premium_badge_html('Telegram Premium ⭐', 'large') ?></div><?php endif; ?>
        </div>
    </div>
    ...
    <div class="card-bottom">
        ...
        <?php if ($tgUserId > 0): ?>
        <div style="margin-top:14px;padding:11px 13px;border-radius:var(--r-md);background:rgba(0,136,204,.12);border:1px solid rgba(0,136,204,.35);font-size:12px;color:var(--text);display:flex;align-items:flex-start;gap:10px;text-align:left">
            <span style="font-size:18px;line-height:1;margin-top:2px">🤖</span>
            <div style="flex:1">
                <div style="display:flex;align-items:center;gap:6px;flex-wrap:wrap">
                    <span style="font-weight:700;color:#0088cc">Bot đã gửi mã Key <?= htmlspecialchars($keyPkg['tag']) ?> về Telegram!</span>
                    <?php if ($isTelegramPremium): ?>
                        <?= telegram_premium_badge_html('Premium') ?>
                    <?php endif; ?>
                </div>
                <div style="font-size:11px;color:var(--text-dim);margin-top:3px;line-height:1.45">
                    ⚠️ <b>Cảnh báo quan trọng:</b> Bạn <b>bắt buộc</b> phải luôn ở trong nhóm Telegram yêu cầu. Nếu bạn thoát khỏi bất kỳ nhóm nào, Key này sẽ tự động <b>bị xoá vĩnh viễn</b> ngay lập tức!
                </div>
            </div>
        </div>
        <?php endif; ?>
